...
static int
_customer_system_init_spb5060s(
    void);

static int
_customer_system_post_init_spb5060s(
    void);
...
static const CUSTOMER_SYSTEM_INIT_FUNC_MAP_T _customer_system_init_funcs_map[]=
{
    ...
    {MW_PRODUCT_ID_SPB5060S_6GN, _customer_system_init_spb5060s, _customer_system_post_init_spb5060s},
};
    ...
    void)
{
    ...
    /* Hold AN8801SB in reset during switch/SDK initialization. */
    rc |= air_chipscu_setIomuxFuncState(
        0, AIR_CHIPSCU_IOMUX_FORCE_GPIO6_MODE, AIR_CHIPSCU_IOMUX_ENABLE);
    rc |= air_gpio_setDirection(GPIO_PIN6, AIR_GPIO_DIRECTION_OUTPUT);
    SET_GPIO_OUTPUT(GPIO_PIN6, rc);
    rc |= air_gpio_setValue(GPIO_PIN6, GPIO_PIN_LOW);

    rc |= air_chipscu_setIomuxFuncState(
        0, AIR_CHIPSCU_IOMUX_FORCE_GPIO13_MODE, AIR_CHIPSCU_IOMUX_ENABLE);
