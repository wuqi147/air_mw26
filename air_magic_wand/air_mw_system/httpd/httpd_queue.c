...
/* FILE NAME:  httpd_queue.c
 * PURPOSE:
 *  Implement internal queue function of httpd daemon.
 *
 * NOTES:
 *
 */
    ...
    UI32_T size)
{
    ...
    if (MW_E_OK != rc)
    {
        ...
        CGI_LOG_WARN(HTTPD, "DB is not ready");
        osapi_free(ptr_msg);
        return rc;
    }
    ...
    {
        /* This message could not be send, drop it */
        CGI_LOG_ERROR(HTTPD, "send request to Db failed");
        osapi_free(ptr_msg);
    }
    return rc;
}
 ...
 */
/* FUNCTION NAME: httpd_queue_init
 * PURPOSE:
...
/* FUNCTION NAME: httpd_queue_free
 * PURPOSE:
 *      Release all allocated memory in httpd queue.
 *
 * INPUT:
 *      None
 *
...
httpd_queue_free()
{
    osapi_msgDelete(HTTPD_QUEUE_CLI);
}

/* FUNCTION NAME: httpd_queue_recv
 * PURPOSE:
...
/* FUNCTION NAME: httpd_queue_send
 * PURPOSE:
    ...
    DB_MSG_T **pptr_out_msg)
{
    ...
    if (MW_E_OK != rc)
    {
        CGI_LOG_ERROR(HTTPD, "send queue: allocate memory failed(%d)", rc);
        return MW_E_NO_MEMORY;
    }
    CGI_LOG_DEBUG(HTTPD, "send queue: ptr_msg=0x%p", ptr_msg);

    /* message */
    osapi_strncpy(ptr_msg ->cq_name, HTTPD_QUEUE_CLI, DB_Q_NAME_SIZE);
    ptr_msg ->method = method;
    ptr_msg ->type.count = 1;
    CGI_LOG_DEBUG(HTTPD, "send queue: cq_name=%s, method=0x%X, count=%u, size=%u",
                  ptr_msg ->cq_name, ptr_msg ->method, ptr_msg ->type.count, size);
    ...
    CGI_LOG_DEBUG(HTTPD, "send queue: ptr_payload=0x%p, req_type=(%u, %u, %u), data_size=%u",
                  ptr_payload,
    ...
    /* Send message to DB */
    rc = _httpd_queue_send(ptr_msg, msg_size);
    if (MW_E_OK != rc)
    {
...
/* FUNCTION NAME: httpd_queue_setData
 * PURPOSE:
    ...
    const UI16_T size)
{
    ...
    rc = httpd_queue_send(method, t_idx, f_idx, e_idx, ptr_data, size, &ptr_msg);
    if (MW_E_OK != rc)
    {
        return rc;
    }
    ...
    rc = httpd_queue_recv((void **)&ptr_msg);
    if(MW_E_OK != rc)
    {
        CGI_LOG_ERROR(HTTPD, "set data: recv failed(%d)", rc);
    }
...
/* FUNCTION NAME: httpd_queue_getData
 * PURPOSE:
    ...
    void **pptr_out_data)
{
    ...
    if (MW_E_OK != rc)
    {
       CGI_LOG_ERROR(HTTPD, "get data: getDataSize failed(%d)", rc);
       return rc;
    }
    CGI_LOG_DEBUG(HTTPD, "get data: total_size = %d", total_size);

    rc = httpd_queue_send(M_GET, in_t_idx, in_f_idx, in_e_idx, NULL, total_size, &ptr_msg);
    if (MW_E_OK != rc)
    {
       CGI_LOG_ERROR(HTTPD, "get data: send failed(%d)", rc);
       return rc;
    }
    ...
    if(MW_E_OK != rc)
    {
        CGI_LOG_ERROR(HTTPD, "get data: recv failed(%d)", rc);
        osapi_free(ptr_msg);
        return rc;
    }
    ...
    CGI_LOG_DEBUG(HTTPD, "get data: *pptr_out_msg = 0x%p, *pptr_out_data = 0x%p", *pptr_out_msg, *pptr_out_data);
