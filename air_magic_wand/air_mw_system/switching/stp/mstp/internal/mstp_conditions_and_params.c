...
bool allSynced(const STP_BRIDGE* bridge, PortIndex givenPort, TreeIndex givenTree)
{
    ...
    for (portIndex = 0; portIndex < bridge->portCount; portIndex++)
    {
        ...
        portTree = bridge->ports[portIndex]->trees[givenTree];
        if (NULL == portTree)
        {
            continue;
        }
    ...
    if (NULL == bridge->ports[givenPort]->trees[givenTree])
    {
        return false;
    }
...
unsigned short EdgeDelay(const STP_BRIDGE* bridge, PortIndex givenPort)
{
    ...
    if (NULL == port)
    {
        return bridge->MigrateTime;
    }
    return port->operPointToPointMAC ? bridge->MigrateTime : bridge->ports[givenPort]->trees[CIST_INDEX]->designatedTimes.MaxAge;
}
...
{
    if ((NULL != bridge->ports[givenPort]) && (bridge->ports[givenPort]->sendRSTP))
    {
...
bool mstiDesignatedOrTCpropagatingRootPort(const STP_BRIDGE* bridge, PortIndex givenPort)
{
    ...
    for (mstiIndex = 1; mstiIndex < bridge->treeCount; mstiIndex++)
    {
        ...
        mstiInstance = bridge->ports[givenPort]->trees[mstiIndex];
        if (NULL == mstiInstance)
        {
            continue;
        }
...
bool mstiMasterPort(const STP_BRIDGE* bridge, PortIndex givenPort)
{
    ...
    for (mstiIndex = 1; mstiIndex < (bridge->treeCount); mstiIndex++)
    {
        if (NULL == port->trees[mstiIndex])
        {
            continue;
        }
        if (port->trees[mstiIndex]->role == STP_PORT_ROLE_MASTER)
        {
            return true;
        }
    }
...
bool rcvdMstiMsg(const STP_BRIDGE* bridge, PortIndex givenPort, TreeIndex givenTree)
{
    ...
    // this must be invoked on MSTIs only
