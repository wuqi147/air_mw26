...
#if defined(AIR_SUPPORT_MSTP) && defined(STP_DEBUG)
#define LOG(b,p,t,...)                      \
    do                                      \
    {                                       \
        if (mstp_GetPortStmDebugflag(p) || (0 == p))    \
        {                                   \
            MW_LOG_DEBUG(STP, __VA_ARGS__); \
        }                                   \
    }while(0);

#else
#define LOG(b,p,t,...)      ((void)0)
#endif
#define FLUSH_LOG(b)        ((void)0)
#define LOG_INDENT(b)       ((void)0)
#define LOG_UNINDENT(b)     ((void)0)
#endif
