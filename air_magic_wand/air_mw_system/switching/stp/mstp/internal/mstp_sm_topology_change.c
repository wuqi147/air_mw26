...
void mstp_setInstancePortFlush(unsigned int port, unsigned short fid, unsigned char ageTime);
#endif
...
static unsigned char CheckConditions(const STP_BRIDGE* bridge, PortTreeArgs* pt, unsigned char state)
{
    ...
    if (state == ACTIVE)
    {
        if (((portTree->role != STP_PORT_ROLE_ROOT) && (portTree->role != STP_PORT_ROLE_DESIGNATED)
                && (portTree->role != STP_PORT_ROLE_MASTER)) || (port->operEdge))
        {
            return LEARNING;
        }
    ...
    if (state == LEARNING)
    {
        if (((portTree->role == STP_PORT_ROLE_ROOT) || (portTree->role == STP_PORT_ROLE_DESIGNATED) || (portTree->role == STP_PORT_ROLE_MASTER))
                && (portTree->forward) && (!port->operEdge))
        {
            return DETECTED;
        }
        ...
        if ((portTree->role != STP_PORT_ROLE_ROOT) && (portTree->role != STP_PORT_ROLE_DESIGNATED)
                && (portTree->role != STP_PORT_ROLE_MASTER) && (!(portTree->learn || portTree->learning))
                && (!(portTree->rcvdTc || port->rcvdTcn || port->rcvdTcAck || portTree->tcProp)))
        {
            return INACTIVE;
        }
#else
...
static void InitState(STP_BRIDGE* bridge, PortTreeArgs* pt, unsigned char state, unsigned int timestamp)
{
    ...
    else if (state == INACTIVE)
    {
...
#ifdef AIR_SUPPORT_MSTP
        mstp_setInstancePortFlush(givenPort+1, givenTree, 1);
#endif
        ...
        if (port->operEdge == false)
        {
            ...
            LOG(bridge, givenPort+1, givenTree, "Flushing entries for P%u... \n", givenPort);
        }
