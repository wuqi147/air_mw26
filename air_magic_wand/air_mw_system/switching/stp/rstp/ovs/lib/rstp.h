...
#ifdef AIR_SUPPORT_RSTP
extern semaphorehandle_t rstp_mutex;
#else
 ...
 */
#ifdef AIR_SUPPORT_RSTP
#define RSTP_MIN_SEC_INTERVAL       (1)
#define RSTP_MAX_SEC_INTERVAL       (10)
#define RSTP_MIN_SEC_THRESHOLD      (0)
#define RSTP_MAX_SEC_THRESHOLD      (100)
#endif
 ...
 */
#ifdef AIR_SUPPORT_RSTP
typedef uint8_t  rstp_identifier[8];
#else
typedef uint64_t rstp_identifier;
#endif

#ifdef AIR_SUPPORT_RSTP
#define RSTP_ID_FMT "%02"PRIx8"%02"PRIx8".%02"PRIx8"%02"PRIx8"%02"PRIx8"%02"PRIx8"%02"PRIx8"%02"PRIx8
#define RSTP_ID_ARGS(rstp_id) \
    rstp_id[0], rstp_id[1],  \
    rstp_id[2], rstp_id[3], rstp_id[4], rstp_id[5], rstp_id[6], rstp_id[7]
#else
...
#ifdef AIR_SUPPORT_RSTP
/* Move ROLE_DISABLED to the begin, due to port role is not configuration and
 * will be clear as 0 every DB data init */
enum rstp_port_role {
    ROLE_DISABLED,
    ROLE_ROOT,
    ROLE_DESIGNATED,
...
uint16_t rstp_get_bridge_priority(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex);
...
void rstp_set_bridge_max_age(struct rstp *, int new_max_age)
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t rstp_get_bridge_max_age(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
void rstp_set_bridge_forward_delay(struct rstp *, int new_forward_delay)
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t rstp_get_bridge_forward_delay(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
void rstp_set_bridge_transmit_hold_count(struct rstp *,
    ...
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t rstp_get_bridge_transmit_hold_count(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
...
const char * rstp_get_name(const struct rstp *)
    OVS_EXCLUDED(rstp_mutex);
#ifdef AIR_SUPPORT_RSTP
void rstp_get_root_id(const struct rstp *, rstp_identifier)
    OVS_EXCLUDED(rstp_mutex);
void rstp_get_bridge_id(const struct rstp *, rstp_identifier)
    OVS_EXCLUDED(rstp_mutex);
void rstp_get_designated_id(const struct rstp *, rstp_identifier)
    OVS_EXCLUDED(rstp_mutex);
...
struct rstp_port * rstp_get_root_port(const struct rstp *)
    OVS_EXCLUDED(rstp_mutex);
#ifdef AIR_SUPPORT_RSTP
void rstp_get_designated_root(const struct rstp *, rstp_identifier)
    OVS_EXCLUDED(rstp_mutex);
#else
...
#endif
#ifdef AIR_SUPPORT_RSTP
void rstp_del_port(struct rstp *rstp, uint16_t port_number)
    OVS_EXCLUDED(rstp_mutex);
void rstp_run_stm(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex);
#endif
...
enum rstp_state rstp_port_get_state(const struct rstp_port *)
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
enum rstp_port_role rstp_port_get_role(struct rstp_port *p)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
...
#ifdef AIR_SUPPORT_RSTP
void rstp_set_port_mode__(UI32_T portMode)
    OVS_REQUIRES(rstp_mutex);
#endif
...
void rstp_port_set_priority(struct rstp_port *port, int priority)
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint8_t rstp_port_get_priority(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
...
void rstp_port_set_path_cost(struct rstp_port *port, uint32_t path_cost)
    OVS_EXCLUDED(rstp_mutex);
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint32_t rstp_port_get_path_cost(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex);
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
void rstp_port_set_state(struct rstp_port *p, enum rstp_state state)
    OVS_EXCLUDED(rstp_mutex);
#ifdef AIR_SUPPORT_RSTP
void rstp_port_set_admin_edge(struct rstp_port *port, bool admin_edge)
    OVS_EXCLUDED(rstp_mutex);
#ifdef STP_DEBUG
bool rstp_port_get_admin_edge(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex);
#endif /* STP_DEBUG */
void
