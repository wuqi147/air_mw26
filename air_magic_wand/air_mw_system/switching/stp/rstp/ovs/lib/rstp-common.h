...
#ifdef AIR_SUPPORT_RSTP
    PORT_ROLE_TRANSITION_SM_DESIGNATED_AGREE_EXEC,
#endif
...
struct rstp_port {
     ...
     *     defined in 6.5.
     */
    uint8_t admin_point_to_point_mac OVS_GUARDED_BY(rstp_mutex);
     ...
     *  d) Finally if the port is disabled, infoIs is Disabled.
     */
    uint8_t info_is OVS_GUARDED_BY(rstp_mutex);
     ...
     * Set to the result of the rcvInfo() procedure (17.21.8).
     */
    uint8_t rcvd_info OVS_GUARDED_BY(rstp_mutex);
     ...
     * The assigned Port Role (17.7).
     */
    uint8_t role OVS_GUARDED_BY(rstp_mutex);
     ...
     * The newly computed role for the Port (17.7, 17.28, 17.21.25, 17.19.35).
     */
    uint8_t selected_role OVS_GUARDED_BY(rstp_mutex);
    ...
    uint8_t rstp_state OVS_GUARDED_BY(rstp_mutex);
    bool state_changed OVS_GUARDED_BY(rstp_mutex);

    /* Per-port state machines state */
    uint8_t port_receive_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t port_protocol_migration_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t bridge_detection_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t port_transmit_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t port_information_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t port_role_transition_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t port_state_transition_sm_state OVS_GUARDED_BY(rstp_mutex);
    uint8_t topology_change_sm_state OVS_GUARDED_BY(rstp_mutex);

    uint8_t blk_while OVS_GUARDED_BY(rstp_mutex);

    bool align_reserve OVS_GUARDED_BY(rstp_mutex);
    bool enable_status OVS_GUARDED_BY(rstp_mutex);

    bool bpdu_status OVS_GUARDED_BY(rstp_mutex);
    uint8_t loop_status: 1,
            root_status: 1,
            tc_status:   1,
            tc_interval: 5 OVS_GUARDED_BY(rstp_mutex);
    uint8_t tc_threshold OVS_GUARDED_BY(rstp_mutex);
    uint32_t rcvd_tc_bpdu OVS_GUARDED_BY(rstp_mutex);
    uint32_t tc_guard_while:   8,
             is_interval_rcvd: 1,
             is_loop_inc:      1,
             is_root_inc:      1,
             loop_inc_block_while: 4,
             root_inc_block_while: 4,
             reserve:         14 OVS_GUARDED_BY(rstp_mutex);
};
#endif
