...
#else /* AIR_SUPPORT_RSTP */
semaphorehandle_t rstp_mutex = NULL;
#endif
...
static struct rstp_port *rstp_get_root_port__(const struct rstp *rstp)
    OVS_REQUIRES(rstp_mutex);
#ifdef AIR_SUPPORT_RSTP
static void rstp_get_root_id__(const struct rstp *rstp, rstp_identifier root_id)
    OVS_REQUIRES(rstp_mutex);
static void
rstp_port_set_enable_status__(struct rstp_port *port,bool status)
    OVS_REQUIRES(rstp_mutex);
static void
_rstp_initialize_port_stm(struct rstp_port *p)
    OVS_REQUIRES(rstp_mutex);
#else
...
{
#ifdef AIR_SUPPORT_RSTP
    osapi_mutexCreate("rstp", &rstp_mutex);
    if(NULL == rstp_mutex)
    {
        VLOG_DBG("create mutex fail\n");
        return;
    }
#else
    ...
    OVS_EXCLUDED(rstp_mutex)
{
    ...
    if (MW_E_OK != osapi_calloc(sizeof(struct rstp), STP_MODULE_NAME, (void**)&rstp))
    {
        VLOG_ERR("Fail to allocate memory");
        return NULL;
    }
    else
    {
        if (MW_E_OK != osapi_calloc(sizeof(C8_T)*(osapi_strlen("rstp0") + 1), STP_MODULE_NAME, (void**)&ptr_name))
        {
            VLOG_ERR("Fail to allocate memory");
            MW_FREE(rstp);
            return NULL;
        ...
        {
            memcpy(ptr_name, "rstp0", sizeof(C8_T)*(osapi_strlen("rstp0") + 1));
            rstp->name = ptr_name;
        }
    }
#else
...
#ifdef AIR_SUPPORT_RSTP
    osapi_memcpy(rstp->bridge_priority.root_bridge_id, rstp->bridge_identifier, sizeof(rstp_identifier));
    osapi_memcpy(rstp->bridge_priority.designated_bridge_id, rstp->bridge_identifier, sizeof(rstp_identifier));
#else
...
#ifdef AIR_SUPPORT_RSTP
    if (0 != osapi_memcmp(rstp->address, bridge_address, sizeof(rstp_identifier))) {
        osapi_memcpy(rstp->address, bridge_address, sizeof(rstp_identifier));
        osapi_memcpy(rstp->bridge_identifier + 2, bridge_address + 2, sizeof(rstp_identifier) - 2);
#else
...
#ifdef AIR_SUPPORT_RSTP
void
rstp_get_bridge_id(const struct rstp *rstp, rstp_identifier bridge_id)
    OVS_EXCLUDED(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    osapi_memcpy(bridge_id, rstp->bridge_identifier, sizeof(rstp_identifier));
    ovs_mutex_unlock(&rstp_mutex);
}
#else
rstp_identifier
...
#ifdef AIR_SUPPORT_RSTP
        new_priority = htons(new_priority);
        osapi_memcpy(rstp->bridge_identifier, &new_priority, sizeof(uint8_t) * 2);
#else
...
uint16_t
rstp_get_bridge_priority(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex)
{
    uint16_t priority;

    ovs_mutex_lock(&rstp_mutex);
    priority = rstp->priority;
    ovs_mutex_unlock(&rstp_mutex);
    ...
    OVS_REQUIRES(rstp_mutex)
{
    ...
    struct rstp_port *p;
#ifdef AIR_SUPPORT_RSTP
    uint16_t    forward_delay = 0;
    uint16_t    max_age = 0;
    uint16_t    transmit_hold_count = 0;
    uint16_t    priority = 0;
#endif /* AIR_SUPPORT_RSTP */

    /* Copy rstp in temp */
#ifndef AIR_SUPPORT_RSTP
    osapi_memcpy(&temp, rstp, sizeof(struct rstp));
#else
    temp = *rstp;
#endif
    ports = rstp->ports;
...
#ifdef AIR_SUPPORT_RSTP
    /* Set parameters values from DB */
    stp_db_get_forward_delay_config(&forward_delay);
    stp_db_get_max_age_config(&max_age);
    stp_db_get_transmit_hold_count_config(&transmit_hold_count);
    stp_db_get_bridge_priority_config(&priority);

    rstp_set_bridge_priority__(rstp, priority);
    rstp_set_bridge_ageing_time__(rstp, RSTP_DEFAULT_AGEING_TIME);
    rstp_set_bridge_forward_delay__(rstp, forward_delay);
    rstp_set_bridge_hello_time__(rstp);
    rstp_set_bridge_max_age__(rstp, max_age);
    rstp_set_bridge_migrate_time__(rstp);
    rstp_set_bridge_transmit_hold_count__(rstp,
                                          transmit_hold_count);
    rstp_set_bridge_times__(rstp, forward_delay,
                            RSTP_BRIDGE_HELLO_TIME,
                            max_age, 0);
    /* 17.18.7 rootTimes
     * The rootTimes variable comprises the Bridge's operational timer parameter
     * values (Message Age, Max Age, Forward Delay and Hello Time), derived from
     * the values stored in portTimes(17.19.22) for the Root Port of from
     * BridgeTimes(17.18.4)*/
    osapi_memcpy(&rstp->root_times, &rstp->bridge_times, sizeof(struct rstp_times));
#else
    /* Set default parameters values. */
    rstp_set_bridge_priority__(rstp, RSTP_DEFAULT_PRIORITY);
    rstp_set_bridge_ageing_time__(rstp, RSTP_DEFAULT_AGEING_TIME);
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
enum rstp_force_protocol_version
rstp_get_bridge_force_protocol_version(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex)
{
    enum rstp_force_protocol_version force_protocol_version;

    ovs_mutex_lock(&rstp_mutex);
    force_protocol_version = rstp->force_protocol_version;
    ovs_mutex_unlock(&rstp_mutex);

    return force_protocol_version;
}
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t
rstp_get_bridge_max_age(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex)
{
    uint16_t max_age;

    ovs_mutex_lock(&rstp_mutex);
    max_age = rstp->bridge_max_age;
    ovs_mutex_unlock(&rstp_mutex);
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t
rstp_get_bridge_forward_delay(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex)
{
    uint16_t forward_delay;

    ovs_mutex_lock(&rstp_mutex);
    forward_delay = rstp->bridge_forward_delay;
    ovs_mutex_unlock(&rstp_mutex);
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint16_t
rstp_get_bridge_transmit_hold_count(struct rstp *rstp)
    OVS_EXCLUDED(rstp_mutex)
{
    uint16_t transmit_hold_count;

    ovs_mutex_lock(&rstp_mutex);
    transmit_hold_count = rstp->transmit_hold_count;
    ovs_mutex_unlock(&rstp_mutex);

    return transmit_hold_count;
}
#endif /* defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG) */
    ...
    OVS_REQUIRES(rstp_mutex)
{
    if (port->port_path_cost != path_cost
        ...
        VLOG_DBG("%s, port %u, set RSTP port path cost to %lu",
...
{
    ovs_mutex_lock(&rstp_mutex);
    if (RSTP_ADMIN_P2P_MAC_AUTO == p->admin_point_to_point_mac)
    {
        rstp_port_set_oper_point_to_point_mac__(p, new_oper_p2p_mac);
    }
    ovs_mutex_unlock(&rstp_mutex);
}
#endif

/* Initializes a port stm with init state. */
static void
_rstp_initialize_port_stm(struct rstp_port *p)
    OVS_REQUIRES(rstp_mutex)
{
    ...
    OVS_REQUIRES(rstp_mutex)
{
#ifdef AIR_SUPPORT_RSTP
    uint8_t     priority = 0;
    uint8_t     admin_edge = 0;
#ifdef AIR_SUPPORT_STP_AUTO_COST
    uint8_t     auto_cost_support = 0;
    UI32_T      oper_cost = 0;
#else
    uint32_t    cost = 0;
#endif /* AIR_SUPPORT_STP_AUTO_COST */
    uint8_t     status = 0;
    uint8_t     admin_state = RSTP_ADMIN_BRIDGE_PORT_STATE_DISABLED;
#endif
    ...
    rstp_port_set_administrative_bridge_port__(p,
                                               ...
                                               true);
#else
    stp_db_get_port_enable_status(p->port_number, &status);
    admin_state = status ? RSTP_ADMIN_BRIDGE_PORT_STATE_ENABLED :\
                        RSTP_ADMIN_BRIDGE_PORT_STATE_DISABLED;
    rstp_port_set_administrative_bridge_port__(p,
                                               admin_state,
                                               true);
    ...
    rstp_get_port_security_param(p->port_number, &sec_info);
    p->bpdu_status = sec_info.bpdu_status;
    p->tc_status = sec_info.tc_status;
    p->root_status = sec_info.root_status;
    p->loop_status = sec_info.loop_status;
    p->tc_interval = 5;
    p->tc_threshold = 0;
    if ((sec_info.tc_interval >= RSTP_MIN_SEC_INTERVAL)
        && (sec_info.tc_interval <= RSTP_MAX_SEC_INTERVAL))
    {
        p->tc_interval = sec_info.tc_interval;
    }
    if ((sec_info.tc_threshold >= RSTP_MIN_SEC_THRESHOLD)
        && (sec_info.tc_threshold <= RSTP_MAX_SEC_THRESHOLD))
    {
        p->tc_threshold = sec_info.tc_threshold;
    }
#endif
#endif
#ifndef AIR_SUPPORT_RSTP
...
#else
    stp_db_get_port_priority_config(p->port_number, &priority);
#ifdef AIR_SUPPORT_STP_AUTO_COST
    stp_db_port_compute_oper_cost(p->port_number, &auto_cost_support, &oper_cost);
#else
    stp_db_get_port_cost_config(p->port_number, (UI32_T *)&cost);
#endif /* AIR_SUPPORT_STP_AUTO_COST */

    rstp_port_set_priority__(p, priority);
#ifdef AIR_SUPPORT_STP_AUTO_COST
    if (true == auto_cost_support) {
        rstp_db_port_oper_cost_update(p->port_number, oper_cost);
    }
    rstp_port_set_path_cost__(p, (uint32_t)oper_cost);
#else
    rstp_port_set_path_cost__(p, cost);
#endif /* AIR_SUPPORT_STP_AUTO_COST */

    rstp_port_set_admin_point_to_point_mac__(p, RSTP_ADMIN_P2P_MAC_AUTO);
...
#else
    stp_db_get_port_admin_edge_config(p->port_number, &admin_edge);
    rstp_port_set_admin_edge__(p, admin_edge);
#endif
     ...
     * Hello Time */
    osapi_memcpy(&p->designated_times, &p->rstp->root_times, sizeof(struct rstp_times));
    p->designated_times.hello_time = p->rstp->bridge_times.hello_time;
#endif
...
#ifdef AIR_SUPPORT_RSTP
    stp_port_state_data_update(p->port_number, RSTP_DISCARDING);
    p->mac_operational = temp_port.mac_operational;
    p->enable_status = temp_port.enable_status;
#endif

    rstp_initialize_port_defaults__(p);
#ifdef AIR_SUPPORT_RSTP
    p->oper_point_to_point_mac = temp_port.oper_point_to_point_mac;
#endif
    ...
    OVS_REQUIRES(rstp_mutex)
{
    ...
    p->rstp_state = state;
#ifdef AIR_SUPPORT_RSTP
    stp_port_state_data_update(p->port_number, (UI32_T)state);
#endif
}
...
/* Sets the port mode. */
void
rstp_set_port_mode__(UI32_T portMode)
    OVS_REQUIRES(rstp_mutex)
{
    stp_db_port_mode_update(portMode);
}

struct rstp_port *
rstp_add_port_w_port_number(struct rstp *rstp, uint16_t port_number)
    OVS_EXCLUDED(rstp_mutex)
{
    struct rstp_port    *p = NULL;
    MW_ERROR_NO_T       rc = MW_E_OK;
    rc = osapi_calloc(sizeof(struct rstp_port), STP_MODULE_NAME, (void **)&p);
    if (MW_E_OK != rc)
    {
        VLOG_ERR("Fail to allocate memory");
        return NULL;
    ...
    rstp_port_set_port_number__(p, port_number);
    p->aux = NULL;
    p->port_name = NULL;
    rstp_initialize_port_defaults__(p);
...
#ifdef AIR_SUPPORT_RSTP
void
rstp_del_port(struct rstp *rstp, uint16_t port_number)
    OVS_EXCLUDED(rstp_mutex)
{
    struct rstp_port *rp = NULL;
    ovs_mutex_lock(&rstp_mutex);
    rp = rstp_get_port__(rstp, port_number);
    if (NULL != rp)
    {
        rstp_port_set_state__(rp, RSTP_DISABLED);
        stp_fdb_update_mac_entry_timer_by_port(rp->port_number, 1);
        MW_FREE(rp->port_name);
...
#ifdef AIR_SUPPORT_RSTP
static void
rstp_port_set_enable_status__(
    struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    if (status)
    {
        /* set port discard */
        if (port->is_administrative_bridge_port != RSTP_ADMIN_BRIDGE_PORT_STATE_ENABLED)
        {
            rstp_port_set_state__(port, RSTP_DISCARDING);
        }
        rstp_port_set_administrative_bridge_port__(port,
                                                   RSTP_ADMIN_BRIDGE_PORT_STATE_ENABLED,
                                                   false);
    }
    else
    {
        rstp_port_set_administrative_bridge_port__(port,
                                                   RSTP_ADMIN_BRIDGE_PORT_STATE_DISABLED,
                                                   false);
        _rstp_initialize_port_stm(port);
        /* recover port forward */
        rstp_port_set_state__(port, RSTP_FORWARDING);
    }

    VLOG_DBG("%s, port %u: set RSTP enable status to %s", port->rstp->name,
             port->port_number, status ? "enable":"disable");
    return;
}
#endif
...
#ifdef AIR_SUPPORT_RSTP
void
rstp_get_designated_id(const struct rstp *rstp, rstp_identifier designated_id)
    OVS_EXCLUDED(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    osapi_memcpy(designated_id, rstp->root_priority.designated_bridge_id, sizeof(rstp_identifier));
    ovs_mutex_unlock(&rstp_mutex);
}
#else
rstp_identifier
...
#ifdef AIR_SUPPORT_RSTP
static void
rstp_get_root_id__(const struct rstp *rstp, rstp_identifier root_id)
    OVS_REQUIRES(rstp_mutex)
{
    osapi_memcpy(root_id, rstp->root_priority.root_bridge_id, sizeof(rstp_identifier));
}

void
rstp_get_root_id(const struct rstp *rstp, rstp_identifier root_id)
    OVS_EXCLUDED(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    rstp_get_root_id__(rstp, root_id);
    ovs_mutex_unlock(&rstp_mutex);
}
...
#ifdef AIR_SUPPORT_RSTP
void
rstp_get_designated_root(const struct rstp *rstp, rstp_identifier designated_root)
    OVS_EXCLUDED(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    osapi_memcpy(designated_root, rstp->root_priority.designated_bridge_id, sizeof(rstp_identifier));
    ovs_mutex_unlock(&rstp_mutex);
}
#else
rstp_identifier
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
enum rstp_port_role
rstp_port_get_role(struct rstp_port *p)
    OVS_EXCLUDED(rstp_mutex)
{
...
#ifdef AIR_SUPPORT_RSTP
    osapi_memcpy(designated_bridge_id, p->port_priority.designated_bridge_id, sizeof(rstp_identifier));
#else
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint8_t
rstp_port_get_priority(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex)
{
    uint8_t priority;
...
#if defined(AIR_SUPPORT_RSTP) && defined(STP_DEBUG)
uint32_t
rstp_port_get_path_cost(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex)
{
    uint32_t path_cost;

    ovs_mutex_lock(&rstp_mutex);
    path_cost = port->port_path_cost;
    ovs_mutex_unlock(&rstp_mutex);
...
#ifdef AIR_SUPPORT_RSTP
void
rstp_port_set_admin_edge(struct rstp_port *port, bool admin_edge)
    OVS_EXCLUDED(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    rstp_port_set_admin_edge__(port, admin_edge);
    ovs_mutex_unlock(&rstp_mutex);
}
#ifdef STP_DEBUG
bool
rstp_port_get_admin_edge(struct rstp_port *port)
    OVS_EXCLUDED(rstp_mutex)
{
    uint8_t admin_edge;

    ovs_mutex_lock(&rstp_mutex);
    admin_edge = port->admin_edge;
    ovs_mutex_unlock(&rstp_mutex);
...
void
rstp_port_set_mcheck(struct rstp_port *port, bool mcheck)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    rstp_port_set_mcheck__(port, mcheck);
    ovs_mutex_unlock(&rstp_mutex);
}

void
rstp_port_set_enable_status(struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    rstp_port_set_enable_status__(port, status);
    ovs_mutex_unlock(&rstp_mutex);
}

#ifdef AIR_SUPPORT_RSTP_SECURITY
void
rstp_port_set_bpdu_guard_status(struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    port->bpdu_status = status;
    ovs_mutex_unlock(&rstp_mutex);
}

void
rstp_port_set_tc_guard_status(struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    if (port->tc_status != status)
    {
        port->tc_status = status;
        port->tc_guard_while = 0;
        port->rcvd_tc_bpdu = 0;
        port->is_interval_rcvd = false;
    }
    ovs_mutex_unlock(&rstp_mutex);
}

void
rstp_port_set_tc_guard_interval(struct rstp_port *port, UI8_T interval)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    if (port->tc_interval != interval)
    {
        port->tc_interval = interval;
        port->tc_guard_while = 0;
        port->rcvd_tc_bpdu = 0;
        port->is_interval_rcvd = false;
    }
...
void
rstp_port_set_tc_guard_threshold(struct rstp_port *port, UI8_T threshold)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    if (port->tc_threshold != threshold)
    {
        port->tc_threshold = threshold;
        port->tc_guard_while = 0;
        port->rcvd_tc_bpdu = 0;
        port->is_interval_rcvd = false;
    }
...
void
rstp_port_set_root_guard_status(struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    if (port->root_status != status)
    {
        port->root_status = status;
        port->is_root_inc = false;
        rstp_db_port_update_inconsistent(port->port_number, false, RSTP_INCONSISTENT_ROOT);
    }
    ovs_mutex_unlock(&rstp_mutex);
}

void
rstp_port_set_loop_guard_status(struct rstp_port *port, bool status)
    OVS_REQUIRES(rstp_mutex)
{
    ovs_mutex_lock(&rstp_mutex);
    if (port->loop_status != status)
    {
        port->loop_status = status;
        port->is_loop_inc = false;
        rstp_db_port_update_inconsistent(port->port_number, false, RSTP_INCONSISTENT_LOOP);
    }
    ovs_mutex_unlock(&rstp_mutex);
}

void
rstp_port_set_loop_inc(UI32_T port, UI8_T state)
    OVS_REQUIRES(rstp_mutex)
{
    rstp_db_port_update_inconsistent(port, state, RSTP_INCONSISTENT_LOOP);
}

void
rstp_port_set_root_inc(UI32_T port, UI8_T state)
    OVS_REQUIRES(rstp_mutex)
{
    rstp_db_port_update_inconsistent(port, state, RSTP_INCONSISTENT_ROOT);
}
#endif /* AIR_SUPPORT_RSTP_SECURITY */
#endif /* AIR_SUPPORT_RSTP */
