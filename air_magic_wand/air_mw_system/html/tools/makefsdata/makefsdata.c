...
static u8_t *modified_data = NULL;
static size_t modified_size = 0;

typedef struct WEBFILE_T
{
    char webname[50];
}WEBFILE_S;

static WEBFILE_S webpage_list[] =
{
#ifndef AIR_SUPPORT_IGMP_SNP
    "cfg_IgmpSnoopingRpm.html",
#endif
#ifndef AIR_SUPPORT_LP
    "cfg_LoopPreventionRpm.html",
#endif
#ifndef AIR_SUPPORT_DHCP_SNOOP
    "security_DhcpSnooping.html",
#endif
#ifndef AIR_SUPPORT_ICMP_CLIENT
    "tools_icmpClient.html",
#endif
#ifndef AIR_SUPPORT_VOICE_VLAN
    "vlan_Voice.html",
#endif
#ifndef AIR_SUPPORT_SNMP
    "cfg_SnmpRpm.html",
#endif
#ifndef AIR_SUPPORT_STP
    "cfg_SpanningTree.html",
#ifndef AIR_SUPPORT_MSTP
    "cfg_SpanningTreeInstance.html",
#endif /*AIR_SUPPORT_MSTP*/
#endif
#ifndef AIR_SUPPORT_MQTTD
    "cfg_CloudConnection.html",
#endif
#ifndef AIR_SUPPORT_SFP_DDM
    "sys_OpticalModuleStatus.html",
#endif
#ifndef AIR_SUPPORT_SNTP
    "cfg_Systime.html",
#endif
#ifndef AIR_SUPPORT_ERPS
    "cfg_erps.html",
#endif /* AIR_SUPPORT_ERPS */
#ifndef AIR_SUPPORT_LLDPD
    "cfg_LldpConfigRpm.html",
    "cfg_LldpNeighborRpm.html",
#endif
#ifndef AIR_SUPPORT_SURVEILLANCE_VLAN
    "vlan_Surveillance.html",
#endif
#ifndef AIR_SUPPORT_POE
    "poe.html",
#endif
#ifndef AIR_SUPPORT_CABLE_DIAG
...
int main(int argc, char *argv[])
{
  ...
  strcpy(path, "./air_mw_system/html/webpage/lwip_http_fs");
  for (i = 1; i < argc; i++) {
    if (argv[i] == NULL) {
      continue;
    }
      ...
      for (i = 0; i < dir.n_files; i++) {
        ...
        if (ret == 0) {
          if (!file.is_dir) {
            ...
            for(idx = 0; idx < sizeof(webpage_list)/sizeof(WEBFILE_S); idx++)
            {
                if(0 == strcmp(curName, webpage_list[idx].webname))
                {
                    break;
                }
            }
            if(idx != sizeof(webpage_list)/sizeof(WEBFILE_S))
            {
                continue;
            }
...
static void preprocess_file(const char *tag, const char *repl_tag, u8_t *file_data, size_t file_size, int is_ssi)
{
    u8_t *new_data = NULL;
    int tag_cnt = 0, new_size = 0;
    const size_t tag_len = strlen(tag);
    const size_t repl_tag_len = strlen(repl_tag);

    if (is_ssi)
    {
        for (size_t idx = 0; idx < file_size;)
        {
            if (((idx + tag_len) <= file_size) &&
                (memcmp(file_data + idx, tag, tag_len) == 0))
            {
                tag_cnt++;
                idx += tag_len;
            }
        ...
        new_size = file_size + tag_cnt * (repl_tag_len - tag_len);
        new_data = (u8_t *)malloc(new_size + 1);
        if (new_data)
        {
            size_t new_idx = 0;

            memset(new_data, 0, new_size + 1);
            for (size_t old_idx = 0; old_idx < file_size;)
            {
                if (((old_idx + tag_len) <= file_size) &&
                    (memcmp(file_data + old_idx, tag, tag_len) == 0))
                {
                    memcpy(new_data + new_idx, repl_tag, repl_tag_len);
                    new_idx += repl_tag_len;
                    old_idx += tag_len;
                }
                else
                {
                    new_data[new_idx++] = file_data[old_idx++];
                }
            }
            if (modified_data)
            {
...
static void process_file_data(FILE *data_file, u8_t *file_data, size_t file_size, int is_ssi)
{
    size_t written, i = 0, src_off = 0;
    size_t off = 0;
    u8_t  *processed_data = NULL;
    size_t processed_size = 0;

#ifdef AIR_SUPPORT_HTTPD_SECURE
    /* replace html content with ssi tags */
    preprocess_file("<script", "<!--#js-->", file_data, file_size, is_ssi);
    preprocess_file("<style", "<!--#css-->", modified_data, modified_size, is_ssi);
#endif

    /* used preprocessed data if httpd secure is enabled */
    processed_data = (modified_data != NULL) ? modified_data : file_data;
    processed_size = (modified_data != NULL) ? modified_size : file_size;

#ifdef HTTP_TRANSFER_ENCODING_CHUNKED
    unsigned char *tag_s = NULL, *tag_e = NULL;
    int tag_len = 0, tag_offt = 0;
    char block_len[5] = {0};
    char j = 0;
    char is_xml = 0;
#else
    LWIP_UNUSED_ARG(is_ssi); /* for LWIP_NOASSERT */
#endif
    ...
    if(is_ssi)
    {
        ...
        if(NULL != tag_s)
        {
            do
            {
                ...
                tag_e += 3;
                tag_len = tag_e - tag_s;
                tag_offt = tag_s - processed_data;
                /* Chekc is xml request */
                if(tag_len == processed_size)
                {
                    is_xml = 1;
                    break;
                }

                memset(block_len, 0, 5);
                sprintf(block_len, "%lx", (tag_offt - i));
                ...
                for(j = 0; j < 5; j++)
                {
                    if(block_len[j] != 0)
                    {
                        sprintf(&file_buffer_c[off], "0x%02x,", block_len[j]);
                        off += 5;
                    }
                }
                memcpy(&file_buffer_c[off], "0x0d,0x0a,", 10);
                off += 10;
                memcpy(&file_buffer_c[off], NEWLINE, NEWLINE_LEN);
                off += NEWLINE_LEN;

                for ( ; i < processed_size; i++) {
                    if(i  == tag_offt)
                    {
                        memcpy(&file_buffer_c[off], "0x0d,", 5);
                        off += 5;
                        ...
                        memcpy(&file_buffer_c[off], "0x0a,", 5);
                        off += 5;
                        if ((++src_off % HEX_BYTES_PER_LINE) == 0) {
                            LWIP_ASSERT("file_buffer_c overflow", off < sizeof(file_buffer_c) - NEWLINE_LEN);
                            memcpy(&file_buffer_c[off], NEWLINE, NEWLINE_LEN);
                        ...
                        for(j = 0; j < tag_len; j++)
                        {
                            sprintf(&file_buffer_c[off], "0x%02x,", tag_s[j]);
                            off += 5;
                            if ((++src_off % HEX_BYTES_PER_LINE) == 0) {
                                LWIP_ASSERT("file_buffer_c overflow", off < sizeof(file_buffer_c) - NEWLINE_LEN);
                                memcpy(&file_buffer_c[off], NEWLINE, NEWLINE_LEN);
                        ...
                        memcpy(&file_buffer_c[off], NEWLINE, NEWLINE_LEN);
                        off += NEWLINE_LEN;
                        memcpy(&file_buffer_c[off], "/* -- Tag End -- */\r\n", 21);
                        off += 21;
                        src_off = 0;
                        i += tag_len;
                        break;
                    ...
                    sprintf(&file_buffer_c[off], "0x%02x,", processed_data[i]);
                    off += 5;
                    if ((++src_off % HEX_BYTES_PER_LINE) == 0) {
                        LWIP_ASSERT("file_buffer_c overflow", off < sizeof(file_buffer_c) - NEWLINE_LEN);
                        memcpy(&file_buffer_c[off], NEWLINE, NEWLINE_LEN);
            ...
            {
                /* Next Block size */
                sprintf(block_len, "%lx", (processed_size - i));
                for(j = 0; j < 5; j++)
                {
                    if(block_len[j] != 0)
                    {
        ...
        {
            /* Create first block */
            sprintf(block_len, "%lx", processed_size);
            for(j = 0; j < 5; j++)
            {
                if(block_len[j] != 0)
                {
        ...
        if(0 == is_xml)
        {
            /* End block 0x0,0x0d,0x0a,0x0d,0x0a,*/
            memcpy(&file_buffer_c[off], "0x0d,0x0a,0x30,0x0d,0x0a,0x0d,0x0a,", 35);
            off += 35;
        }
        else
        {
            memcpy(&file_buffer_c[off], "0x30,0x0d,0x0a,0x0d,0x0a,", 25);
            off += 25;
        }
    }
    else
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename)
{
...
#ifdef HTTP_TRANSFER_ENCODING_CHUNKED
    fputs("FS_FILE_FLAGS_SSI | FS_FILE_FLAGS_HEADER_CHUNKED", struct_file);
#else
  ...
  process_file_data(data_file, file_data, file_size, is_ssi);
  fprintf(data_file, "};" NEWLINE NEWLINE);
  free(file_data);
  return 0;
}
                           ...
                           u16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed)
{
  ...
  if (provide_last_modified) {
    ...
    /* Add set cookie if the web page is index_re.html */
    if(!strcmp(filename, "index_re.html"))
    {
        cur_string = g_psHTTPHeaderStrings[HTTP_HDR_SET_COOKIES];
        cur_len = strlen(cur_string);
        fprintf(data_file, NEWLINE "/* \"%s\" (%"SZT_F" bytes) */" NEWLINE, cur_string, cur_len);
        written += file_put_ascii(data_file, cur_string, cur_len, &i);
        i = 0;
...
#ifdef HTTP_TRANSFER_ENCODING_CHUNKED
            cur_string = g_psHTTPHeaderStrings[HTTP_TRANSER_ENCODING_TRUNK];
#else
            cur_string = g_psHTTPHeaderStrings[HTTP_HDR_CONN_CLOSE];
#endif
        }
