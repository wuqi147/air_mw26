 ...
 */
static int xvprintf_help(unsigned c, void **ptr){
...
static int uart_printf_help(unsigned c, void **ptr){
    extern int putchar(int c);
...
void osapi_write_to_ram(
    unsigned c){
#ifdef AIR_SUPPORT_REMOTE_DEBUG
    if ((TRUE == remote_debug_log_has_active_session()) && (MW_E_OK == remote_debug_log_init_done()))
    {
        remote_debug_set_tick_count(xTaskGetTickCount());
        remote_debug_log_write_to_ram(c);
#if !defined(AIR_SUPPORT_CLI)
        if ('\n' == c)
        {
            remote_debug_log_write_to_ram('\r');
        }
#endif
    }
#endif
...
int xvprintf(
    ...
    return do_printf(fmt, args, xvprintf_help, (void *)0);
}
...
int uart_vprintf(
    ...
    return do_printf(fmt, args, uart_printf_help, (void *)0);
}
