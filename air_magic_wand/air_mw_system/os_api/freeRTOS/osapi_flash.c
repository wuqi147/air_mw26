    ...
    const UI32_T size)
{
    ...
    if (flash_partition_copy(src_part, dst_part, size) != 0)
    {
    ...
    const UI8_T  part)
{
    ...
    if (flash_partition_read(ptr_data, size, offset, part) != 0)
    {
        ret = MW_E_BAD_PARAMETER;
    }
    return ret;
    ...
    const UI8_T  part)
{
    ...
    if (INIT == opt)
    {
        if (0 != flash_partition_write_init(part))
            ret = MW_E_OTHERS;
    }
    else if(WRITE == opt)
    {
        MW_CHECK_PTR(ptr_data);
        MW_PARAM_CHK((0 == size), MW_E_BAD_PARAMETER);
        if (0 != flash_partition_write((unsigned char *) ptr_data, size))
            ret = MW_E_OTHERS;
    }
    else if(END == opt)
    {
        if (0 != flash_partition_write_finish())
            ret = MW_E_OTHERS;
    }
    else
    {
