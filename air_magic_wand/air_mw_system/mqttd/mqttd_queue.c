    ...
    DB_MSG_T **pptr_out_msg)
{
    ...
    {
        MW_LOG_ERROR(MQTTD, "mqttd queue does not exist");
        return MW_E_NOT_INITED;
    }
    ...
    if (MW_E_OK != rc)
    {
        MW_LOG_ERROR(MQTTD, "%s: Send message to DB failed(%d)", __func__, rc);
        return rc;
    }
    ...
    const UI16_T size)
{
    ...
    if (MW_E_OK != rc)
    {
        MW_LOG_ERROR(MQTTD, "%s: mqttd_queue_send failed(%d)", __func__, rc);
    }
    return rc;
}
    ...
    void **pptr_out_data)
{
    ...
    if (MW_E_OK != rc)
    {
       MW_LOG_ERROR(MQTTD, "dbapi_getDataSize failed(%d)\n", rc);
       return rc;
    }
    ...
    if (MW_E_OK != rc)
    {
       MW_LOG_ERROR(MQTTD, "mqttd_queue_send failed(%d)\n", rc);
       return rc;
    }
    ...
    if(MW_E_OK == rc)
    {
        MW_LOG_INFO(MQTTD, "mqttd_queue_recv success \n");
    }
    else
    {
        MW_LOG_ERROR(MQTTD, "mqttd_queue_recv failed(%d) \n", rc);
        osapi_free(ptr_msg);
        return rc;
    }
    ...
    MW_LOG_DEBUG(MQTTD, "*pptr_out_msg = %p, *pptr_out_data = %p \n", *pptr_out_msg, *pptr_out_data);
