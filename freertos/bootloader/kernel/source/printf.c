...
#include "ace_log.h"
#define ETHERNET_TEXT_LOG_BUFFER_SIZE     (256)
#endif
...
int printf(const char *fmt, ...){
	...
	{
		uint8_t buffer[ETHERNET_TEXT_LOG_BUFFER_SIZE];
		va_start(args, fmt);
		ret_val = vsprintf((char *)buffer, fmt, args);
		va_end(args);

		send_ace_log(buffer, sizeof(buffer), NULL);
		return ret_val;
	}
#endif
