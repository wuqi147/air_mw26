...
/* check if we can flush data */
size_t mbedtls_ssl_flush_outleft_get(mbedtls_ssl_context *ssl)
{
    if (ssl == NULL) {
        return 0;
    }
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl)
{
    ...
    /* when need send data by ssl->f_send and update ssl->outleft infor, should take ssl mutex */
    if( ssl->mutex )
    {
        xSemaphoreTake( ssl->mutex, (5000 / portTICK_RATE_MS) );
    }
