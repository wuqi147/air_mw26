...
#define PDMA_RXDESC_UNIT                (6)
#define PDMA_RXDESC_NO                  (PDMA_RXDESC_UNIT*4)
#define PDMA_TX_QUEUE_NUM               (1)
#define PDMA_TXDESC_NO                  (2)
...
#define PDMA_RX_PKT_LEN                 PDMA_4BYTES_ALIGN(830)
#define PDMA_TX_PKT_LEN                 PDMA_4BYTES_ALIGN(PDMA_ETHERNET_TX_PKT_LEN)
#define PDMA_ISR_RX_PKT_THRESHOLD       (16)
#define PDMA_TX_BUF_RELEASE_THRESHOLD   (1)

/* RX buffer re-alloacted in the gdmpsram */
//#define ETHERNET_RX_PKT_BUFFER_ADDR     GDMPSRAM_BASE
#define ETHERNET_RX_PKT_BUFFER_NUM      (5)
...
typedef struct
{
    unsigned int use;
    unsigned int len;
    unsigned char data[PDMA_ETHERNET_RX_PKT_LEN];
} ETHERNET_PKT_MSG_T;
...
typedef struct
{
    ...
    SemaphoreHandle_t  rx_pkt_mutex;
 ...
 */
GDMPSRAM_DATA static PDMA_CFG_T g_pdma_cfg = {0};
GDMPSRAM_BSS  static ETHERNET_PKT_MSG_POOL_T g_pdma_rx_pool;

/* debug vars */
GDMPSRAM_DATA static unsigned int g_pdma_tx_full_cnt = 0;
GDMPSRAM_DATA static unsigned int g_pdma_tx_done_bit_err = 0;
GDMPSRAM_DATA static unsigned char g_debug_detail = 0;
GDMPSRAM_DATA static unsigned char g_debug_record = 0;
GDMPSRAM_DATA static unsigned int g_debug_record_cnt = 0;
GDMPSRAM_DATA static unsigned int g_debug_record_drop_cnt = 0;
GDMPSRAM_DATA static unsigned int g_debug_record_get_cnt = 0;
GDMPSRAM_DATA static unsigned int g_debug_record_release_cnt = 0;

extern GDMPSRAM_BSS QueueHandle_t g_pkt_queue;
...
void pdma_debug(const signed char * cmd)
{
    ...
    else if (!strncmp(sub_cmd, "dump", strlen("dump")))
    {
        printf("g_pdma_tx_full_cnt          :0x%d\n", g_pdma_tx_full_cnt);
        printf("g_pdma_tx_done_bit_err      :0x%d\n", g_pdma_tx_done_bit_err);
        printf("g_debug_detail              :0x%d\n", g_debug_detail);
        printf("g_debug_record              :0x%d\n", g_debug_record);
        printf("g_debug_record_cnt          :0x%d\n", g_debug_record_cnt);
        printf("g_debug_record_drop_cnt     :0x%d\n", g_debug_record_drop_cnt);
        printf("g_debug_record_get_cnt      :0x%d\n", g_debug_record_get_cnt);
        printf("g_debug_record_release_cnt  :0x%d\n", g_debug_record_release_cnt);
    }
}
...
void* pdma_get_ethernet_pkt_buf(unsigned int index, unsigned int* ptr_len)
{
    ...
    p_rx_pkt_msg = &g_pdma_rx_pool.rx_pkt_msg[index];
...
void pdma_release_ethernet_pkt_buf(unsigned int index)
{
    ...
    p_rx_pkt_mutex = g_pdma_rx_pool.rx_pkt_mutex;
    p_rx_pkt_msg = &g_pdma_rx_pool.rx_pkt_msg[index];
...
unsigned char* pdma_get_tx_pkt_buf(int len, int txq)
{
    TX_DESC_T *p_tx_desc = NULL;

    if ((len > PDMA_ETHERNET_TX_PKT_LEN) || (txq >= PDMA_TX_QUEUE_NUM))
    {
        return NULL;
    }
    ...
    //if (g_pdma_cfg.tx_unreleased_buf_cnt[txq] == PDMA_TXDESC_NO)
    if (g_pdma_cfg.tx_unreleased_buf_cnt[txq] != 0)
    {
        g_pdma_tx_full_cnt++;
    ...
    return g_pdma_cfg.tx_pkt_data[txq][g_pdma_cfg.tx_cur_desc[txq]];
}
            ...
            if (g_pkt_queue)
            {
                ...
                if (xSemaphoreTakeFromISR(p_rx_pkt_mutex, NULL))
                {
                    for (eth_pkt_idx = 0; eth_pkt_idx < ETHERNET_RX_PKT_BUFFER_NUM; eth_pkt_idx++)
                    {
                        ...
                        p_rx_pkt_msg = &g_pdma_rx_pool.rx_pkt_msg[g_pdma_rx_pool.rx_cur_pkt_msg];
                        if (pdFALSE == p_rx_pkt_msg->use)
                        {
                            ...
                            if (pdPASS == xQueueSendFromISR(g_pkt_queue, &g_pdma_rx_pool.rx_cur_pkt_msg, NULL))
                            {
                        ...
                        g_pdma_rx_pool.rx_cur_pkt_msg = (g_pdma_rx_pool.rx_cur_pkt_msg + 1) % ETHERNET_RX_PKT_BUFFER_NUM;
                        if (pdFALSE == drop_pkt)
                        {
                            break;
                        }
                ...
                if (xSemaphoreTakeFromISR(p_rx_pkt_mutex, NULL))
                {
                    for (eth_pkt_idx = 0; eth_pkt_idx < ETHERNET_RX_PKT_BUFFER_NUM; eth_pkt_idx++)
                    {
                        ...
                        if (pdFALSE == p_rx_pkt_msg->use)
                        {
                            ...
                            if (pdPASS == xQueueSendFromISR(g_pkt_queue, &g_pdma_rx_pool.rx_cur_pkt_msg, NULL))
                            {
                                PDMA_DEBUG_DETAIL("\r\nDebug: rx_cur_pkt_msg %d, data 0x%x, len %d\r\n", g_pdma_rx_pool.rx_cur_pkt_msg, p_rx_pkt_msg->data, p_rx_pkt_msg->len);
                            }
                            else
                            {
                                /* roll back rx pkt buf use flag */
...
void pdma_init (void)
{
    ...
    PDMA_DEBUG_DETAIL("Ethernet buffer ptr: 0x%x, Ethernet buffer cnt: %d\n", &g_pdma_rx_pool, ETHERNET_RX_PKT_BUFFER_NUM);
    g_pdma_rx_pool.rx_cur_pkt_msg = 0;
    ...
    PDMA_DEBUG_DETAIL("Ethernet buffer start ptr: 0x%x\n", g_pdma_rx_pool.rx_pkt_msg);
    g_pdma_rx_pool.rx_pkt_mutex = xSemaphoreCreateMutex("pdma");
    if (!g_pdma_rx_pool.rx_pkt_mutex)
    {
    ...
    for (txq = 0; txq < PDMA_TX_QUEUE_NUM; txq++)
    {
        for (i = 0; i < PDMA_TXDESC_NO; i++)
        {
            ...
            // For AN8855, tx only use 1 buf, 2 descriptors point to same 1buf
