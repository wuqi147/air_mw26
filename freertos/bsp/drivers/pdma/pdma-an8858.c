...
#define PDMA_RX_DROP_JUMB0                              PDMA_CSR_REG(0x260)
#define PDMA_RX_DROP_JUMB0_DROP_ENABLE_OFFT             (31)
#define PDMA_RX_DROP_JUMB0_DROP_ENABLE_RELMASK          (0x1)
#define PDMA_RX_DROP_JUMB0_DROP_ENABLE_MASK             (PDMA_RX_DROP_JUMB0_DROP_ENABLE_RELMASK << PDMA_RX_DROP_JUMB0_DROP_ENABLE_OFFT)
#define PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_OFFT            (30)
#define PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_RELMASK         (0x1)
#define PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_MASK            (PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_RELMASK << PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_OFFT)
#define PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_OFFT       (0x0)
#define PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_RELMASK    (0x3FFF)
#define PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_MASK       (PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_RELMASK << PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_OFFT)

#define PDMA_RX_DROP_CNT                                PDMA_CSR_REG(0x264)
...
#define PDMA_BUFFER_START_ADDR          (0x8003D000)
#define PDMA_RX_QUEUE_NUM               (1)
#define PDMA_RXDESC_NO                  (5)
...
#define PDMA_ISR_RX_PKT_THRESHOLD       (4)
...
typedef struct
{
    ...
    void *rx_pkt_data[PDMA_RX_QUEUE_NUM][PDMA_RXDESC_NO];
...
static unsigned char g_pdma_inited = 0;

/*looptest command use*/
struct loopback_test{
    unsigned int p_mask;
    unsigned int p_agc;
    unsigned int start_time;
    unsigned int end_time;
    unsigned int cmp_fail;
    unsigned int rx_cnt;
    unsigned int tx_cnt;
}g_loopback_test;
...
static unsigned int g_test_tx_random = 0;
static unsigned int g_test_tx_len = 64;
static unsigned int g_test_tx_delay = 0;
static unsigned char g_test_sa[6] = {0x00, 0x10, 0x94, 0x00, 0x00, 0x11};
static unsigned char g_test_da[6] = {0x00, 0x02, 0x00, 0x02, 0x00, 0x01};
static unsigned char g_test_pkt[PDMA_RX_PKT_LEN];
static unsigned char g_loop_pkt[PDMA_RX_PKT_LEN];
static unsigned int  g_loop_pkt_len = 0;
 ...
 */
static void pdma_loopback_test(void *pvParameters)
{
    int i;
    unsigned int *p = (unsigned int *)pvParameters;
    unsigned int time =  (*p)*configTICK_RATE_HZ;

    /* prepare packet payload */
    g_test_pkt[0]  = g_test_da[0];
    g_test_pkt[1]  = g_test_da[1];
    g_test_pkt[2]  = g_test_da[2];
    g_test_pkt[3]  = g_test_da[3];
    g_test_pkt[4]  = g_test_da[4];
    g_test_pkt[5]  = g_test_da[5];
    g_test_pkt[6]  = g_test_sa[0];
    g_test_pkt[7]  = g_test_sa[1];
    g_test_pkt[8]  = g_test_sa[2];
    g_test_pkt[9]  = g_test_sa[3];
    g_test_pkt[10] = g_test_sa[4];
    g_test_pkt[11] = g_test_sa[5];
    g_test_pkt[12] = 0x88;
    g_test_pkt[13] = 0xb5;

    for (i = 14; i < PDMA_RX_PKT_LEN; i++)
    {
        if (i % 2)
            g_test_pkt[i] = 0xaa;
        else
    ...
    io_write32(0x1020943c, 0x80000400);
    g_loopback_test.p_agc = io_read32(0x1020000c);
    io_write32(0x1020000c, 0x00001899);
    g_loopback_test.p_mask = io_read32(INT_MASK);
    io_write32(INT_MASK, RX_COHERENT | TX_COHERENT);

    g_loopback_test.start_time = tmpCpuTimerCnt;
    g_loopback_test.rx_cnt = 0;
    g_loopback_test.tx_cnt = 0;
    g_loopback_test.cmp_fail = 0;

    while(1)
    {
        if (0 == pdma_tx_pkt(g_test_pkt, g_test_tx_len, 0, 0))
        {
            g_loopback_test.tx_cnt++;

            while(0 != pdma_rx_pkt(0, 1));
                //dump rx pkt
                g_loopback_test.rx_cnt++;

                //compare tx & rx pkt content
                if (g_test_tx_len == g_loop_pkt_len)
                {
                    for (i = 0; i < (g_test_tx_len); i++)
                    {
                        if (g_test_pkt[i] != g_loop_pkt[i])
                        {
                            printf("test pkt : 0x%p = 0x%x\n", &g_test_pkt[i], g_test_pkt[i]);
                            dump_pkt_data(&g_test_pkt[0], g_test_tx_len);

                            printf("loop pkt : 0x%p = 0x%x\n", &g_loop_pkt[i], g_loop_pkt[i]);
                            dump_pkt_data(&g_loop_pkt[0], g_loop_pkt_len);

                            g_loopback_test.cmp_fail++;
                            break;
                        }
                    }
                }
                ...
                {
                    g_loopback_test.cmp_fail++;
                    printf("g_test_tx_len: %d\n", g_test_tx_len);
                    printf("g_loop_pkt_len: %d\n", g_loop_pkt_len);
                }
        }

        g_loopback_test.end_time = tmpCpuTimerCnt;

        if ((g_loopback_test.end_time - g_loopback_test.start_time) > time)
            break;
    }

    io_write32(INT_MASK, g_loopback_test.p_mask);
    io_write32(0x1020943c, 0);
    io_write32(0x1020000c, g_loopback_test.p_agc);
    xTaskNotifyGive(xTaskGetHandle("CMD"));
    vTaskSuspend(NULL);
}
...
void pdma_release_ethernet_pkt_buf(unsigned int index)
{
    ...
    if (xSemaphoreTake(p_rx_pkt_mutex, portMAX_DELAY))
    {
        ...
        {
            printf("Warning: Abnormal status of ethernet pkt buf %d\n", index);
        }
    ...
    {
        printf("Error: Failed to release ethernet pkt buf %d\n", index);
    }
}
...
void pdma_dump_rx_desc(void)
{
    ...
    for (rxq = 0; rxq < PDMA_RX_QUEUE_NUM; rxq++)
    {
        ...
        for (i = 0 ; i< PDMA_RXDESC_NO; i++)
        {
            ...
            //printf("DESC %d\n", i);
            //printf(" INFO1.PDP0      : 0x%x\n", p_rx_desc->rxd_info1.bits.PDP0);
            //printf(" INFO2.PLEN1     : 0x%x\n", p_rx_desc->rxd_info2.bits.PLEN1);
            //printf(" INFO2.LS1       : 0x%x\n", p_rx_desc->rxd_info2.bits.LS1);
            //printf(" INFO2.UN_USED   : 0x%x\n", p_rx_desc->rxd_info2.bits.UN_USED);
            //printf(" INFO2.PLEN0     : 0x%x\n", p_rx_desc->rxd_info2.bits.PLEN0);
            //printf(" INFO2.LS0       : 0x%x\n", p_rx_desc->rxd_info2.bits.LS0);
            //printf(" INFO2.DDONE_bit : 0x%x\n", p_rx_desc->rxd_info2.bits.DDONE_bit);
            //printf(" INFO3.UN_USE1   : 0x%x\n", p_rx_desc->rxd_info3.bits.UN_USE1);
            //printf(" INFO4.FOE_Entry : 0x%x\n", p_rx_desc->rxd_info4.bits.FOE_Entry);
            //printf(" INFO4.CRSN      : 0x%x\n", p_rx_desc->rxd_info4.bits.CRSN);
            //printf(" INFO4.SPORT     : 0x%x\n", p_rx_desc->rxd_info4.bits.SPORT);
            //printf(" INFO4.PKT_INFO  : 0x%x\n", p_rx_desc->rxd_info4.bits.PKT_INFO);
            //printf(" INFO4.RSV       : 0x%x\n", p_rx_desc->rxd_info4.bits.RSV);
        }
    }
}
...
void pdma_dump_tx_desc(void)
{
    ...
    for (txq = 0; txq < PDMA_TX_QUEUE_NUM; txq++)
    {
        ...
        for (i = 0 ; i < PDMA_TXDESC_NO ; i++)
        {
            ...
            //printf("DESC %d\n", i);
            //printf(" INFO1.SDP0      : 0x%x\n", p_tx_desc->txd_info1.bits.SDP0);
            //printf(" INFO2.SDL1      : 0x%x\n", p_tx_desc->txd_info2.bits.SDL1);
            //printf(" INFO2.LS1_bit   : 0x%x\n", p_tx_desc->txd_info2.bits.LS1_bit);
            //printf(" INFO2.BURST_bit : 0x%x\n", p_tx_desc->txd_info2.bits.BURST_bit);
            //printf(" INFO2.SDL0      : 0x%x\n", p_tx_desc->txd_info2.bits.SDL0);
            //printf(" INFO2.LS0_bit   : 0x%x\n", p_tx_desc->txd_info2.bits.LS0_bit);
            //printf(" INFO2.DDONE_bit : 0x%x\n", p_tx_desc->txd_info2.bits.DDONE_bit);
            //printf(" INFO3.SDP1      : 0x%x\n", p_tx_desc->txd_info3.bits.SDP1);
            //printf(" INFO4.VIDX      : 0x%x\n", p_tx_desc->txd_info4.bits.VIDX);
            //printf(" INFO4.CFI       : 0x%x\n", p_tx_desc->txd_info4.bits.CFI);
            //printf(" INFO4.VPRI      : 0x%x\n", p_tx_desc->txd_info4.bits.VPRI);
            //printf(" INFO4.TPID      : 0x%x\n", p_tx_desc->txd_info4.bits.TPID);
            //printf(" INFO4.INSV      : 0x%x\n", p_tx_desc->txd_info4.bits.INSV);
            //printf(" INFO4.PN        : 0x%x\n", p_tx_desc->txd_info4.bits.PN);
            //printf(" INFO4.UDF       : 0x%x\n", p_tx_desc->txd_info4.bits.UDF);
            //printf(" INFO4.TSO       : 0x%x\n", p_tx_desc->txd_info4.bits.TSO);
            //printf(" INFO4.TCO       : 0x%x\n", p_tx_desc->txd_info4.bits.TCO);
            //printf(" INFO4.UCO       : 0x%x\n", p_tx_desc->txd_info4.bits.UCO);
            //printf(" INFO4.ICO       : 0x%x\n", p_tx_desc->txd_info4.bits.ICO);
        }
    }
}

void pdma_debug(const signed char * cmd)
{
    char str[32];
    char sub_cmd[32];
    ...
    else if (!strncmp(sub_cmd, "test-tx-sa", strlen("test-tx-sa")))
    {
        char tmp[32];
        unsigned int addr[6];

        sscanf(cmd, "%s %s %x:%x:%x:%x:%x:%x", tmp, tmp, &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
        g_test_sa[0] = addr[0];
        g_test_sa[1] = addr[1];
        g_test_sa[2] = addr[2];
        g_test_sa[3] = addr[3];
    ...
    else if (!strncmp(sub_cmd, "test-tx-da", strlen("test-tx-da")))
    {
        char tmp[32];
        unsigned int addr[6];
    ...
    else if (!strncmp(sub_cmd, "perf-test", strlen("perf-test")))
    {
        if (enable)
        {
            pdma_performance_test(enable * configTICK_RATE_HZ);
        }
    }
    else if (!strncmp(sub_cmd, "loop-test", strlen("loop-test")))
    {
        TaskHandle_t loopback_handle = xTaskGetHandle("loopback");
        if (enable && loopback_handle == NULL)
        {
            static unsigned int test_seconds = 0;
            test_seconds = enable;
            xTaskCreate(pdma_loopback_test,
                        "loopback",
                        configCMD_STACK_SIZE,
                        (void *)&test_seconds,
                        (tskIDLE_PRIORITY + 2),
                        NULL);
        }
        else if(loopback_handle != NULL)
        {
            printf("loopback task can not be created because it already exists.\n");
        }
    }
    else if (!strncmp(sub_cmd, "loop-delete", strlen("loop-delete")))
    {
        unsigned int notifyvalue = ulTaskNotifyTake(pdTRUE, 0);
        TaskHandle_t loopback_handle = xTaskGetHandle("loopback");
        if(loopback_handle != NULL)
        {
            if(enable || notifyvalue)
            {
                vTaskDelete(loopback_handle);
                io_write32(INT_MASK, g_loopback_test.p_mask);
                io_write32(0x1020943c, 0);
                io_write32(0x1020000c, g_loopback_test.p_agc);
                printf("loopback task delete success!\n");
            }
            else
            {
                printf("loopback task delete failed!\n");
            }
        }
        else
        {
            printf("Unable to delete the non-existent task named loopback!\n");
        }
    }
    else if (!strncmp(sub_cmd, "loop-result", strlen("loop-result")))
    {
        printf("time       : %d\n", (g_loopback_test.end_time - g_loopback_test.start_time)*portTICK_PERIOD_MS);
        printf("length     : %d\n", g_test_tx_len);
        printf("Rx         : %d\n", g_loopback_test.rx_cnt);
        printf("Tx         : %d\n", g_loopback_test.tx_cnt);
        printf("Check_fail : %d\n", g_loopback_test.cmp_fail);
...
int pdma_rx_pkt(int rxq, int is_loop_test)
{
    RX_DESC_T *p_rx_desc = NULL;
    void* pkt_data;
    int pkt_len;
    ...
    if (p_rx_desc->rxd_info2.bits.DDONE_bit)
    {
        ...
        if ((g_test_tx_random) && (pkt_len >= 60) && (pkt_len <= (PDMA_RX_PKT_LEN - 4)))
        {
            g_test_tx_len = pkt_len + 4;
        }
        ...
        if (is_loop_test)
        {
            g_loop_pkt_len = pkt_len;
            memcpy(g_loop_pkt, (unsigned char*)pkt_data, pkt_len);
            if ((g_debug_detail == 2) && (pkt_len <= PDMA_RX_PKT_LEN))
            {
                dump_pkt_data(g_loop_pkt, pkt_len);
            }
            memset((unsigned char*)pkt_data, 0, PDMA_RX_PKT_LEN);
            air_dcache_wback_inv(pkt_data, PDMA_RX_PKT_LEN);
        }
        ...
        p_rx_desc->rxd_info1.bits.PDP0 = g_pdma_cfg.rx_pkt_data[rxq][g_pdma_cfg.rx_cur_desc[rxq]];
        p_rx_desc->rxd_info2.bits.PLEN0  = PDMA_RX_PKT_LEN;

        /* cache write back */
        //air_dcache_wback_inv(p_rx_desc, sizeof(RX_DESC_T));

        io_write32(RX_CALC_IDX(rxq), g_pdma_cfg.rx_cur_desc[rxq]);
...
void pdma_performance_test(int time)
{
    unsigned int old_val, val = 0;
    int i, old_time, new_time, tx = 0, rx = 0, tx_fail = 0;
    ...
    old_val = io_read32(INT_MASK);
    val = RX_COHERENT | TX_COHERENT;
    io_write32(INT_MASK, val);

    for (i = 0 ; i < PDMA_TXDESC_NO ; i++)
    {
        memcpy(g_pdma_cfg.tx_pkt_data[0][i], g_test_pkt, PDMA_RX_PKT_LEN);
        air_dcache_wback_inv(((unsigned long)g_pdma_cfg.tx_pkt_data[0][i]), PDMA_RX_PKT_LEN);
    }

    old_time = tmpCpuTimerCnt;

    while(1)
    {
        if (0 == pdma_rx_pkt(0, 0))
        {
            rx++;
            if (0 == pdma_tx_pkt(g_test_pkt, g_test_tx_len, 0, 1))
            {
                tx++;
            }
            else
...
void pdma_test_tx(int burst)
{
    ...
    pkt = pvPortMalloc(PDMA_ETHERNET_TX_PKT_LEN, "pdma");
    if (!pkt)
    {
...
int pdma_tx_pkt(unsigned char* data, int len, int txq, int is_perf_test)
{
    TX_DESC_T *p_tx_desc = NULL;
    int i;
    ...
    p_tx_desc->txd_info1.bits.SDP0 = g_pdma_cfg.tx_pkt_data[txq][g_pdma_cfg.tx_cur_desc[txq]];
...
static void pdma_desc_init(void)
{
    ...
    for (rxq = 0; rxq < PDMA_RX_QUEUE_NUM; rxq++)
    {
        for (i = 0 ; i< PDMA_RXDESC_NO; i++)
        {
            ...
            // Assign flag
            p_rx_desc->rxd_info2.bits.LS0 = 1;
            p_rx_desc->rxd_info1.bits.PDP0 = g_pdma_cfg.rx_pkt_data[rxq][i];
            p_rx_desc->rxd_info2.bits.PLEN0  = PDMA_RX_PKT_LEN;
...
static void pdma_driver_reg_init(void)
{
    ...
    for (txq = 0; txq < PDMA_TX_QUEUE_NUM; txq++)
    {
        io_write32(TX_BASE_PTR(txq), g_pdma_cfg.tx_ring_base_addr[txq]);
    ...
    for (rxq = 0; rxq < PDMA_RX_QUEUE_NUM; rxq++)
    {
        io_write32(RX_BASE_PTR(rxq), g_pdma_cfg.rx_ring_base_addr[rxq]);
        io_write32(RX_MAX_CNT(rxq), PDMA_RXDESC_NO);
        io_write32(RX_CALC_IDX(rxq), PDMA_RXDESC_NO - 1);
    ...
    io_write32(PDMA_RX_DROP_JUMB0, PDMA_RX_DROP_JUMB0_DROP_CNT_CLN_MASK);
    io_write32(PDMA_RX_DROP_JUMB0, 0);
    io_write32(PDMA_RX_DROP_JUMB0, PDMA_RX_DROP_JUMB0_DROP_ENABLE_MASK | (((PDMA_RXDESC_NO-1)*PDMA_RX_PKT_LEN) & PDMA_RX_DROP_JUMB0_FRAME_SIZE_CONFIG_MASK));
    /* set cpu port force link */
...
void pdma_rx_ring_proc(int rxq)
{
    ...
    int rx_split_cnt = 1, i = 0, merge_len = 0;
#ifdef AIR_SUPPORT_ERPS
    QueueHandle_t       ptr_erps_queue = erps_pkt_queue_handle_get();
    unsigned char       is_raps_msg = 0;
#endif /* AIR_SUPPORT_ERPS */
    ...
    while(p_rx_desc->rxd_info2.bits.DDONE_bit)
    {
        ...
        PDMA_DEBUG_DETAIL("Debug: INFO1.PDP0: 0x%x, INFO2.PLEN0: %d, INFO2.LS0: 0x%x, INFO2.DDONE_bit: 0x%x\n",
            p_rx_desc->rxd_info1.bits.PDP0, p_rx_desc->rxd_info2.bits.PLEN0,
            p_rx_desc->rxd_info2.bits.LS0, p_rx_desc->rxd_info2.bits.DDONE_bit);
        //PDMA_DEBUG_DETAIL(" INFO1.PDP0      : 0x%x\n", p_rx_desc->rxd_info1.bits.PDP0);
        //PDMA_DEBUG_DETAIL(" INFO2.PLEN1     : 0x%x\n", p_rx_desc->rxd_info2.bits.PLEN1);
        //PDMA_DEBUG_DETAIL(" INFO2.LS1       : 0x%x\n", p_rx_desc->rxd_info2.bits.LS1);
        //PDMA_DEBUG_DETAIL(" INFO2.UN_USED   : 0x%x\n", p_rx_desc->rxd_info2.bits.UN_USED);
        //PDMA_DEBUG_DETAIL(" INFO2.PLEN0     : 0x%x\n", p_rx_desc->rxd_info2.bits.PLEN0);
        //PDMA_DEBUG_DETAIL(" INFO2.LS0       : 0x%x\n", p_rx_desc->rxd_info2.bits.LS0);
        //PDMA_DEBUG_DETAIL(" INFO2.DDONE_bit : 0x%x\n", p_rx_desc->rxd_info2.bits.DDONE_bit);
        //PDMA_DEBUG_DETAIL(" INFO3.UN_USE1   : 0x%x\n", p_rx_desc->rxd_info3.bits.UN_USE1);
        //PDMA_DEBUG_DETAIL(" INFO4.FOE_Entry : 0x%x\n", p_rx_desc->rxd_info4.bits.FOE_Entry);
        //PDMA_DEBUG_DETAIL(" INFO4.CRSN      : 0x%x\n", p_rx_desc->rxd_info4.bits.CRSN);
        //PDMA_DEBUG_DETAIL(" INFO4.SPORT     : 0x%x\n", p_rx_desc->rxd_info4.bits.SPORT);
        //PDMA_DEBUG_DETAIL(" INFO4.PKT_INFO  : 0x%x\n", p_rx_desc->rxd_info4.bits.PKT_INFO);
        //PDMA_DEBUG_DETAIL(" INFO4.RSV       : 0x%x\n", p_rx_desc->rxd_info4.bits.RSV);
        ...
        if(p_rx_desc->rxd_info2.bits.LS0)
        {
...
#ifdef AIR_SUPPORT_ERPS
            if(NULL != erps_check_buffer_is_raps((void *)pkt_data, pkt_len))
            {
                is_raps_msg = 1;
            }
            else
            ...
            if (g_pkt_queue)
            {
                ...
                {
                    p_rx_pkt_mutex = g_pdma_cfg.rx_pkt_msg_pool->rx_pkt_mutex[g_pdma_cfg.rx_cur_pkt_msg];
                    if (xSemaphoreTakeFromISR(p_rx_pkt_mutex, NULL))
                    {
                        p_rx_pkt_msg = &g_pdma_cfg.rx_pkt_msg_pool->rx_pkt_msg[g_pdma_cfg.rx_cur_pkt_msg];
                        if (pdFALSE == p_rx_pkt_msg->use)
                        {
                            /* update rx pkt msg content */
                            p_rx_pkt_msg->use = pdTRUE;
                            memcpy(p_rx_pkt_msg->data, pkt_data, pkt_len);
                            ...
                            if(1 == is_raps_msg)
                            {
                                if (pdPASS == xQueueSendFromISR(ptr_erps_queue, &g_pdma_cfg.rx_cur_pkt_msg, NULL))
                                {
                                ...
                                {
                                    p_rx_pkt_msg->use = pdFALSE;
                                    PDMA_DEBUG_DETAIL("Warning: ERPS queue full!\n");
                                }
                            }
                            else
#endif /* AIR_SUPPORT_ERPS */
        ...
        else
        {
                    ...
                    if (xSemaphoreTakeFromISR(p_rx_pkt_mutex, NULL))
                    {
                        ...
                        if (pdFALSE == p_rx_pkt_msg->use)
                        {
                            ...
                            for(i= 0; i< rx_split_cnt; i++)
                            {
                                ...
                                pkt_len  = p_rx_desc->rxd_info2.bits.PLEN0;
                                air_dcache_inv(((unsigned long)pkt_data), pkt_len);
                                memcpy(p_rx_pkt_msg->data + merge_len, pkt_data, pkt_len);
                                merge_len = merge_len + pkt_len;
                            }
                            ...
                            {
                                dump_pkt_data(p_rx_pkt_msg->data, p_rx_pkt_msg->len);
                            }
            ...
            for(i= 0; i< rx_split_cnt; i++) /* clear rx desc */
            {
                ...
                p_rx_desc->rxd_info2.bits.LS0 = 1;
                p_rx_desc->rxd_info1.bits.PDP0 = g_pdma_cfg.rx_pkt_data[rxq][((rx_cur_desc_start + i) % PDMA_RXDESC_NO)];
                p_rx_desc->rxd_info2.bits.PLEN0  = PDMA_RX_PKT_LEN;
...
void pdma_init (void)
{
    ...
    {
        g_pdma_cfg.rx_pkt_msg_pool->rx_pkt_mutex[i] = xSemaphoreCreateMutex("pdma");
        ...
        g_pdma_cfg.rx_pkt_msg_pool->rx_pkt_msg[i].data = pkt_data;
