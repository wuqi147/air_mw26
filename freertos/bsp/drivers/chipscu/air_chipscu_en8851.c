    ...
    int *ptr_state)
{
    ...
    if(func_idx <= AIR_CHIPSCU_IOMUX_LAN7_LED2_MODE )
    {
        u32regOff = IOMUX_CONTROL1_REGISTER;
        bit_idx += func_idx;
    }
    else if( (func_idx >= AIR_CHIPSCU_IOMUX_C_MDIO_MODE) && (func_idx <= AIR_CHIPSCU_IOMUX_CHIP_PROBE0_MODE) )
    {
        u32regOff = IOMUX_CONTROL2_REGISTER;
        bit_idx = func_idx - AIR_CHIPSCU_IOMUX_C_MDIO_MODE;
    }
    else
    {
...
{
    unsigned int u32regOff = 0x0, u32dat = 0x0, bit_idx = 0x0;
    /* LAN_LED function corresponds to GPIO pin */
    int iomux_ctrl1_tbl[24] = {0, 4, -1, 1, 5, -1, 2, 9, 6, 3, 13, 7, 19, 14, 8, 20, 15, 10, 21, 16, 11, 22, 17, 12};
    /* MISC function corresponds to bitmap of GPIO pins */
    int iomux_ctrl2_tbl[8] = {0xc, 0x30, 0xc0, 0x100, 0x1e00, 0x0, 0xc00, 0xfff};
    int i, j;

    if (func_idx < AIR_CHIPSCU_IOMUX_C_MDIO_MODE)
    {
        u32regOff = IOMUX_CONTROL1_REGISTER;
        bit_idx = func_idx;
    }
    else if (func_idx < AIR_CHIPSCU_IOMUX_FORCE_GPIO0_MODE)
    {
        u32regOff = IOMUX_CONTROL2_REGISTER;
        bit_idx = func_idx - AIR_CHIPSCU_IOMUX_C_MDIO_MODE;
    }
    else if (func_idx < AIR_CHIPSCU_IOMUX_FUNC_MAX)
    {
        u32regOff = IOMUX_CONTROL3_REGISTER;
        bit_idx = func_idx - AIR_CHIPSCU_IOMUX_FORCE_GPIO0_MODE;
    }
    ...
    if (TRUE == state)
    {
        /* Set LAN_LED Group (IOMUX_CONTROL1_REGISTER) */
        if ((IOMUX_CONTROL1_REGISTER == u32regOff) && (0 <= iomux_ctrl1_tbl[bit_idx]))
        {
            /* Reset Force_GPIO Group (IOMUX_CONTROL3_REGISTER) */
            u32dat = io_read32(IOMUX_CONTROL3_REGISTER);
            u32dat &= ~BIT(iomux_ctrl1_tbl[bit_idx]);
            io_write32(IOMUX_CONTROL3_REGISTER, u32dat);

            /* Reset MISC Group (IOMUX_CONTROL2_REGISTER) */
            for (i = 0; i < (sizeof(iomux_ctrl2_tbl) / sizeof(iomux_ctrl2_tbl[0])); i++)
            {
                /* iomux_ctrl2_tbl[i] include the pin iomux_ctrl1_tbl[bit_idx] */
                if (iomux_ctrl2_tbl[i] & (1 << iomux_ctrl1_tbl[bit_idx]))
                {
                    u32dat = io_read32(IOMUX_CONTROL2_REGISTER);
                    u32dat &= ~BIT(i);
                    io_write32(IOMUX_CONTROL2_REGISTER, u32dat);
                }
            }
        }
        /* Set MISC Group (IOMUX_CONTROL2_REGISTER) */
        else if (IOMUX_CONTROL2_REGISTER == u32regOff)
        {
            /* Reset Force_GPIO Group (IOMUX_CONTROL3_REGISTER) */
            u32dat = io_read32(IOMUX_CONTROL3_REGISTER);
            u32dat &= ~iomux_ctrl2_tbl[bit_idx];
            io_write32(IOMUX_CONTROL3_REGISTER, u32dat);

            /* Reset LAN_LED Group (IOMUX_CONTROL1_REGISTER) */
            for (i = 0; i < AIR_CHIPSCU_IOMUX_CONTROL2_ENTRY_BIT_LENGTH + 1; i++)
            {
                /* GPIO pin i should be reset */
                if (1 << i & iomux_ctrl2_tbl[bit_idx])
                {
                    /* j is the bit of LAN_LED Group (IOMUX_CONTROL1_REGISTER) corresponds to GPIO pin i */
                    for (j = 0; j < (sizeof(iomux_ctrl1_tbl) / sizeof(iomux_ctrl1_tbl[0])); j++)
                    {
                        if (i == iomux_ctrl1_tbl[j])
                        {
                            u32dat = io_read32(IOMUX_CONTROL1_REGISTER);
                            u32dat &= ~BIT(j);
                            io_write32(IOMUX_CONTROL1_REGISTER, u32dat);
                        }
                    }
                }
            }
            ...
            for (i = 0; i < (sizeof(iomux_ctrl2_tbl) / sizeof(iomux_ctrl2_tbl[0])); i++)
            {
                ...
                {
                    /* Different MISC function using the same GPIO pin, reset function i */
                    if (iomux_ctrl2_tbl[i] & iomux_ctrl2_tbl[bit_idx])
                    {
                        u32dat = io_read32(IOMUX_CONTROL2_REGISTER);
                        u32dat &= ~BIT(i);
                        io_write32(IOMUX_CONTROL2_REGISTER, u32dat);
        ...
        /* Set Force_GPIO Group (IOMUX_CONTROL3_REGISTER) */
        else
        {
            /* Reset LAN_LED Group (IOMUX_CONTROL1_REGISTER) */
            for (i = 0; i < (sizeof(iomux_ctrl1_tbl) / sizeof(iomux_ctrl1_tbl[0])); i++)
            {
                /* GPIO pin bit_idx should be reset, reset function i of LAN_LED Group */
                if (bit_idx == iomux_ctrl1_tbl[i])
                {
                    u32dat = io_read32(IOMUX_CONTROL1_REGISTER);
                    u32dat &= ~BIT(i);
                    io_write32(IOMUX_CONTROL1_REGISTER, u32dat);
            ...
            for (i = 0; i < (sizeof(iomux_ctrl2_tbl) / sizeof(iomux_ctrl2_tbl[0])); i++)
            {
                /* iomux_ctrl2_tbl[i] include the GPIO pin bit_idx, reset function i */
                if (iomux_ctrl2_tbl[i] & (1 << bit_idx))
                {
                    u32dat = io_read32(IOMUX_CONTROL2_REGISTER);
                    u32dat &= ~BIT(i);
                    io_write32(IOMUX_CONTROL2_REGISTER, u32dat);
