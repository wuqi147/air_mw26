#include "util/flash_util.h"
#include "spinor/spi_controller.h"
...
#define _SPI_NOR_OP_READ_UUID                   (0x4B)  /* read flash unique ID*/
...
static uint32_t __noex9__  _getFlashSize(void);
static void __noex9__ _getFlashUuid(uint8_t *reg_data, uint32_t len);

static void __noex9__ _spiflash_write_status_register1(uint8_t data);
static void __noex9__ _spiflash_read_status_register1(uint8_t *status);
static void __noex9__ _spiflash_disable_write(void);
static void __noex9__ _spiflash_read_id(uint8_t *id, uint32_t len);
static void __noex9__ _spiflash_read_sfdp(uint32_t offset, uint8_t *p_sfdp_data, uint8_t len);
static void __noex9__ _spiflash_read_uuid(uint8_t *uuid, uint32_t len);
static void __noex9__ _spiflash_enable_write(void);
static void __noex9__ _spiflash_erase_sector(uint32_t addr);
static void __noex9__ _spiflash_write_page(uint8_t *p_data, uint32_t addr, uint32_t len);
static void __noex9__ _spiflash_write_sector(uint8_t *p_data, uint32_t addr, uint32_t len);
...
{
    _SPI_NOR_CHIP_SELECT_LOW();
    _SPI_NOR_WRITE_ONE_BYTE(_SPI_NOR_OP_READ_UUID);
    _SPI_NOR_WRITE_ONE_BYTE(0x00);  /*dummy byte : 0x00, this value is don't care*/
    _SPI_NOR_WRITE_ONE_BYTE(0x00);  /*dummy byte : 0x00, this value is don't care*/
    _SPI_NOR_WRITE_ONE_BYTE(0x00);  /*dummy byte : 0x00, this value is don't care*/
    _SPI_NOR_WRITE_ONE_BYTE(0x00);  /*dummy byte : 0x00, this value is don't care*/
    _SPI_NOR_READ_NBYTE(uuid, len, SPI_CONTROLLER_SPEED_SINGLE);
    _SPI_NOR_CHIP_SELECT_HIGH();
}
 ...
 *      None
 * NOTES:
 *      SPI driver save the uuid value into SPI_NOR_PARAM_WRITE_BUF,
 *      then RTOS will get the uuid by this address.
 */
static void __noex9__ _getFlashUuid(uint8_t *reg_data, uint32_t len)
{
    _spiflash_read_uuid(reg_data, len);
}

SPI_ERR_T __noex9__ _spi_nor_entry(uint32_t src, uint32_t dst, uint32_t size)
{
...
SPI_ERR_T __noex9__ _spi_nor_instr_read(uint8_t flash_ins, uint8_t *reg_data, uint32_t reg_data_len)
{
    SPI_ERR_T error_code = SPI_SUCCESS;
    _SPI_NOR_ENABLE_MANUAL_MODE();
    switch(flash_ins)
    {
        case _SPI_NOR_OP_READ_UUID:
            _getFlashUuid(reg_data, reg_data_len);
            break;
        default:
            error_code = -SPI_ERR_ILLEGAL_INS;
            break;
    }
    _SPI_NOR_ENABLE_AUTO_MODE();
    return error_code;
