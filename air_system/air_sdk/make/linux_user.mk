...
HW_UTIL_LIB             := $(AIR_SYSTEM)/air_sdk/lib/linux/cmlib_hw_util.a
else
HW_UTIL_LIB             :=
endif
################################################################################
...
ifeq ($(findstring SHARED_LIB,$(BUILD_TYPE)),SHARED_LIB)
	$(CC) -shared $(CFLAGS) -o  $(AIR_SYSTEM_BUILD)/image/lib$(ARCHIVE_NAME).so $(TGT_OBJS_TOTAL) $(HW_UTIL_LIB)
else
	$(AR) -cr $(AIR_SYSTEM_BUILD)/image/$(ARCHIVE_NAME).a $(TGT_OBJS_TOTAL) $(HW_UTIL_LIB)
endif
$(AIR_SYSTEM_BUILD)/%.o: ../%.c
	$(TEST_PATH) $(dir $@) || $(MKDIR) $(dir $@)
	-$(CC) $(EXTRA_CFLAGS) $(CFLAGS) $(SDK_CFLAGS) $(LIB_OPTION)  -c -o $@ $< >> $(AIR_LOG) 2>&1
	-$(CC) -MM -MT $@ $(EXTRA_CFLAGS) $(CFLAGS) $(SDK_CFLAGS) $(LIB_OPTION)  $< > $(patsubst %.o,%.d, $@)
