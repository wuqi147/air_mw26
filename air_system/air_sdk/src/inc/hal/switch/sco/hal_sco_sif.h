 ...
 *
 */
#ifndef HAL_SCO_SIF_H
#define HAL_SCO_SIF_H
 ...
 */
#define HAL_SCO_SIF_MASTER_COUNT (2)

/* Maximum Page size*/
#define HAL_SCO_SIF_MAX_PAGE_SIZE (8)
...
typedef enum AIR_SIF_CLK_SPEED_E
{
    ...
    AIR_SIF_CLK_LAST,
} HAL_SCO_SIF_CLK_SPEED_T;

/* This enum number depends on the i2c master number
 */
typedef enum HAL_SCO_SIF_DEVICE_E
{
    HAL_SCO_SIF_LOCAL_DEVICE = 0,
    HAL_SCO_SIF_REMOTE_DEVICE,
    HAL_SCO_SIF_DEVICE_LAST
} HAL_SCO_SIF_DEVICE_T;

typedef enum HAL_SCO_SIFM_STATUS_E
{
    HAL_SCO_SIFM_SLAVE = 0,
    HAL_SCO_SIFM_MASTER,
    HAL_SCO_SIFM_LAST
} HAL_SCO_SIFM_STATUS_T;

typedef enum HAL_SCO_SIFM_OP_E
{
    HAL_SCO_SIFM_WRITE = 0,
    HAL_SCO_SIFM_READ,
    HAL_SCO_SIFM_BIT_LAST
} HAL_SCO_SIFM_OP_T;

typedef struct HAL_SCO_SIF_INFO_S
{
    UI16_T channel;
    UI16_T slave_id;
...
typedef struct HAL_SCO_SIF_PARAM_S
{
    UI32_T             addr_len;
    UI32_T             addr;
    UI32_T             data_len;
    UI32_T            *ptr_data;
    HAL_SCO_SIF_INFO_T device_info;
} HAL_SCO_SIF_PARAM_T;

/* This enum start number is according to  programming guide
 * Configuration Register 1, 0x10008044 and 0x10022044
 */
typedef enum HAL_SCO_SIF_CR1_OP_MODE_E
{
    HAL_SCO_SIF_START_BIT = 1,
    HAL_SCO_SIF_WRITE_DATA,
    HAL_SCO_SIF_STOP_BIT,
    HAL_SCO_SIF_READ_FINAL_BIT,
    HAL_SCO_SIF_READ_BIT,
    HAL_SCO_SIF_BIT_LAST
} HAL_SCO_SIF_CR1_OP_MODE_T;
