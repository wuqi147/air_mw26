...
/* FILE NAME:  hal_an8858_phy.c
 * PURPOSE:
 *  Implement an8858 phy module HAL function.
 *
 * NOTES:
 *
 */
 ...
 */
#define HAL_AN8858_PHY_LED_CFG_REG (HAL_AN8858_PHY_LED_3_BLK_MASK)

#define HAL_AN8858_PHY_LONG_REACH         (0xc00)
#define HAL_AN8858_PHY_NORMAL             (0x800)
#define HAL_AN8858_PHY_UNPLUG_LOW_POWER   (0x50aa)
#define HAL_AN8858_PHY_100M_10M_LOW_POWER (0x53aa)
#define HAL_AN8858_PHY_1G_LOW_POWER       (0x5faa)
...
#define HAL_AN8858_PHY_WORD_SIZE          (4)
#define HAL_AN8858_PHY_EFUSE_PHY_BASE     (5)
#define HAL_AN8858_PHY_EFUSE_SIZE_PER_PHY (4)

#define HAL_AN8858_PHY_EFUSE_DATA5 (0x1000A514)
#define HAL_AN8858_PHY_EFUSE_DATA7 (0x1000A51C)
#define HAL_AN8858_PHY_EFUSE_DATA8 (0x1000A520)

#define HAL_AN8858_PHY_GEAR_NUM (8)

#define HAL_AN8858_PHY_PORT0_3_SEED    (0x1000A5B0)
#define HAL_AN8858_PHY_PORT4_7_SEED    (0x1000A5B4)
#define HAL_AN8858_PHY_RANDOM_SEED_LSB (0x760)
#define HAL_AN8858_PHY_RANDOM_SEED_MSB (0x761)

#define MAX_DOWN_SHIFT_TIMEOUT (10)

#define HAL_AN8858_PHY_FLASH_SET_HALF_HZ   (0x00)
#define HAL_AN8858_PHY_FLASH_SET_ONE_HZ    (0x01)
#define HAL_AN8858_PHY_FLASH_SET_TWO_HZ    (0x02)
#define HAL_AN8858_PHY_FLASH_SET_FORCE_OFF (0x04)
#define HAL_AN8858_PHY_FLASH_SET_FORCE_ON  (0x05)
...
#define HAL_AN8858_PHY_GET_PSR_LINK_STS(data, phy_port)                                           \
    ((data & (HAL_AN8858_PHY_PSR_LINK_STS_MASK                                                    \
              << ((phy_port % HAL_AN8858_PHY_PER_PSR_PORTS) * HAL_AN8858_PHY_PER_PSR_OFFSET))) >> \
     ((phy_port % HAL_AN8858_PHY_PER_PSR_PORTS) * HAL_AN8858_PHY_PER_PSR_OFFSET))

#define HAL_AN8858_PHY_LED_SRC_SEL_POS(__port__, __ledId__)   (1 << ((__port__ * 3) + (__ledId__)))
#define HAL_AN8858_PHY_RG_LED_FLASH_SET_CTRL(__port__)        (CORAL_SLED_FLASH_SET_CTRL0 + ((__port__ / 2) * 4))
#define HAL_AN8858_PHY_LED_FLASH_SET_POS(__port__, __ledId__) (((__port__ & 0x01) * 16) + (__ledId__ * 4))
#define HAL_AN8858_PHY_LED_FLASH_SET_MASK                     (0x7)
#define HAL_AN8858_PHY_GPIO_FLASH_MAP_CFG_POS(__gpio__)       ((__gpio__) * 4)
 ...
 */
DIAG_SET_MODULE_INFO(AIR_MODULE_PHY, "hal_an8858_phy.c");

extern const HAL_PHY_CFG_T _hal_an8858_longreach[];
extern const UI32_T        _hal_an8858_longreach_size;
extern const HAL_PHY_CFG_T _hal_an8858_normal[];
extern const UI32_T        _hal_an8858_normal_size;
extern const HAL_PHY_CFG_T _hal_an8858_pre_init_global[];
extern const UI32_T        _hal_an8858_pre_init_global_size;
extern const HAL_PHY_CFG_T _hal_an8858_pre_init_port[];
extern const UI32_T        _hal_an8858_pre_init_port_size;

/* STATIC VARIABLE DECLARATIONS */
static const UI8_T         _hal_an8858_phy_r50ohm_table[] = {127, 127, 127, 127, 127, 127, 127, 127, 127, 127, /* 10 */
 ...
 */
static void
_hal_an8858_phy_getSurgeVal(
    UI8_T  base,
    UI8_T *ptr_val)
{
    UI8_T  index = 0, table_idx = 0;
    UI32_T sz = sizeof(_hal_an8858_phy_r50ohm_table) / sizeof(UI8_T);

    for (index = 0; index < sz; ++index)
    {
        if (_hal_an8858_phy_r50ohm_table[index] == base)
        {
            break;
        }
    }

    if (index < HAL_AN8858_PHY_GEAR_NUM || index >= sz)
    {
        table_idx = 25; /* index of 94 */
    }
    else
    {
        table_idx = index - HAL_AN8858_PHY_GEAR_NUM;
    }

    if (table_idx < sz)
    {
        *ptr_val = _hal_an8858_phy_r50ohm_table[table_idx];
...
static void
_hal_an8858_phy_tkrg_wait_ready(
    UI8_T phyAddr)
{
    ...
    do
    {
        rv = hal_mdio_readC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, &reg_data);
        if (AIR_E_OK == rv)
        {
            retry--;
        }
    ...
    UI32_T wdata)
{
    ...
    hal_mdio_writeC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x11, low_data);
    hal_mdio_writeC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x12, high_data);
    hal_mdio_writeC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, reg_data);
}

static UI32_T
_hal_an8858_phy_tkrg_read(
    UI8_T phyAddr,
    UI8_T channel_addr,
    UI8_T node_addr,
    UI8_T data_addr)
{
    ...
    hal_mdio_writeC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, reg_data);
    _hal_an8858_phy_tkrg_wait_ready(phyAddr);

    rv = hal_mdio_readC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x11, &reg_data);
    if (AIR_E_OK == rv)
    {
        rdata = reg_data;
        rv = hal_mdio_readC22FromIntBus(0, 0, phyAddr, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x12, &reg_data);
        if (AIR_E_OK == rv)
        {
            rdata |= (reg_data << 16);
        }
    ...
    UI32_T      *random_seed)
{
    ...
    rv = hal_mdio_readC22FromIntBus(0, 0, phy_addr, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, &backup_reg0);
    rv = hal_mdio_writeC22FromIntBus(0, 0, phy_addr, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x140);

    rv = hal_mdio_readC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC1, 0x3C, &backup_dev1e_reg03ch);
    rv = hal_mdio_readC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC2, 0x15, &backup_dev1f_reg015h);
    backup_vga = _hal_an8858_phy_tkrg_read(phy_addr, 0x1, 0xf, 0x10);

    rv = hal_mdio_writeC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC1, 0x3C, 0x8800);
    phy_port = phy_addr & 0x03;
    /* debug port assigned by b13..b15 , every 4 ports for one group */
    rv = hal_mdio_writeC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC2, 0x15, (phy_port << 13) | 0x108b);
    value = backup_vga | 0xF8000;
    _hal_an8858_phy_tkrg_write(phy_addr, 0x1, 0xF, 0x10, value);

    for (index = 0; index < 16; index++)
    {
        rv = hal_mdio_readC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC2, 0x1A, &reg_data);
        seed |= ((reg_data & 0x1) << index);
    }

    _hal_an8858_phy_tkrg_write(phy_addr, 0x1, 0xF, 0x10, backup_vga);
    rv = hal_mdio_writeC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC2, 0x15, backup_dev1f_reg015h);
    rv = hal_mdio_writeC45FromIntBus(0, 0, phy_addr, MMD_DEV_VSPEC1, 0x3C, backup_dev1e_reg03ch);

    backup_reg0 |= BMCR_PDOWN;
    rv = hal_mdio_writeC22FromIntBus(0, 0, phy_addr, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, backup_reg0);
...
static AIR_ERROR_NO_T
_hal_an8858_phy_restartLink(
    const UI32_T unit,
    const UI32_T port)
{
    AIR_ERROR_NO_T rv = AIR_E_OK;

    rv = hal_an8858_phy_setAdminState(unit, port, HAL_PHY_ADMIN_STATE_DISABLE);
    if (AIR_E_OK == rv)
    {
        rv = hal_an8858_phy_setAdminState(unit, port, HAL_PHY_ADMIN_STATE_ENABLE);
        if (AIR_E_OK == rv)
        {
            rv = hal_an8858_phy_setAutoNego(unit, port, HAL_PHY_AUTO_NEGO_RESTART);
        }
    }
    return rv;
}
...
static AIR_ERROR_NO_T
_hal_an8858_phy_checkLinkStatus(
    const UI32_T unit,
    const UI32_T port,
    const BOOL_T link_status)
{
    ...
    if (TRUE == link_status)
    {
        ...
        if (HAL_PHY_PORT_DEV_SPD_DOWN_ADV_CHANGED(unit, port))
        {
            ...
            rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, reg_data);
    ...
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, &reg_data);
    if ((AIR_E_OK == rv) && (reg_data & MASTER_SLAVE_CFAULT))
    {
        /* phy power down then up if AN is in fault state */
        rv = _hal_an8858_phy_restartLink(unit, port);
        return rv;
    }
#ifdef AIR_EN_SPEED_DOWNSHIFT_ENHANCE
    rv = hal_an8858_phy_getSmartSpeedDown(unit, port, &spd_mode);
    if ((AIR_E_OK == rv) && (HAL_PHY_SSD_MODE_DISABLE != spd_mode))
    {
        /* speed down shift check for work around*/
        rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, &reg_data);
        if (AIR_E_OK == rv)
        {
            ...
            reg_data = _hal_an8858_phy_tkrg_read(HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), 0x0, 0xf, 0x1);
            reg_data &= 0xf000;
            ...
            if (0xa000 == reg_data)
            {
                ...
                {
                    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000,
                                                          &reg_data);
                    if (AIR_E_OK == rv)
                    {
                        ...
                        rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000,
                                                               reg_data);
                        if (AIR_E_OK == rv)
                        {
                            HAL_PHY_PORT_DEV_SPD_DOWN_ADV_CHANGED(unit, port) = 1;
                            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR,
                                                                  &reg_data);
                            if (AIR_E_OK == rv)
                            {
                                reg_data |= BMCR_ANRESTART;
                                rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000,
                                                                       MII_BMCR, reg_data);
                            }
                        }
                    }
...
static AIR_ERROR_NO_T
_hal_an8858_phy_setRandomSeedByGenModule(
    const UI32_T unit,
    const UI32_T port)
{
    AIR_ERROR_NO_T rv = AIR_E_OK;
    UI32_T         efuse_data1 = 0, efuse_data2 = 0, bus_id = 0, index = 0, seed = 0;
    UI16_T         reg_data = 0, tmp_data = 0;
    BOOL_T         efuse_valid = FALSE;

    rv = aml_readReg(unit, HAL_AN8858_PHY_PORT0_3_SEED, &efuse_data1, sizeof(UI32_T));
    if (AIR_E_OK == rv)
    {
        rv = aml_readReg(unit, HAL_AN8858_PHY_PORT4_7_SEED, &efuse_data2, sizeof(UI32_T));
        if (AIR_E_OK == rv)
        {
            DIAG_PRINT(HAL_DBG_INFO, "read efuse addr 0x%08x:0x%08x\n", HAL_AN8858_PHY_PORT0_3_SEED, efuse_data1);
            DIAG_PRINT(HAL_DBG_INFO, "read efuse addr 0x%08x:0x%08x\n", HAL_AN8858_PHY_PORT4_7_SEED, efuse_data2);
            ...
            if (TRUE == efuse_valid)
            {
                ...
                rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x1, MMD_DEV_VSPEC2, HAL_AN8858_PHY_RANDOM_SEED_LSB,
                                                 reg_data);
                DIAG_PRINT(HAL_DBG_INFO, "set phy addr 0x%02x cl45 0x1f 0x760 to 0x%04x\n", 0x1, reg_data);
                if (AIR_E_OK == rv)
                {
                    reg_data = ((BITS_OFF_R(efuse_data1, 16, 15)) | BIT(15));
                    rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x1, MMD_DEV_VSPEC2, HAL_AN8858_PHY_RANDOM_SEED_MSB,
                                                     reg_data);
                ...
                if (AIR_E_OK == rv)
                {
                    ...
                    rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x5, MMD_DEV_VSPEC2, HAL_AN8858_PHY_RANDOM_SEED_LSB,
                                                     reg_data);
                    DIAG_PRINT(HAL_DBG_INFO, "set phy addr 0x%02x cl45 0x1f 0x760 to 0x%04x\n", 0x5, reg_data);
                    if (AIR_E_OK == rv)
                    {
                        reg_data = ((BITS_OFF_R(efuse_data2, 16, 15)) | BIT(15));
                        rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x5, MMD_DEV_VSPEC2,
                                                         HAL_AN8858_PHY_RANDOM_SEED_MSB, reg_data);
                        DIAG_PRINT(HAL_DBG_INFO, "set phy addr 0x%02x cl45 0x1f 0x761 to 0x%04x\n", 0x5, reg_data);
                    }
                }
            }
            ...
            {
                rv = _hal_an8858_phy_getRandomSeed(unit, 0, &seed);
                if (AIR_E_OK == rv)
                {
                    for (index = 0; index < 0x4; index++)
                    {
                        rv |= hal_mdio_readC45FromIntBus(unit, bus_id, index, MMD_DEV_VSPEC1, 0x12, &tmp_data);
                        if (AIR_E_OK == rv)
                        {
                            seed += tmp_data;
                        }
                ...
                if (AIR_E_OK == rv)
                {
                    ...
                    if (AIR_E_OK == rv)
                    {
                        ...
                        rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x1, MMD_DEV_VSPEC2,
                                                         HAL_AN8858_PHY_RANDOM_SEED_MSB, reg_data);
                        DIAG_PRINT(HAL_DBG_INFO, "set phy addr 0x%02x cl45 0x1f 0x761 to 0x%04x\n", 0x1, reg_data);
                    }
                }
                if (AIR_E_OK == rv)
                {
                    rv = _hal_an8858_phy_getRandomSeed(unit, 4, &seed);
                    if (AIR_E_OK == rv)
                    {
                    ...
                    if (AIR_E_OK == rv)
                    {
                        reg_data = (seed & BITS(0, 15));
                        rv = hal_mdio_writeC45FromIntBus(unit, bus_id, 0x5, MMD_DEV_VSPEC2,
                                                         HAL_AN8858_PHY_RANDOM_SEED_LSB, reg_data);
                        DIAG_PRINT(HAL_DBG_INFO, "set phy addr 0x%02x cl45 0x1f 0x760 to 0x%04x\n", 0x5, reg_data);
                        if (AIR_E_OK == rv)
                        {
                            reg_data = ((~(seed & BITS(0, 15))) | BIT(15));
    ...
    HAL_PHY_LINK_STATUS_T *ptr_status)
{
    ...
    /* Do a fake read */
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMSR, &status);
    if (AIR_E_OK == rv)
    {
        ...
        if (AIR_E_OK == rv)
        {
            ...
            /* 10M link status */
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x16, &reg_data);
            if (AIR_E_OK == rv)
            {
                if (reg_data & BIT(6))
                {
            ...
            /* 100M link status */
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x10, &reg_data);
            if (AIR_E_OK == rv)
            {
                if (reg_data & BIT(12))
                {
            ...
            /* 1000M link status */
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x11, &reg_data);
            if (AIR_E_OK == rv)
            {
                if (reg_data & BIT(12))
                {
    ...
    HAL_PHY_LINK_STATUS_T *ptr_status)
{
    ...
    if (AIR_E_OK == rv)
    {
        rv = hal_an8858_phy_getAutoNego(unit, port, &auto_nego);
        if (AIR_E_OK == rv)
        {
            if (HAL_PHY_AUTO_NEGO_ENABLE == auto_nego)
            {
                rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, &lpagb);
                if (AIR_E_OK == rv)
                {
                    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, &adv);
                    if (AIR_E_OK == rv)
                    {
                        common_adv_gb = lpagb & adv << 2;
                        rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, &lpa);
                        if (AIR_E_OK == rv)
                        {
                            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000,
                                                                  MII_ADVERTISE, &adv);
                            if (AIR_E_OK == rv)
                            {
                                common_adv = lpa & adv;
            ...
            {
                rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, &bmcr);
                if (AIR_E_OK == rv)
                {
            ...
            /* Workaround for the issue of obtaining incorrect speeds after enabling EEE and downshift to 100M
               on RTL ic */
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_AUX_CTRL_STA,
                                                  &speed_status);
            if (AIR_E_OK == rv)
            {
...
/* FUNCTION NAME:   _hal_an8858_phy_restoreConfigFromEfuse
 * PURPOSE:
 *      Restore AN8858 PHY config from efuse
 * INPUT:
 ...
 */
static AIR_ERROR_NO_T
_hal_an8858_phy_restoreConfigFromEfuse(
    const UI32_T unit,
    const UI32_T port)
{
    ...
    /* i2mpb_a_tbt */
    reg_addr = HAL_AN8858_PHY_EFUSE_DATA7 + (phy_port * 0x10);
    rv = aml_readReg(unit, reg_addr, &efuse_data, sizeof(UI32_T));
    if (AIR_E_OK == rv)
    {
        rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x12, &data);
        if (AIR_E_OK == rv)
        {
            data &= ~(BITS(0, 5));
            data |= BITS_OFF_R(efuse_data, 12, 6);
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x12, data);
        }
    }
    ...
    if (AIR_E_OK == rv)
    {
        reg_addr = HAL_AN8858_PHY_EFUSE_DATA8 + (phy_port * 0x10);
        rv = aml_readReg(unit, reg_addr, &efuse_data, sizeof(UI32_T));
        if (AIR_E_OK == rv)
        {
            rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x17, &data);
            if (AIR_E_OK == rv)
            {
                data &= ~(BITS(0, 5));
                data |= BITS_OFF_R(efuse_data, 12, 6);
                rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x17, data);
            }
        }
    }
    return rv;
...
/* FUNCTION NAME:   _hal_an8858_phy_restoreCfg
 * PURPOSE:
 *      Restore AN8858 PHY config when operation mode from long-reach to normal.
 * INPUT:
 *      unit            --  Device unit number
 *      port            --  Port number
 *
    ...
    const UI32_T port)
{
    ...
    rv = hal_an8858_phy_getPhyOpMode(unit, port, &mode);
    if ((AIR_E_OK == rv) && (HAL_PHY_OP_MODE_LONG_REACH == mode))
    {
        rv = _hal_an8858_phy_restoreConfigFromEfuse(unit, port);
        if (AIR_E_OK == rv)
        {
            rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x11, 0x1e);
            rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x12, 0x0);
            rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, 0x8f80);
...
/* FUNCTION NAME:   _hal_an8858_phy_applyParam
 * PURPOSE:
 *      Apply AN8858 PHY parameters
 * INPUT:
 *      unit            --  Device unit number
 *      port            --  Port number
 *
    ...
    const HAL_PHY_CFG_T phy_cfg)
{
    ...
    if ((HAL_PHY_MAX_DATA_MSB == phy_cfg.data_msb) && (HAL_PHY_MIN_DATA_LSB == phy_cfg.data_lsb))
    {
        if (HAL_PHY_ACCESS_METHOD_CL22 == phy_cfg.access_method)
        {
            if (MMD_DEV_VSPEC2 == phy_cfg.reg_addr)
            {
                ...
                {
                    case 0x0:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_0000;
                        break;
                    case 0x2A30:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_2A30;
                        break;
                    case 0x52B5:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_52B5;
                        break;
                    case 0x1:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_0001;
                        break;
                    case 0x2:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_0002;
                        break;
                    case 0x3:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_0003;
                        break;
                    case 0x4:
                        *ptr_page = HAL_AN8858_PHY_PHY_PAGE_0004;
                        break;
                    default:
            ...
            {
                rv = hal_mdio_writeC22FromIntBusByPort(unit, port, set_page, phy_cfg.reg_addr, phy_cfg.reg_data);
        ...
        {
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, phy_cfg.device_id, phy_cfg.reg_addr, phy_cfg.reg_data);
    ...
    else
    {
        if (HAL_PHY_ACCESS_METHOD_CL22 == phy_cfg.access_method)
        {
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, set_page, phy_cfg.reg_addr, &data);
        }
        else
        {
            rv = hal_mdio_readC45FromIntBusByPort(unit, port, phy_cfg.device_id, phy_cfg.reg_addr, &data);
        }
        ...
        if (AIR_E_OK == rv)
        {
            ...
            if (HAL_PHY_ACCESS_METHOD_CL22 == phy_cfg.access_method)
            {
                rv = hal_mdio_writeC22FromIntBusByPort(unit, port, set_page, phy_cfg.reg_addr, data);
                DIAG_PRINT(HAL_DBG_INFO, "[CL22]port %u, page %u, reg_addr 0x%x, reg_data 0x%x\n", port, set_page,
                           phy_cfg.reg_addr, phy_cfg.reg_data);
            }
            else
            {
                rv = hal_mdio_writeC45FromIntBusByPort(unit, port, phy_cfg.device_id, phy_cfg.reg_addr, data);
                DIAG_PRINT(HAL_DBG_INFO, "[CL45]port %u, dev 0x%x, reg_addr 0x%x, reg_data 0x%x\n", port,
    ...
    const HAL_PHY_OP_MODE_T mode)
{
    ...
    {
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x12, 0x66);
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x11, 0x4d2a);
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, 0x8fa4);
    }
    else
    {
        /* default value */
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x12, 0x3e);
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x11, 0x4d2a);
        rv |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_52B5, 0x10, 0x8fa4);
    }
    return rv;
    ...
    const UI32_T port)
{
    ...
    rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC2, 0x268, &data);
    if (AIR_E_OK == rv)
    {
        if (0x7f1 != data) /* check if it has been initialized before */
        {
            for (count = 0; count < _hal_an8858_pre_init_global_size; count++)
            {
                phy_cfg = _hal_an8858_pre_init_global[count];
                rv = _hal_an8858_phy_applyParam(unit, port, page, &page, phy_cfg);
                if (AIR_E_OK != rv)
                {
                    DIAG_PRINT(HAL_DBG_ERR, "port %u, an8858 PHY global initialized fail !\n", port);
                    break;
                }
            }
        }
    ...
    if (AIR_E_OK == rv)
    {
        for (count = 0; count < _hal_an8858_pre_init_port_size; count++)
        {
            phy_cfg = _hal_an8858_pre_init_port[count];
            rv = _hal_an8858_phy_applyParam(unit, port, page, &page, phy_cfg);
            if (AIR_E_OK != rv)
            {
                DIAG_PRINT(HAL_DBG_ERR, "port %u, an8858 PHY per-port initialized fail !\n", port);
                break;
            }
        }
    }
...
/* FUNCTION NAME:   _hal_an8858_phy_setPhyOpMode
 * PURPOSE:
 *      Set AN8858 PHY operation mode
 * INPUT:
 *      unit            --  Device unit number
 *      port            --  Port number
 *
    ...
    const HAL_PHY_OP_MODE_T mode)
{
    ...
    rv = hal_an8858_phy_getAdminState(unit, port, &admin_state);
    ...
    if (AIR_E_OK == rv)
    {
        ...
        {
            rv = _hal_an8858_phy_restoreCfg(unit, port);
            if (AIR_E_OK == rv)
            {
                for (count = 0; count < _hal_an8858_normal_size; count++)
                {
                    if (AIR_E_OK == rv)
                    {
                        phy_cfg = _hal_an8858_normal[count];
                        rv = _hal_an8858_phy_applyParam(unit, port, page, &page, phy_cfg);
                    }
                }
            }
        ...
        {
            rv = hal_an8858_phy_getLocalAdvAbility(unit, port, &hal_adv);
            if (AIR_E_OK == rv)
            {
                ...
                {
                    for (count = 0; count < _hal_an8858_longreach_size; count++)
                    {
                        if (AIR_E_OK == rv)
                        {
                            phy_cfg = _hal_an8858_longreach[count];
                            rv = _hal_an8858_phy_applyParam(unit, port, page, &page, phy_cfg);
                        }
                    }
                }
    ...
    if (AIR_E_OK == rv)
    {
        _hal_an8858_phy_enhanceIotVga(unit, port, mode);
    }
...
/* FUNCTION NAME:   _hal_an8858_phy_setSurgeProtect
 * PURPOSE:
 *      Set AN8858 PHY surge protection
 * INPUT:
 *      unit            --  Device unit number
 *      port            --  Port number
 *
 ...
 */
static AIR_ERROR_NO_T
_hal_an8858_phy_setSurgeProtect(
    const UI32_T unit,
    const UI32_T port)
{
    AIR_ERROR_NO_T  rv = AIR_E_OK;
    ...
    HAL_AIR_PORT_TO_PHY_PORT(unit, port, phy_port);
    for (raw = 0; raw < HAL_AN8858_PHY_EFUSE_SIZE_PER_PHY; raw++)
    {
        reg_addr = (HAL_AN8858_PHY_EFUSE_DATA5 +
                    (HAL_AN8858_PHY_EFUSE_SIZE_PER_PHY * (raw + (HAL_AN8858_PHY_WORD_SIZE * phy_port))));
        rv = aml_readReg(unit, reg_addr, &efuse_data, sizeof(UI32_T));
        if (AIR_E_OK == rv)
        {
    ...
    if (AIR_E_OK == rv)
    {
        ...
        if (AIR_E_OK == rv)
        {
            ...
            {
                DIAG_PRINT(HAL_DBG_INFO, "[Dbg]: an8858 surge protection mode - 0R\n");
                /* cr_r50ohm_rsel_tx_a */
            ...
            {
                DIAG_PRINT(HAL_DBG_INFO, "[Dbg]: an8858 surge protection mode - 5R\n");
                /* cr_r50ohm_rsel_tx_a */
                reg_data &= ~(BITS_RANGE(8, 7));
                efuse_data = BITS_OFF_R(efuse_data_raw[0], 24, 7);
                _hal_an8858_phy_getSurgeVal(efuse_data, &table_val);
            ...
            {
                DIAG_PRINT(HAL_DBG_ERR, "port %u, an8858 surge protection mode %u is invalid!\n", port,
                           surge_mode.value);
                return AIR_E_BAD_PARAMETER;
            }
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x174, reg_data);
    ...
    if (AIR_E_OK == rv)
    {
        ...
        if (AIR_E_OK == rv)
        {
            ...
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x175, reg_data);
            DIAG_PRINT(HAL_DBG_INFO, "[Port %u]Write CL45 dev 0x%02X addr:0x%04X, data:0x%04X\n", port, MMD_DEV_VSPEC1,
                       0x175, reg_data);
        }
    }
    ...
    const UI32_T port)
{
    ...
    UI16_T          on_rate_addr = 0, blk_rate_addr = 0, port_index = 0;
    UI32_T          reg_data = 0;

    osal_memset(&led_behavior, 0, sizeof(AIR_CFG_VALUE_T));
    led_behavior.value = HAL_AN8858_PHY_LED_COUNT;
    ...
    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, HAL_AN8858_PHY_LED_CTRL_SELECT, data);
    if (AIR_E_OK == rv)
    {
        for (led_id = 0; led_id < led_count; led_id++)
        {
            if (led_id == 0)
            {
                link_reg_addr = HAL_AN8858_PHY_LED_0_ON_MASK;
                blk_reg_addr = HAL_AN8858_PHY_LED_0_BLK_MASK;
                on_rate_addr = HAL_AN8858_PHY_LED_0_ON_DURATION;
                blk_rate_addr = HAL_AN8858_PHY_LED_0_BLK_DURATION;
            }
            else if (led_id == 1)
            {
                link_reg_addr = HAL_AN8858_PHY_LED_1_ON_MASK;
                blk_reg_addr = HAL_AN8858_PHY_LED_1_BLK_MASK;
                on_rate_addr = HAL_AN8858_PHY_LED_1_ON_DURATION;
                blk_rate_addr = HAL_AN8858_PHY_LED_1_BLK_DURATION;
            }
            else if (led_id == 2)
            {
                link_reg_addr = HAL_AN8858_PHY_LED_2_ON_MASK;
                blk_reg_addr = HAL_AN8858_PHY_LED_2_BLK_MASK;
                on_rate_addr = HAL_AN8858_PHY_LED_2_ON_DURATION;
                blk_rate_addr = HAL_AN8858_PHY_LED_2_BLK_DURATION;
            }
            else
            {
                link_reg_addr = HAL_AN8858_PHY_LED_3_ON_MASK;
                blk_reg_addr = HAL_AN8858_PHY_LED_3_BLK_MASK;
                on_rate_addr = HAL_AN8858_PHY_LED_3_ON_DURATION;
                blk_rate_addr = HAL_AN8858_PHY_LED_3_BLK_DURATION;
            }
            ...
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_LINK_1000, HAL_AN8858_PHY_LED_LINK_1000, link_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_LINK_100, HAL_AN8858_PHY_LED_LINK_100, link_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_LINK_10, HAL_AN8858_PHY_LED_LINK_10, link_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_LINK_FULLDPLX, HAL_AN8858_PHY_LED_LINK_FULLDPLX,
                            link_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_LINK_HALFDPLX, HAL_AN8858_PHY_LED_LINK_HALFDPLX,
                            link_reg_data);

            if (HAL_AN8858_PHY_LED_TYPE_SERIAL == led_type)
            {
                GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_HIGH_ACTIVE, HAL_AN8858_PHY_LED_POL_HIGH_ACT,
                                link_reg_data);
            }
            else
            {
                link_reg_data |= HAL_AN8858_PHY_LED_POL_HIGH_ACT;
            }

            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_TX_1000, HAL_AN8858_PHY_LED_BLINK_TX_1000,
                            blk_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_RX_1000, HAL_AN8858_PHY_LED_BLINK_RX_1000,
                            blk_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_TX_100, HAL_AN8858_PHY_LED_BLINK_TX_100,
                            blk_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_RX_100, HAL_AN8858_PHY_LED_BLINK_RX_100,
                            blk_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_TX_10, HAL_AN8858_PHY_LED_BLINK_TX_10,
                            blk_reg_data);
            GET_PHY_ABILITY(led_config, HAL_AN8858_PHY_LED_BHV_BLINK_RX_10, HAL_AN8858_PHY_LED_BLINK_RX_10,
                            blk_reg_data);

            if (link_reg_data != 0 || blk_reg_data != 0)
            {
                link_reg_data |= HAL_AN8858_PHY_LED_FUNC_ENABLE;
            }

            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, on_rate_addr,
                                                   HAL_AN8858_PHY_LED_ON_RATE_DEFAULT);
            if (AIR_E_OK == rv)
            {
                rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, blk_rate_addr,
                                                       HAL_AN8858_PHY_LED_BLINK_RATE_DEFAULT);
                if (AIR_E_OK == rv)
                {
                    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, link_reg_addr, link_reg_data);
                    if (AIR_E_OK == rv)
                    {
                        rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, blk_reg_addr, blk_reg_data);
                    }
                }
            }

            if (HAL_AN8858_PHY_LED_TYPE_PARALLEL == led_type)
            {
                if (AIR_E_OK == rv)
                {
                    rv = aml_readReg(unit, CORAL_RG_GPIO_L_INV_REG, &reg_data, sizeof(reg_data));
                    if (AIR_E_OK == rv)
                    {
                        HAL_AIR_PORT_TO_PHY_PORT(unit, port, phy_port);
                        if (led_config & HAL_AN8858_PHY_LED_BHV_HIGH_ACTIVE)
                        {
                            /* high active */
                            reg_data &= ~(1 << phy_port);
                        }
                        ...
                        rv = aml_writeReg(unit, CORAL_RG_GPIO_L_INV_REG, &reg_data, sizeof(reg_data));
                    }
                }
            }
            ...
            {
                rv |= hal_mdio_writeC45FromIntBusByPort(unit, port_index, MMD_DEV_VSPEC2, 0x720, 0xff);
            }
            AIR_PORT_FOREACH(HAL_PORT_BMP_GPHY(unit), port_index)
            {
                rv |= hal_mdio_writeC45FromIntBusByPort(unit, port_index, MMD_DEV_VSPEC2, 0x720, 0x0);
            }
        }
    }
    return rv;
...
static AIR_ERROR_NO_T
_hal_an8858_phy_ledInit(
    const UI32_T unit,
    const UI32_T port)
{
    AIR_ERROR_NO_T rv = AIR_E_OK;

    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, HAL_AN8858_PHY_LED_CFG_REG, 0);
    if (AIR_E_OK == rv)
    {
        rv = _hal_an8858_phy_ledConfig(unit, port);
    }
    return rv;
}

/* FUNCTION NAME:   _hal_an8858_phy_setWaveGen
 * PURPOSE:
 *      Set AN8858 PHY wave-gen
 * INPUT:
 *      unit            --  Device unit number
 *      port            --  Port number
 *
    ...
    const UI32_T port)
{
    ...
    rv = aml_readReg(unit, CORAL_RG_CYCLE_CFG_VALUE1, &reg_data, sizeof(UI32_T));
    if (AIR_E_OK == rv)
    {
        reg_data = (reg_data & 0x000000ff) | HAL_AN8858_PHY_WAVE_GEN_CYCLE_567;
        rv = aml_writeReg(unit, CORAL_RG_CYCLE_CFG_VALUE1, &reg_data, sizeof(UI32_T));
        if (AIR_E_OK == rv)
        {
            ...
            if (AIR_E_OK == rv)
            {
                if (led_behavior.value & HAL_AN8858_PHY_LED_BHV_HIGH_ACTIVE)
                {
                    /* high active */
                    reg_data = (HAL_AN8858_PHY_PATTERN_OFF << 16) | HAL_AN8858_PHY_PATTERN_ON;
                }
                else
                {
                    /* low active */
                    reg_data = (HAL_AN8858_PHY_PATTERN_ON << 16) | HAL_AN8858_PHY_PATTERN_OFF;
                }
                rv = aml_writeReg(unit, CORAL_RG_GPIO_FLASH_PRD_SET2, &reg_data, sizeof(UI32_T));
            }
        }
    }
    return rv;
...
/* FUNCTION NAME: hal_an8858_phy_init
 * PURPOSE:
 *      AN8858 PHY initialization
 *
 * INPUT:
 *      unit            --  Device ID
 *      port            --  PHY address
    ...
    const UI32_T port)
{
    ...
    rv = _hal_an8858_phy_pre_init(unit, port);
    if (AIR_E_OK == rv)
    {
        rv = _hal_an8858_phy_setSurgeProtect(unit, port);
        if (AIR_E_OK == rv)
        {
            rv = _hal_an8858_phy_setPhyOpMode(unit, port, HAL_PHY_OP_MODE_NORMAL);
            if (AIR_E_OK == rv)
            {
                rv = _hal_an8858_phy_ledInit(unit, port);
                if (AIR_E_OK == rv)
                {
                    rv = _hal_an8858_phy_checkInitDone(unit, port, &init_status);
            ...
            if (AIR_E_OK == rv)
            {
                rv = _hal_an8858_phy_setRandomSeedByGenModule(unit, port);
            }
            if (AIR_E_OK == rv)
            {
                rv = _hal_an8858_phy_setWaveGen(unit, port);
            }
        }
    }
    return rv;
...
/* FUNCTION NAME:   hal_an8858_phy_setAdminState
 * PURPOSE:
    ...
    const HAL_PHY_ADMIN_STATE_T state)
{
    ...
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, &reg_value);
    if (AIR_E_OK == rv)
    {
        ...
        rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, reg_value);
        if ((AIR_E_OK == rv) && (HAL_PHY_ADMIN_STATE_ENABLE == state))
        {
            /* Re-train RX gain */
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x148, 0x200);
            if (AIR_E_OK == rv)
            {
                rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x148, 0x0);
            }
        }
    }
    return rv;
...
/* FUNCTION NAME:   hal_an8858_phy_getAdminState
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_setAutoNego
 * PURPOSE:
    ...
    const HAL_PHY_AUTO_NEGO_T auto_nego)
{
    ...
    if (HAL_PHY_PORT_DEV_REVISION_ID(unit, port) == HAL_AN8858_PHY_DEVICE_REV_ID_E1)
    {
        if (HAL_PHY_AUTO_NEGO_DISABLE == auto_nego)
        {
            rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE, &reg_value);
            if (AIR_E_OK == rv)
            {
                reg_value |= (ADVERTISE_10HALF | ADVERTISE_10FULL | ADVERTISE_100HALF | ADVERTISE_100FULL);
                rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE,
                                                       reg_value);
            }
        }
    }
...
/* FUNCTION NAME:   hal_an8858_phy_getAutoNego
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_setLocalAdvAbility
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setLocalAdvAbility(
    const UI32_T            unit,
    const UI32_T            port,
    const HAL_PHY_AN_ADV_T *ptr_adv)
{
    ...
    {
        rv = hal_an8858_phy_getPhyOpMode(unit, port, &phy_mode);
    ...
    if (AIR_E_OK == rv)
    {
        rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, &reg_value);
        if (AIR_E_OK == rv)
        {
            SET_PHY_ABILITY(ptr_adv->flags, HAL_PHY_AN_ADV_FLAGS_1000FUDX, ADVERTISE_1000FULL, reg_value);
            rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, reg_value);
        }
        ...
        if (AIR_E_OK == rv)
        {
            ...
            rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_ANEG, MMD_EEEAR, reg_value);
        }
    }
...
/* FUNCTION NAME:   hal_an8858_phy_getLocalAdvAbility
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getLocalAdvAbility(
    const UI32_T      unit,
    const UI32_T      port,
    HAL_PHY_AN_ADV_T *ptr_adv)
{
    ...
    rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_ANEG, MMD_EEEAR, &reg_value);
    if (AIR_E_OK == rv)
    {
...
/* FUNCTION NAME:   hal_an8858_phy_getRemoteAdvAbility
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getRemoteAdvAbility(
    const UI32_T      unit,
    const UI32_T      port,
    HAL_PHY_AN_ADV_T *ptr_adv)
{
    ...
    ptr_adv->flags = 0;
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, &reg_value);
    if (AIR_E_OK == rv)
    {
        GET_PHY_ABILITY(reg_value, LPA_1000FULL, HAL_PHY_AN_ADV_FLAGS_1000FUDX, ptr_adv->flags);
    }

    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, &reg_value);
    if (AIR_E_OK == rv)
    {
    ...
    rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_ANEG, MMD_EEELPAR, &reg_value);
    if (AIR_E_OK == rv)
    {
        GET_PHY_ABILITY(reg_value, (EEE_1000BASE_T | EEE_100BASE_TX), HAL_PHY_AN_ADV_FLAGS_EEE, ptr_adv->flags);
    }
...
/* FUNCTION NAME:   hal_an8858_phy_setSpeed
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_getSpeed
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_setDuplex
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_getDuplex
 * PURPOSE:
...
/* FUNCTION NAME:   hal_an8858_phy_getLinkStatus
 * PURPOSE:
    ...
    HAL_PHY_LINK_STATUS_T *ptr_status)
{
    ...
    if (AIR_E_OK == rv)
    {
        rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x0c6, &reg_data);
        if (AIR_E_OK == rv)
        {
            if ((ptr_status->flags & HAL_PHY_LINK_STATUS_FLAGS_LINK_UP) && (HAL_PHY_SPEED_1000M == ptr_status->speed))
            {
                if (HAL_AN8858_PHY_1G_LOW_POWER != reg_data)
                {
                    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x0c6,
                                                           HAL_AN8858_PHY_1G_LOW_POWER);
                }
            }
            ...
            {
                if (HAL_AN8858_PHY_100M_10M_LOW_POWER != reg_data)
                {
                    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x0c6,
                                                           HAL_AN8858_PHY_100M_10M_LOW_POWER);
                }
            }
            else
            {
                if (HAL_AN8858_PHY_UNPLUG_LOW_POWER != reg_data)
                {
                    rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x0c6,
                                                           HAL_AN8858_PHY_UNPLUG_LOW_POWER);
                }
            }
        }
    }
    if (AIR_E_OK == rv)
    {
        rv = _hal_an8858_phy_checkLinkStatus(unit, port,
                                             (ptr_status->flags & HAL_PHY_LINK_STATUS_FLAGS_LINK_UP ? TRUE : FALSE));
    }
    return rv;
}

/* FUNCTION NAME:   hal_an8858_phy_setLoopBack
 * PURPOSE:
    ...
    const BOOL_T         enable)
{
    ...
    if (HAL_PHY_LPBK_FAR_END == lpbk_type)
    {
        ...
        rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, MII_RESV2, &reg_data);
        if (AIR_E_OK == rv)
        {
            if (TRUE == enable)
            {
                reg_data |= HAL_AN8858_PHY_LPBK_FAR_END;
            }
            else
            {
                reg_data &= ~(HAL_AN8858_PHY_LPBK_FAR_END);
            }
            rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, MII_RESV2, reg_data);
        }
    }
    else
    {
        ...
        if (AIR_E_OK == rv)
        {
            ...
            rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, reg_data);
        }
    }
...
/* FUNCTION NAME:   hal_an8858_phy_getLoopBack
 * PURPOSE:
    ...
    BOOL_T              *ptr_enable)
{
    ...
    if (HAL_PHY_LPBK_FAR_END == lpbk_type)
    {
        ...
        if (AIR_E_OK == rv)
        {
            if (reg_data & HAL_AN8858_PHY_LPBK_FAR_END)
            {
                *ptr_enable = TRUE;
            }
            else
...
/* FUNCTION NAME:   hal_an8858_phy_setSmartSpeedDown
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setSmartSpeedDown(
    const UI32_T             unit,
    const UI32_T             port,
    const HAL_PHY_SSD_MODE_T ssd_mode)
{
    ...
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x14, &data);
    if (AIR_E_OK == rv)
    {
        data &= ~BITS(2, 3);
        ...
        rv = hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x14, data);
    }
    return rv;
}

/* FUNCTION NAME:   hal_an8858_phy_getSmartSpeedDown
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getSmartSpeedDown(
    const UI32_T        unit,
    const UI32_T        port,
...
/* FUNCTION NAME:   hal_an8858_phy_testTxCompliance
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_testTxCompliance(
    const UI32_T                       unit,
    const UI32_T                       port,
    const HAL_PHY_TX_COMPLIANCE_MODE_T mode)
{
    ...
    ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x3e, 0x0);
    switch (mode)
    {
        ...
        case HAL_PHY_TX_COMPLIANCE_MODE_10M_SINE_PAIR_B:
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x8000);
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x100);
            ...
            ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x145, u16dat);
            ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x33, 0x177);
            ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC2, 0x27b, 0x1177);
            if ((HAL_PHY_TX_COMPLIANCE_MODE_10M_NLP_PAIR_A == mode) ||
            ...
            ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x1a3, u16dat);
            if ((HAL_PHY_TX_COMPLIANCE_MODE_10M_NLP_PAIR_A == mode) ||
                (HAL_PHY_TX_COMPLIANCE_MODE_10M_NLP_PAIR_B == mode))
            {
                u16dat = 0x110e;
            ...
            ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x1a4, u16dat);
            if ((HAL_PHY_TX_COMPLIANCE_MODE_10M_NLP_PAIR_A == mode) ||
                (HAL_PHY_TX_COMPLIANCE_MODE_10M_NLP_PAIR_B == mode))
            {
                u16dat = 0x0;
            ...
            ret |=
                hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, MII_AUX_CTRL_STA, u16dat);
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, 0x0);
            break;
        ...
        case HAL_PHY_TX_COMPLIANCE_MODE_1000M_TM3:
            ...
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, u16dat);
            if (HAL_PHY_TX_COMPLIANCE_MODE_1000M_TM1 == mode)
            {
                ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x14, 0x4444);
            }
            else if (HAL_PHY_TX_COMPLIANCE_MODE_1000M_TM3 == mode)
            {
                ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x143, 0x200);
            }
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x1040);
            break;
        ...
        case HAL_PHY_TX_COMPLIANCE_MODE_1000M_TM4_PAIR_D:
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, 0x8700);
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x1040);
            ...
            {
                ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x3e, u16dat);
            }
            break;
        ...
        case HAL_PHY_TX_COMPLIANCE_MODE_100M_PAIR_B_DISCRETE:
            ret |= hal_mdio_writeC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, 0x2100);
            ...
            if ((HAL_PHY_TX_COMPLIANCE_MODE_100M_PAIR_A == mode) ||
                (HAL_PHY_TX_COMPLIANCE_MODE_100M_PAIR_A_DISCRETE == mode))
            {
                ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x23, 0x882);
            }
            else
            {
                ret |= hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, 0x24, 0x882);
            }
            break;
...
/* FUNCTION NAME:   hal_an8858_phy_setPhyLedCtrlMode
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedCtrlMode(
    const UI32_T                  unit,
    const UI32_T                  port,
    const UI32_T                  led_id,
    const HAL_PHY_LED_CTRL_MODE_T ctrl_mode)
{
    ...
    if (HAL_AN8858_PHY_LED_TYPE_SERIAL == led_behavior.value)
    {
        rv = aml_readReg(unit, CORAL_SLED_SRC_SEL, &reg_data, sizeof(reg_data));
        if (AIR_E_OK == rv)
        {
            bit_mask = HAL_AN8858_PHY_LED_SRC_SEL_POS(phy_port, led_id);
            ...
            rv = aml_writeReg(unit, CORAL_SLED_SRC_SEL, &reg_data, sizeof(reg_data));
            if (AIR_E_OK == rv)
            {
                if (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode)
                {
                    rv = hal_an8858_phy_getPhyLedForceState(unit, port, led_id, &state);
                    if (AIR_E_OK == rv)
                    {
                        rv = hal_an8858_phy_setPhyLedForceState(unit, port, led_id, state);
                    }
                }
                else
                {
    ...
    else if (HAL_AN8858_PHY_LED_TYPE_PARALLEL == led_behavior.value)
    {
        rv = aml_readReg(unit, CORAL_RG_GPIO_FLASH_MODE_CFG, &reg_data, sizeof(reg_data));
        if (AIR_E_OK == rv)
        {
            bit_mask = (1 << phy_port);
            if (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode)
            ...
            rv = aml_writeReg(unit, CORAL_RG_GPIO_FLASH_MODE_CFG, &reg_data, sizeof(reg_data));
            if (AIR_E_OK == rv)
            {
                rv = aml_readReg(unit, CORAL_REG_GPIO_MODE_SEL(phy_port), &reg_data, sizeof(reg_data));
                if (AIR_E_OK == rv)
                {
                    if (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode)
                    {
                        reg_data &= ~(CORAL_REG_GPIO_AUX_CFG(phy_port, 0xf));
                    }
                    else
                    {
                        reg_data |= (CORAL_REG_GPIO_AUX_CFG(phy_port, 0xa));
                    }
                    rv = aml_writeReg(unit, CORAL_REG_GPIO_MODE_SEL(phy_port), &reg_data, sizeof(reg_data));
                }
            }
            if (AIR_E_OK == rv)
            {
                rv = aml_readReg(unit, CORAL_REG_GPIOOE, &reg_data, sizeof(reg_data));
                if (AIR_E_OK == rv)
                {
                    if (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode)
                    {
                    ...
                    rv = aml_writeReg(unit, CORAL_REG_GPIOOE, &reg_data, sizeof(reg_data));
                }
            }
            if (AIR_E_OK == rv)
            {
...
/* FUNCTION NAME:   hal_an8858_phy_getPhyLedCtrlMode
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedCtrlMode(
    const UI32_T             unit,
    const UI32_T             port,
    const UI32_T             led_id,
    HAL_PHY_LED_CTRL_MODE_T *ptr_ctrl_mode)
{
    UI32_T          reg_data = 0, phy_port = 0, bit_mask = 0;
    AIR_ERROR_NO_T  rv = AIR_E_OK;
    ...
    if (HAL_AN8858_PHY_LED_TYPE_SERIAL == led_count.value)
    {
        rv = aml_readReg(unit, CORAL_SLED_SRC_SEL, &reg_data, sizeof(reg_data));
        if (AIR_E_OK == rv)
        {
    ...
    else if (HAL_AN8858_PHY_LED_TYPE_PARALLEL == led_count.value)
    {
        rv = aml_readReg(unit, CORAL_RG_GPIO_FLASH_MODE_CFG, &reg_data, sizeof(reg_data));
        if (AIR_E_OK == rv)
        {
...
/* FUNCTION NAME:   hal_an8858_phy_setPhyLedForceState
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedForceState(
    const UI32_T              unit,
    const UI32_T              port,
    const UI32_T              led_id,
    const HAL_PHY_LED_STATE_T state)
{
    ...
    rv = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, HAL_AN8858_PHY_LED_CFG_REG, &data);
    if (AIR_E_OK == rv)
    {
        offset = (HAL_AN8858_PHY_LED_CFG_RG_OFFSET * led_id);
        data &= ~(BITS_OFF_L(HAL_AN8858_PHY_LED_CFG_STATE_MASK, offset, HAL_AN8858_PHY_LED_CFG_STATE_WIDTH));
        data |= BITS_OFF_L(state, offset, HAL_AN8858_PHY_LED_CFG_STATE_WIDTH);
        rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, HAL_AN8858_PHY_LED_CFG_REG, data);
    }
    ...
    if (AIR_E_OK == rv)
    {
        ...
        if (HAL_AN8858_PHY_LED_TYPE_SERIAL == led_behavior.value)
        {
            rv = hal_an8858_phy_getPhyLedCtrlMode(unit, port, led_id, &ctrl_mode);
            if ((AIR_E_OK == rv) && (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode))
            {
                HAL_AIR_PORT_TO_PHY_PORT(unit, port, phy_port);
                flash_set_pos = HAL_AN8858_PHY_LED_FLASH_SET_POS(phy_port, led_id);
                reg_addr = HAL_AN8858_PHY_RG_LED_FLASH_SET_CTRL(phy_port);
                rv = aml_readReg(unit, reg_addr, &reg_data, sizeof(reg_data));
                if (AIR_E_OK == rv)
                {
                    bit_mask = (HAL_AN8858_PHY_LED_FLASH_SET_MASK << flash_set_pos);
                    reg_data &= ~(bit_mask);
                    switch (state)
                    {
                        case HAL_PHY_LED_STATE_FORCE_PATT:
                            rv = hal_an8858_phy_getPhyLedForcePattCfg(unit, port, led_id, &led_patt);
                            if (AIR_E_OK == rv)
                            {
                                rv = hal_an8858_phy_setPhyLedForcePattCfg(unit, port, led_id, led_patt);
                            }
                            break;
                        case HAL_PHY_LED_STATE_OFF:
                            reg_data |= (HAL_AN8858_PHY_FLASH_SET_FORCE_OFF << flash_set_pos);
                            rv = aml_writeReg(unit, reg_addr, &reg_data, sizeof(reg_data));
                            break;
                        case HAL_PHY_LED_STATE_ON:
                            reg_data |= (HAL_AN8858_PHY_FLASH_SET_FORCE_ON << flash_set_pos);
                            rv = aml_writeReg(unit, reg_addr, &reg_data, sizeof(reg_data));
                            break;
                        default:
                            rv = AIR_E_BAD_PARAMETER;
        ...
        else if (HAL_AN8858_PHY_LED_TYPE_PARALLEL == led_behavior.value)
        {
            ...
            {
                HAL_AIR_PORT_TO_PHY_PORT(unit, port, phy_port);
                flash_set_pos = HAL_AN8858_PHY_GPIO_FLASH_MAP_CFG_POS(phy_port);
                rv = aml_readReg(unit, CORAL_RG_GPIO_FLASH_MAP_CFG0, &reg_data, sizeof(reg_data));
                {
                    bit_mask = (HAL_AN8858_PHY_LED_FLASH_SET_MASK << flash_set_pos);
                    reg_data &= ~(bit_mask);
                    switch (state)
                    {
                        case HAL_PHY_LED_STATE_FORCE_PATT:
                            ...
                            break;
                        case HAL_PHY_LED_STATE_OFF:
                            reg_data |= ((BIT(3) | HAL_AN8858_PHY_FLASH_SET_FORCE_OFF) << flash_set_pos);
                            rv = aml_writeReg(unit, CORAL_RG_GPIO_FLASH_MAP_CFG0, &reg_data, sizeof(reg_data));
                            break;
                        case HAL_PHY_LED_STATE_ON:
                            reg_data |= ((BIT(3) | HAL_AN8858_PHY_FLASH_SET_FORCE_ON) << flash_set_pos);
                            rv = aml_writeReg(unit, CORAL_RG_GPIO_FLASH_MAP_CFG0, &reg_data, sizeof(reg_data));
                            break;
                        default:
                            rv = AIR_E_BAD_PARAMETER;
...
/* FUNCTION NAME:   hal_an8858_phy_getPhyLedForceState
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedForceState(
    const UI32_T         unit,
    const UI32_T         port,
    const UI32_T         led_id,
    HAL_PHY_LED_STATE_T *ptr_state)
{
    ...
    {
        offset = (HAL_AN8858_PHY_LED_CFG_RG_OFFSET * led_id);
        *ptr_state = BITS_OFF_R(data, offset, HAL_AN8858_PHY_LED_CFG_STATE_WIDTH);
    }
    return rv;
}

/* FUNCTION NAME:   hal_an8858_phy_setPhyLedForcePattCfg
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedForcePattCfg(
    const UI32_T             unit,
    const UI32_T             port,
    const UI32_T             led_id,
    const HAL_PHY_LED_PATT_T pattern)
{
    ...
    if (AIR_E_OK == rv)
    {
        offset = (HAL_AN8858_PHY_LED_CFG_PATT_WIDTH + (HAL_AN8858_PHY_LED_CFG_RG_OFFSET * led_id));
        data &= ~(BITS_OFF_L(HAL_AN8858_PHY_LED_CFG_PATT_MASK, offset, HAL_AN8858_PHY_LED_CFG_PATT_WIDTH));
        data |= BITS_OFF_L(pattern, offset, HAL_AN8858_PHY_LED_CFG_PATT_WIDTH);
        rv = hal_mdio_writeC45FromIntBusByPort(unit, port, MMD_DEV_VSPEC1, HAL_AN8858_PHY_LED_CFG_REG, data);
    }
    ...
    if (AIR_E_OK == rv)
    {
        ...
        if (HAL_AN8858_PHY_LED_TYPE_SERIAL == led_behavior.value)
        {
            ...
            if ((AIR_E_OK == rv) && (HAL_PHY_LED_CTRL_MODE_FORCE == ctrl_mode))
            {
                rv = hal_an8858_phy_getPhyLedForceState(unit, port, led_id, &led_state);
                if ((AIR_E_OK == rv) && (HAL_PHY_LED_STATE_FORCE_PATT == led_state))
                {
                    switch (pattern)
                    {
                        case HAL_PHY_LED_PATT_HZ_HALF:
                            wave_id = HAL_AN8858_PHY_FLASH_SET_HALF_HZ;
                            break;
                        case HAL_PHY_LED_PATT_HZ_ONE:
                            wave_id = HAL_AN8858_PHY_FLASH_SET_ONE_HZ;
                            break;
                        case HAL_PHY_LED_PATT_HZ_TWO:
                            wave_id = HAL_AN8858_PHY_FLASH_SET_TWO_HZ;
                            break;
                        default:
                            rv = AIR_E_BAD_PARAMETER;
                            break;
...
/* FUNCTION NAME:   hal_an8858_phy_getPhyLedForcePattCfg
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedForcePattCfg(
    const UI32_T        unit,
    const UI32_T        port,
    const UI32_T        led_id,
    HAL_PHY_LED_PATT_T *ptr_pattern)
{
    ...
    {
        offset = (HAL_AN8858_PHY_LED_CFG_PATT_WIDTH + (HAL_AN8858_PHY_LED_CFG_RG_OFFSET * led_id));
        *ptr_pattern = BITS_OFF_R(data, offset, HAL_AN8858_PHY_LED_CFG_PATT_WIDTH);
    }
    return rv;
}

/* FUNCTION NAME: hal_an8858_phy_triggerCableTest
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_triggerCableTest(
    const UI32_T               unit,
    const UI32_T               port,
...
/* FUNCTION NAME: hal_an8858_phy_getCableTestRawData
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getCableTestRawData(
    const UI32_T unit,
    const UI32_T port,
...
/* FUNCTION NAME: hal_an8858_phy_setPhyLedGlbCfg
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedGlbCfg(
    const UI32_T           unit,
    const UI32_T           port,
...
/* FUNCTION NAME: hal_an8858_phy_getPhyLedGlbCfg
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedGlbCfg(
    const UI32_T           unit,
    const UI32_T           port,
    HAL_PHY_LED_GLB_CFG_T *ptr_glb_cfg)
{
...
/* FUNCTION NAME: hal_an8858_phy_setPhyLedBlkEvent
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedBlkEvent(
    const UI32_T unit,
    const UI32_T port,
    const UI32_T led_id,
...
/* FUNCTION NAME: hal_an8858_phy_getPhyLedBlkEvent
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedBlkEvent(
    const UI32_T unit,
    const UI32_T port,
    const UI32_T led_id,
...
/* FUNCTION NAME: hal_an8858_phy_setPhyLedDuration
 * PURPOSE:
 *      Set LED duration
 * INPUT:
 *      unit                     -- Device ID
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyLedDuration(
    const UI32_T                      unit,
    const UI32_T                      port,
...
/* FUNCTION NAME: hal_an8858_phy_getPhyLedDuration
 * PURPOSE:
 *      Get LED duration
 * INPUT:
 *      unit                     -- Device ID
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_getPhyLedDuration(
    const UI32_T                      unit,
    const UI32_T                      port,
    const HAL_PHY_LED_BLK_CTRL_MODE_T mode,
    UI32_T                           *ptr_time)
...
/* FUNCTION NAME: hal_an8858_phy_setPhyOpMode
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_an8858_phy_setPhyOpMode(
    const UI32_T            unit,
    const UI32_T            port,
    const HAL_PHY_OP_MODE_T mode)
{
    AIR_ERROR_NO_T rv = AIR_E_OK;
    rv = _hal_an8858_phy_setPhyOpMode(unit, port, mode);
    return rv;
}

/* FUNCTION NAME: hal_an8858_phy_getPhyOpMode
 * PURPOSE:
    ...
    HAL_PHY_OP_MODE_T *ptr_mode)
{
    ...
    if (AIR_E_OK == rv)
    {
        switch (reg_data)
        {
            case HAL_AN8858_PHY_LONG_REACH:
                *ptr_mode = HAL_PHY_OP_MODE_LONG_REACH;
                break;
            case HAL_AN8858_PHY_NORMAL:
...
/* FUNCTION NAME: hal_an8858_phy_dumpPhyPara
 * PURPOSE:
 *      Dump Phy parameters.
 * INPUT:
 *      unit                     -- Device ID
    ...
    const UI32_T port)
{
    ...
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMSR, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMSR, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x10, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, 0x10, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x11, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, 0x11, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x16, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0000, 0x16, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x12, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0001, 0x12, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x14, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0001, 0x14, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x17, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0001, 0x17, reg_data);
    }
    rv = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x1a, &reg_data);
    if (AIR_E_OK == rv)
    {
        osal_printf("unit=%2u    port=%2u    page=%2u    reg-addr=%2u    reg-data=0x%04x\n", unit, port,
                    HAL_AN8858_PHY_PHY_PAGE_0001, 0x1a, reg_data);
    }
...
/* FUNCTION NAME: hal_an8858_phy_dumpDebugInfo
 * PURPOSE:
    ...
    const UI32_T port)
{
    ...
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMCR, reg_data);
    ...
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMSR, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_BMSR, reg_data);
        osal_printf("  an_done = %u             remote_fault = %u\n", BITS_OFF_R(reg_data, 5, 1),
                    BITS_OFF_R(reg_data, 4, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_ADVERTISE, reg_data);
    ...
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_LPA, reg_data);
        osal_printf("  asym_pause = %u          pause = %u               100m_full = %u\n", BITS_OFF_R(reg_data, 11, 1),
                    BITS_OFF_R(reg_data, 10, 1), BITS_OFF_R(reg_data, 8, 1));
        osal_printf("  100m_half = %u           10m_full = %u            10m_half = %u\n", BITS_OFF_R(reg_data, 7, 1),
                    BITS_OFF_R(reg_data, 6, 1), BITS_OFF_R(reg_data, 5, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_CTRL1000, reg_data);
    ...
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, MII_STAT1000, reg_data);
        osal_printf("  master/slave_fault = %u  1000m_full = %u\n", BITS_OFF_R(reg_data, 15, 1),
                    BITS_OFF_R(reg_data, 11, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x10, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, 0x10, reg_data);
        osal_printf("  100m_link_status = %u\n", BITS_OFF_R(reg_data, 12, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x11, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, 0x11, reg_data);
        osal_printf("  1000m_link_status = %u\n", BITS_OFF_R(reg_data, 12, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0000, 0x16, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0000, 0x16, reg_data);
        osal_printf("  10m_link_status = %u\n", BITS_OFF_R(reg_data, 6, 1));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x19, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0001, 0x19, reg_data);
        osal_printf("  lpi_rx_state = %u        lpi_tx_state = %u\n", BITS_OFF_R(reg_data, 12, 3),
                    BITS_OFF_R(reg_data, 8, 3));
    }
    rc = hal_mdio_readC22FromIntBusByPort(unit, port, HAL_AN8858_PHY_PHY_PAGE_0001, 0x1a, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl22 phy_addr/page/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
                    HAL_PHY_PORT_DEV_PHY_ADDR(unit, port), HAL_AN8858_PHY_PHY_PAGE_0001, 0x1a, reg_data);
        osal_printf("  far_end_loopback = %u\n", BITS_OFF_R(reg_data, 15, 1));
    }
    rc = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_ANEG, MMD_EEEAR, &reg_data);
    if (AIR_E_OK == rc)
    {
    ...
    rc = hal_mdio_readC45FromIntBusByPort(unit, port, MMD_DEV_ANEG, MMD_EEELPAR, &reg_data);
    if (AIR_E_OK == rc)
    {
        osal_printf(" cl45 phy_addr/dev/reg_addr/data = 0x%02x/0x%02x/0x%02x/0x%04x\n",
...
const HAL_PHY_DRIVER_T _ext_an8858_phy_func_vec = {
...
AIR_ERROR_NO_T
hal_an8858_phy_getDriver(
    HAL_PHY_DRIVER_T **pptr_hal_driver,
    UI32_T             param)
{
    (*pptr_hal_driver) = (HAL_PHY_DRIVER_T *)&_ext_an8858_phy_func_vec;
