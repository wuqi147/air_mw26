 ...
 */
#define _HAL_PEARL_PORT_SET_REG_BIT(cond, reg, bit) \
    do                                              \
    {                                               \
        if (TRUE == (cond))                         \
        {                                           \
...
#define _HAL_PEARL_PORT_SET_FLAGS(_src_struct_, _src_flag_, _dst_struct_, _dst_flag_) \
    do                                                                                \
    {                                                                                 \
 ...
 */
DIAG_SET_MODULE_INFO(AIR_MODULE_PORT, "hal_pearl_port.c");

/* LOCAL SUBPROGRAM SPECIFICATIONS
 */
/* STATIC VARIABLE DECLARATIONS
 */
static const UI8_T _hal_pearl_port_pled_gpio[5] = {7, 5, 4, 3, 2};
static const UI8_T _hal_pearl_port_pled_mapping[5] = {0, 4, 8, 12, 16};
/* LOCAL SUBPROGRAM BODIES
 */

/* FUNCTION NAME: _hal_pearl_port_updateMac
 * PURPOSE:
    ...
    void        *ptr_cookie)
{
    ...
    hal_pearl_ifmon_getPortStatus(unit, port, &port_status);

    /* Check MAC force mode */
    aml_readReg(unit, PEARL_PMCR(mac_port), &reg_mcr, sizeof(reg_mcr));
    if (reg_mcr & BIT(PEARL_MCR_FORCE_MODE_OFFT))
    {
        /* Force mode */
        /* Modify force link status in force mode only */
        if (TRUE == link)
        {
            reg_mcr |= BIT(PEARL_MCR_FORCE_LINK_OFFT);
        }
        else
        {
            reg_mcr &= ~BIT(PEARL_MCR_FORCE_LINK_OFFT);
    ...
    else
    {
        ...
        if (TRUE == link)
        {
            ...
            if (AIR_E_OK == rv)
            {
                ...
                {
                    aml_readReg(unit, PEARL_PMSR(mac_port), &reg_msr, sizeof(reg_msr));
                    if (!(reg_msr & BIT(PEARL_REG_PORT_DUPLEX_OFFT)))
                    {
                        if (reg_msr & BIT(PEARL_REG_PORT_TX_FLOWCTRL_OFFT))
                        {
                            reg_mcr |= BIT(PEARL_MCR_BACK_PRESSURE_OFFT);
                        }
                        else
                        {
                            reg_mcr &= ~BIT(PEARL_MCR_BACK_PRESSURE_OFFT);
            ...
            {
                reg_mcr &= ~BIT(PEARL_MCR_EEE_1G_EN_OFFT);
                reg_mcr &= ~BIT(PEARL_MCR_EEE_100_EN_OFFT);
                reg_mcr &= ~BIT(PEARL_MCR_EEE_2P5G_EN_OFFT);
                rv = hal_phy_getPhyOpMode(unit, port, &phy_mode);
                if ((AIR_E_OK == rv) && (HAL_PHY_OP_MODE_NORMAL == phy_mode))
                {
    ...
    aml_writeReg(unit, PEARL_PMCR(mac_port), &reg_mcr, sizeof(reg_mcr));
}

static AIR_ERROR_NO_T
_hal_pearl_port_initCpu(
    const UI32_T unit)
{
    AIR_ERROR_NO_T      rv;
    ...
    /* Set PEARL_MFC */
    rv = aml_readReg(unit, PEARL_MFC, &reg_data, sizeof(reg_data));
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    reg_data |= BITS_OFF_L(cpu_mac_port, PEARL_CSR_CPU_PORT, PEARL_CSR_CPU_PORT_LENGTH);
    reg_data |= BIT(PEARL_CSR_CPU_PORT_EN);
    rv = aml_writeReg(unit, PEARL_MFC, &reg_data, sizeof(reg_data));
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    ...
    rv = hal_pearl_stag_setPort(unit, cpu_air_port, TRUE);
    if (AIR_E_OK != rv)
    {
        return rv;
    }

    rv = hal_pearl_stag_setMode(unit, cpu_air_port, AIR_STAG_MODE_INSERT);
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    ...
    rv = hal_pearl_port_setDuplex(unit, cpu_air_port, AIR_PORT_DUPLEX_FULL);
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    ...
    rv = hal_pearl_port_setFlowCtrl(unit, cpu_air_port, AIR_PORT_DIR_BOTH, FALSE);
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    ...
    rv = hal_pearl_port_setSpeed(unit, cpu_air_port, speed);
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    ...
    rv = aml_readReg(unit, PEARL_PMCR(cpu_mac_port), &reg_data, sizeof(reg_data));
    if (AIR_E_OK != rv)
    {
        return rv;
    }
    reg_data |= BIT(PEARL_MCR_FORCE_MODE_OFFT);
    reg_data |= BIT(PEARL_MCR_BACK_PRESSURE_OFFT);

    reg_data &= ~BIT(PEARL_MCR_EEE_100_EN_OFFT);
    reg_data &= ~BIT(PEARL_MCR_EEE_1G_EN_OFFT);
    reg_data &= ~BIT(PEARL_MCR_EEE_2P5G_EN_OFFT);
    rv = aml_writeReg(unit, PEARL_PMCR(cpu_mac_port), &reg_data, sizeof(reg_data));
    if (AIR_E_OK != rv)
    {
        return rv;
    }
...
static AIR_ERROR_NO_T
_hal_pearl_port_setLedGpioReg(
    const UI32_T unit)
{
    UI32_T          u32dat = 0, phy_port = 0, gpio = 0, port = 0;
    AIR_CFG_VALUE_T sled, led_type;
    ...
    if (LED_TYPE_SERIAL == led_type.value)
    {
        /* setup serial LED */
        osal_memset(&sled, 0, sizeof(AIR_CFG_VALUE_T));
        sled.value = 2;
        hal_cfg_getValue(unit, AIR_CFG_TYPE_PHY_LED_COUNT, &sled);
        u32dat = ((sled.value << 1) + PEARL_SLED_RISING_EDGE + PEARL_SLED_OUTPUT_TRANSITION_MODE) | (0xff << 8);
        aml_writeReg(unit, PEARL_SLED_CTRL0, &u32dat, sizeof(u32dat));

        aml_readReg(unit, PEARL_RG_LAN_LED_IOMUX, &u32dat, sizeof(u32dat));
        u32dat |= PEARL_RG_GPIO_SLED_MODE0;
        aml_writeReg(unit, PEARL_RG_LAN_LED_IOMUX, &u32dat, sizeof(u32dat));
    }
    else if (LED_TYPE_PARALLEL == led_type.value)
    {
        aml_writeReg(unit, PEARL_RG_GPIO_LED_SEL(7), &u32dat, sizeof(u32dat));
        aml_writeReg(unit, PEARL_RG_GPIO_LED_SEL(2), &u32dat, sizeof(u32dat));
        AIR_PORT_FOREACH(HAL_PORT_BMP_ETH(unit), port)
        {
            if (HAL_IS_GPHY_PORT_VALID(unit, port))
            {
                ...
                gpio = _hal_pearl_port_pled_gpio[phy_port];

                aml_readReg(unit, PEARL_RG_GPIO_LED_SEL(gpio), &u32dat, sizeof(u32dat));
                u32dat &= ~(PEARL_RG_GPIO_LED_MASK(gpio));
                u32dat |= PEARL_RG_GPIO_LED_VAL(gpio, _hal_pearl_port_pled_mapping[phy_port]);
                aml_writeReg(unit, PEARL_RG_GPIO_LED_SEL(gpio), &u32dat, sizeof(u32dat));
 ...
 */
/* FUNCTION NAME: hal_pearl_port_init
 * PURPOSE:
    ...
    const UI32_T unit)
{
    ...
    AIR_PORT_FOREACH(HAL_PORT_BMP_ETH(unit), port)
    {
        ...
        rc = hal_pearl_port_setAdminState(unit, port, FALSE);
        ...
        rc = hal_pearl_port_setSpeed(unit, port, AIR_PORT_SPEED_100M);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            break;
        }
        ...
        rc = hal_pearl_port_setDuplex(unit, port, AIR_PORT_DUPLEX_FULL);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            break;
        }
        ...
        rc = hal_pearl_port_setPhyLocalAdvAbility(unit, port, &adv);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            break;
        }
        ...
        rc = hal_pearl_port_setPhyAutoNego(unit, port, AIR_PORT_PHY_AN_ENABLE);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            break;
        }
        ...
        rc = hal_pearl_port_setAdminState(unit, port, admin_state);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            break;
        }
        ...
        rc = hal_pearl_port_setPortMatrix(unit, port, HAL_PORT_BMP_TOTAL(unit));
        if (AIR_E_OK != rc)
        {
            break;
        }
    ...
    if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
    {
        DIAG_PRINT(HAL_DBG_WARN, "set unit %u: port %u default value fail, rc = %d\n", unit, port, rc);
        return rc;
    }

    /* Initialize CPU port */
    rc = _hal_pearl_port_initCpu(unit);
    if (AIR_E_OK != rc)
    {
    ...
    {
        rc = _hal_pearl_port_setLedGpioReg(unit);
        if (AIR_E_OK != rc)
        {
    ...
    if (AIR_E_OK == rc)
    {
        if (HAL_MODULE_INITED(unit, AIR_MODULE_IFMON) & HAL_INIT_STAGE_TASK_RSRC)
        {
            ...
            rc = hal_pearl_ifmon_register(unit, _hal_pearl_port_updateMac, NULL);
            if (AIR_E_OK != rc)
            {
...
/* FUNCTION NAME: hal_pearl_port_deinit
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_setSerdesMode
 * PURPOSE:
    ...
    const AIR_PORT_SERDES_MODE_T mode)
{
    ...
    if (AIR_E_OK != rc)
    {
        DIAG_PRINT(HAL_DBG_WARN, "call hal_phy_setSerdesMode() fail, rc = %d\n", rc);
    }

    return rc;
}

/* FUNCTION NAME: hal_pearl_port_getSerdesMode
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_setPhyAutoNego
 * PURPOSE:
    ...
    const AIR_PORT_PHY_AN_T auto_nego)
{
    ...
    if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
    {
        DIAG_PRINT(HAL_DBG_WARN, "call hal_phy_setAutoNego() fail, rc = %d\n", rc);
        return rc;
    }

    if (AIR_PORT_PHY_AN_RESTART != auto_nego)
    {
        /* Set MAC configuration */
        HAL_AIR_PORT_TO_MAC_PORT(unit, port, mac_port);

        /* Read data from register */
        aml_readReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));

        if (AIR_PORT_PHY_AN_ENABLE == auto_nego)
        {
            /* Disable Force mode of port */
            u32dat &= ~BIT(PEARL_MCR_FORCE_MODE_OFFT);
            /* Enable EEE ability bits */
            u32dat |= BIT(PEARL_MCR_EEE_100_EN_OFFT);
            u32dat |= BIT(PEARL_MCR_EEE_1G_EN_OFFT);
            u32dat |= BIT(PEARL_MCR_EEE_2P5G_EN_OFFT);
        }
        ...
        {
            DIAG_PRINT(HAL_DBG_WARN, "Unknown parameter.\n");
            return AIR_E_BAD_PARAMETER;
        }

        /* Write data to register */
        aml_writeReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));
    }

    return AIR_E_OK;
}

/* FUNCTION NAME: hal_pearl_port_getPhyAutoNego
 * PURPOSE:
    ...
    AIR_PORT_PHY_AN_T *ptr_auto_nego)
{
    ...
    else if (AIR_E_NOT_SUPPORT == rc)
    {
        ...
        /* Read data from register */
        aml_readReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));
        (*ptr_auto_nego) = (BITS_OFF_R(u32dat, PEARL_MCR_FORCE_MODE_OFFT, PEARL_MCR_FORCE_MODE_LENG)) ?
                               AIR_PORT_PHY_AN_DISABLE :
                               AIR_PORT_PHY_AN_ENABLE;
        rc = AIR_E_OK;
    }
    return rc;
...
/* FUNCTION NAME: hal_pearl_port_setPhyLocalAdvAbility
 * PURPOSE:
    ...
    const AIR_PORT_PHY_AN_ADV_T *ptr_adv)
{
    ...
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_10HFDX, &hal_adv, HAL_PHY_AN_ADV_FLAGS_10HFDX);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_10FUDX, &hal_adv, HAL_PHY_AN_ADV_FLAGS_10FUDX);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_100HFDX, &hal_adv, HAL_PHY_AN_ADV_FLAGS_100HFDX);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_100FUDX, &hal_adv, HAL_PHY_AN_ADV_FLAGS_100FUDX);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_1000FUDX, &hal_adv, HAL_PHY_AN_ADV_FLAGS_1000FUDX);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_SYM_PAUSE, &hal_adv, HAL_PHY_AN_ADV_FLAGS_SYM_PAUSE);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_ASYM_PAUSE, &hal_adv, HAL_PHY_AN_ADV_FLAGS_ASYM_PAUSE);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_EEE, &hal_adv, HAL_PHY_AN_ADV_FLAGS_EEE);
    _HAL_PEARL_PORT_SET_FLAGS(ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_2500M, &hal_adv, HAL_PHY_AN_ADV_FLAGS_2500M);

    return hal_phy_setLocalAdvAbility(unit, port, &hal_adv);
}

/* FUNCTION NAME: hal_pearl_port_getPhyLocalAdvAbility
 * PURPOSE:
    ...
    AIR_PORT_PHY_AN_ADV_T *ptr_adv)
{
    ...
    if (AIR_E_OK == rc)
    {
        ...
        ptr_adv->flags = 0;
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_10HFDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_10HFDX);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_10FUDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_10FUDX);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_100HFDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_100HFDX);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_100FUDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_100FUDX);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_1000FUDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_1000FUDX);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_SYM_PAUSE, ptr_adv,
                                  AIR_PORT_PHY_AN_ADV_FLAGS_SYM_PAUSE);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_ASYM_PAUSE, ptr_adv,
                                  AIR_PORT_PHY_AN_ADV_FLAGS_ASYM_PAUSE);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_EEE, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_EEE);
        _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_2500M, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_2500M);
...
/* FUNCTION NAME: hal_pearl_port_getPhyRemoteAdvAbility
 * PURPOSE:
    ...
    AIR_PORT_PHY_AN_ADV_T *ptr_adv)
{
    ...
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_100FUDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_100FUDX);
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_1000FUDX, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_1000FUDX);
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_SYM_PAUSE, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_SYM_PAUSE);
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_ASYM_PAUSE, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_ASYM_PAUSE);
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_EEE, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_EEE);
    _HAL_PEARL_PORT_SET_FLAGS(&hal_adv, HAL_PHY_AN_ADV_FLAGS_2500M, ptr_adv, AIR_PORT_PHY_AN_ADV_FLAGS_2500M);
    DIAG_PRINT(HAL_DBG_INFO, "port %u AIR flags = %x\n", port, ptr_adv->flags);
...
/* FUNCTION NAME: hal_pearl_port_setSpeed
 * PURPOSE:
    ...
    const AIR_PORT_SPEED_T speed)
{
    ...
    /* Read data from register */
    aml_readReg(unit, PEARL_PMCR(mac_port), &u32mcr, sizeof(u32mcr));

    u32mcr &= ~BITS_RANGE(PEARL_MCR_FORCE_SPEED_OFFT, PEARL_MCR_FORCE_SPEED_LENG);
    u32mcr |= BITS_OFF_L(speed, PEARL_MCR_FORCE_SPEED_OFFT, PEARL_MCR_FORCE_SPEED_LENG);

    /* Write data to register */
    aml_writeReg(unit, PEARL_PMCR(mac_port), &u32mcr, sizeof(u32mcr));

    return AIR_E_OK;
}

/* FUNCTION NAME: hal_pearl_port_getSpeed
 * PURPOSE:
    ...
    AIR_PORT_SPEED_T *ptr_speed)
{
    ...
    if (HAL_CPU_PORT(unit) == port)
    {
        ...
        /* Read data from register */
        aml_readReg(unit, PEARL_PMCR(mac_port), &u32mcr, sizeof(u32mcr));
        (*ptr_speed) = BITS_OFF_R(u32mcr, PEARL_MCR_FORCE_SPEED_OFFT, PEARL_MCR_FORCE_SPEED_LENG);
    }
    else
    {
        /* Get PHY speed */
...
/* FUNCTION NAME: hal_pearl_port_setDuplex
 * PURPOSE:
    ...
    const AIR_PORT_DUPLEX_T duplex)
{
    ...
    {
        u32mcr |= BIT(PEARL_MCR_FORCE_DUPLEX_OFFT);
    }
    else
    {
        u32mcr &= ~BIT(PEARL_MCR_FORCE_DUPLEX_OFFT);
...
/* FUNCTION NAME: hal_pearl_port_getDuplex
 * PURPOSE:
 *      Get the duplex for a specific port.
 *
 * INPUT:
    ...
    AIR_PORT_DUPLEX_T *ptr_duplex)
{
    ...
    if (HAL_CPU_PORT(unit) == port)
    {
        ...
        /* Read data from register */
        aml_readReg(unit, PEARL_PMCR(mac_port), &u32mcr, sizeof(u32mcr));
        (*ptr_duplex) = BITS_OFF_R(u32mcr, PEARL_MCR_FORCE_DUPLEX_OFFT, PEARL_MCR_FORCE_DUPLEX_LENG);
    }
    else
    {
        /* Get PHY duplex */
...
/* FUNCTION NAME: hal_pearl_port_setBackPressure
 * PURPOSE:
    ...
    const BOOL_T bckPres)
{
    ...
    if (TRUE == bckPres)
    {
        u32dat |= BIT(PEARL_MCR_BACK_PRESSURE_OFFT);
    }
    else
    {
        u32dat &= ~BIT(PEARL_MCR_BACK_PRESSURE_OFFT);
...
/* FUNCTION NAME: hal_pearl_port_getBackPressure
 * PURPOSE:
    ...
    BOOL_T      *ptr_bckPres)
{
    ...
    /* Read data from register */
    aml_readReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));
    (*ptr_bckPres) = BITS_OFF_R(u32dat, PEARL_MCR_BACK_PRESSURE_OFFT, PEARL_MCR_BACK_PRESSURE_LENG);

    return AIR_E_OK;
}

/* FUNCTION NAME: hal_pearl_port_setFlowCtrl
 * PURPOSE:
    ...
    const BOOL_T         fc_en)
{
    ...
    if (TRUE == fc_en)
    {
        ...
        {
            u32mcr |= BIT(PEARL_MCR_FORCE_FLOWCTRL_TX_OFFT);
        }
        else if (AIR_PORT_DIR_RX == dir)
        {
            u32mcr |= BIT(PEARL_MCR_FORCE_FLOWCTRL_RX_OFFT);
        }
...
/* FUNCTION NAME: hal_pearl_port_getFlowCtrl
 * PURPOSE:
    ...
    BOOL_T              *ptr_fc_en)
{
    ...
    if (AIR_PORT_DIR_TX == dir)
    {
        (*ptr_fc_en) = BITS_OFF_R(u32dat, PEARL_MCR_FORCE_FLOWCTRL_TX_OFFT, PEARL_MCR_FORCE_FLOWCTRL_TX_LENG);
    }
    else if (AIR_PORT_DIR_RX == dir)
    {
        (*ptr_fc_en) = BITS_OFF_R(u32dat, PEARL_MCR_FORCE_FLOWCTRL_RX_OFFT, PEARL_MCR_FORCE_FLOWCTRL_RX_LENG);
    }
    else
    {
        return AIR_E_NOT_SUPPORT;
...
/* FUNCTION NAME: hal_pearl_port_setPhyLoopBack
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_getPhyLoopBack
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_getPortStatus
 * PURPOSE:
    ...
    AIR_PORT_STATUS_T *ptr_ps)
{
    ...
    if (HAL_CPU_PORT(unit) == port)
    {
        ...
        aml_readReg(unit, PEARL_PMSR(mac_port), &u32dat, sizeof(u32dat));

        _HAL_PEARL_PORT_SET_REG_BIT(BITS_OFF_R(u32dat, PEARL_REG_PORT_LINK_OFFT, PEARL_REG_PORT_LINK_LENG),
                                    ptr_ps->flags, AIR_PORT_STATUS_FLAGS_LINK_UP);
        ptr_ps->speed = BITS_OFF_R(u32dat, PEARL_REG_PORT_SPEED_OFFT, PEARL_REG_PORT_SPEED_LENG);
        ptr_ps->duplex = BITS_OFF_R(u32dat, PEARL_REG_PORT_DUPLEX_OFFT, PEARL_REG_PORT_DUPLEX_LENG);
    }
    else
    {
        rc = hal_phy_getLinkStatus(unit, port, &hal_status);
        if (AIR_E_OK == rc)
        {
            ...
            _HAL_PEARL_PORT_SET_FLAGS(&hal_status, HAL_PHY_LINK_STATUS_FLAGS_LINK_UP, ptr_ps,
        ...
        else if (AIR_E_NOT_SUPPORT == rc)
        {
            ...
            _HAL_PEARL_PORT_SET_FLAGS(&hal_status, HAL_PHY_LINK_STATUS_FLAGS_LINK_UP, ptr_ps,
                                      AIR_PORT_STATUS_FLAGS_LINK_UP);
            aml_readReg(unit, PEARL_PMSR(mac_port), &u32dat, sizeof(u32dat));
            pmsr_speed = BITS_OFF_R(u32dat, PEARL_REG_PORT_SPEED_OFFT, PEARL_REG_PORT_SPEED_LENG);
            pmsr_duplex = BITS_OFF_R(u32dat, PEARL_REG_PORT_DUPLEX_OFFT, PEARL_REG_PORT_DUPLEX_LENG);
            ...
            switch (pmsr_speed)
            {
                ...
                default:
                    rc = AIR_E_OTHERS;
                    DIAG_PRINT(HAL_DBG_WARN, "unknown type: pmcr speed (0x%x)\n", pmsr_speed);
                    break;
            }
            switch (pmsr_duplex)
            {
                ...
                case HAL_PHY_DUPLEX_FULL:
                    ...
                    break;
                default:
                    DIAG_PRINT(HAL_DBG_WARN, "unknown type: pmcr duplex (0x%x)\n", pmsr_duplex);
                    break;
            }
        }
    }
    ...
    if (hal_status.flags & HAL_PHY_LINK_STATUS_FLAGS_LINK_UP)
    {
            ...
            if (AIR_E_OK == rc)
            {
                ...
                if (AIR_E_OK == rc)
                {
                    ...
                    else
                    {
                        ...
                        aml_readReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));
                        u32datField =
                            BITS_OFF_R(u32dat, PEARL_MCR_FORCE_FLOWCTRL_TX_OFFT, PEARL_MCR_FORCE_FLOWCTRL_TX_LENG);
                        if (u32datField)
                        {
                            ptr_ps->flags |= AIR_PORT_STATUS_FLAGS_FLOW_CTRL_TX;
                        }
                        u32datField =
                            BITS_OFF_R(u32dat, PEARL_MCR_FORCE_FLOWCTRL_RX_OFFT, PEARL_MCR_FORCE_FLOWCTRL_RX_LENG);
                        if (u32datField)
                        {
                            ptr_ps->flags |= AIR_PORT_STATUS_FLAGS_FLOW_CTRL_RX;
                        }
        ...
        {
            /* read mcr */
            rc = aml_readReg(unit, PEARL_PMCR(mac_port), &u32dat, sizeof(u32dat));
            if (AIR_E_OK == rc)
            {
                ...
                {
                    u32datField = BITS_OFF_R(u32dat, PEARL_MCR_BACK_PRESSURE_OFFT, PEARL_MCR_BACK_PRESSURE_LENG);
                    if (u32datField)
                    {
                        ptr_ps->flags |= AIR_PORT_STATUS_FLAGS_FLOW_CTRL_TX;
                    }
...
/* FUNCTION NAME: hal_pearl_port_setAdminState
 * PURPOSE:
    ...
    const BOOL_T state)
{
    ...
    if (TRUE == state)
    {
        u32dat |= BIT(PEARL_MCR_FORCE_LINK_OFFT);
    }
    else
    {
        u32dat &= ~BIT(PEARL_MCR_FORCE_LINK_OFFT);
...
/* FUNCTION NAME: hal_pearl_port_getAdminState
 * PURPOSE:
    ...
    BOOL_T      *ptr_state)
{
    ...
    if (u32dat & BIT(PEARL_MCR_FORCE_LINK_OFFT))
    {
        *ptr_state = TRUE;
    }
    else
...
/* FUNCTION NAME: hal_pearl_port_setPhySmartSpeedDown
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_getPhySmartSpeedDown
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_setPortMatrix
 * PURPOSE:
    ...
    const AIR_PORT_BITMAP_T port_bitmap)
{
    ...
    aml_writeReg(unit, PEARL_PORTMATRIX(mac_port), &mac_port_bitmap[0], sizeof(UI32_T));
    return rc;
}

/* FUNCTION NAME: hal_pearl_port_getPortMatrix
 * PURPOSE:
    ...
    AIR_PORT_BITMAP_T port_bitmap)
{
    ...
    HAL_AIR_PORT_TO_MAC_PORT(unit, port, mac_port);
    aml_readReg(unit, PEARL_PORTMATRIX(mac_port), &mac_port_bitmap[0], sizeof(UI32_T));
    HAL_MAC_PBMP_TO_AIR_PBMP(unit, mac_port_bitmap, port_bitmap);
    return rc;
}

/* FUNCTION NAME: hal_pearl_port_setVlanMode
 * PURPOSE:
    ...
    const AIR_PORT_VLAN_MODE_T mode)
{
    ...
    val |= (mode & PEARL_PCR_PORT_VLAN_RELMASK) << PEARL_PCR_PORT_VLAN_OFFT;
    aml_writeReg(unit, PEARL_PCR(mac_port), &val, sizeof(UI32_T));
    return rc;
}

/* FUNCTION NAME: hal_pearl_port_getVlanMode
 * PURPOSE:
    ...
    AIR_PORT_VLAN_MODE_T *ptr_mode)
{
    ...
    aml_readReg(unit, PEARL_PCR(mac_port), &val, sizeof(UI32_T));
    *ptr_mode = (val >> PEARL_PCR_PORT_VLAN_OFFT) & PEARL_PCR_PORT_VLAN_RELMASK;
    return rc;
}

/* FUNCTION NAME:   hal_pearl_port_setPhyLedOnCtrl
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_getPhyLedOnCtrl
 * PURPOSE:
    ...
    BOOL_T      *ptr_enable)
{
    ...
    if (AIR_E_OK != rc)
    {
        DIAG_PRINT(HAL_DBG_WARN, "port %u get LED-ID[%u] failed, rc = %d\n", port, led_id, rc);
    }
    return rc;
}

/* FUNCTION NAME: hal_pearl_port_setComboMode
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_getComboMode
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_setPhyLedCtrlMode
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_getPhyLedCtrlMode
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_setPhyLedForceState
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_getPhyLedForceState
 * PURPOSE:
 *      Get led control mode for a specific led of the port.
 *
 * INPUT:
...
/* FUNCTION NAME:   hal_pearl_port_setPhyLedForcePattCfg
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_getPhyLedForcePattCfg
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_triggerCableTest
 * PURPOSE:
 *      Get cable status.
 *
 * INPUT:
...
/* FUNCTION NAME: hal_pearl_port_getCableTestRawData
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_setPhyOpMode
 * PURPOSE:
...
/* FUNCTION NAME:   hal_pearl_port_getPhyOpMode
 * PURPOSE:
...
/* FUNCTION NAME: hal_pearl_port_triggerLinkDownCableTest
 * PURPOSE:
 ...
 */
AIR_ERROR_NO_T
hal_pearl_port_triggerLinkDownCableTest(
    const UI32_T                unit,
    const UI32_T                port,
    AIR_PORT_CABLE_TEST_RSLT_T *ptr_test_rslt)
{
