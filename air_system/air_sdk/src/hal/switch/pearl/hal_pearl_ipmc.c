 ...
 */
#define HAL_PEARL_IPMC_WRITE_IPV6_GROUP(unit, __ip__, __size__)                                                \
    {                                                                                                          \
        ...
        aml_writeReg(unit, PEARL_ATA7, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[4], __size__);                                         \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA5, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[8], __size__);                                         \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA3, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[12], __size__);                                        \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA1, &reversed_addr, __size__);                                              \
    }

#define HAL_PEARL_IPMC_WRITE_IPV6_SOURCE(unit, __ip__, __size__)                                               \
    {                                                                                                          \
        UI32_T reversed_addr = 0, ip6addr = 0;                                                                 \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[0], __size__);                                         \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA8, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[4], __size__);                                         \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA6, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[8], __size__);                                         \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA4, &reversed_addr, __size__);                                              \
        osal_memcpy(&ip6addr, &__ip__.ip_addr.ipv6_addr[12], __size__);                                        \
        reversed_addr =                                                                                        \
            (ip6addr >> 24) | ((ip6addr >> 8) & 0x0000FF00) | ((ip6addr << 8) & 0x00FF0000) | (ip6addr << 24); \
        aml_writeReg(unit, PEARL_ATA2, &reversed_addr, __size__);                                              \
    }

#define HAL_PEARL_IPMC_U32_ENDIAN_XCHG(__ip__)                                                              \
 ...
 */
DIAG_SET_MODULE_INFO(AIR_MODULE_IPMC, "hal_pearl_ipmc.c");
    ...
    AIR_PORT_BITMAP_T *p_portmap)
{
    ...
    aml_writeReg(unit, PEARL_ATA1, &gaddr, sizeof(u32dat));
    u32dat |= (vid << 16);
    ...
    if (AIR_E_OK == rc)
    {
        ...
        DIAG_PRINT(HAL_DBG_INFO, "u32dat=(0x%x), addr=(0x%x), banks(0x%x).\n", u32dat,
                   BITS_OFF_R(u32dat, PEARL_ATC_ADDR_OFFSET, PEARL_ATC_ADDR_LENGTH), banks);
        if (banks)
        {
            for (i = 0; i < HAL_PEARL_L2_MAC_SET_NUM; i++)
            {
                ...
                {
                    /* Select bank */
                    u32dat = BITS_OFF_L(i, PEARL_ATRD0_MAC_SEL_OFFSET, PEARL_ATRD0_MAC_SEL_LENGTH);
                    ...
                    if (u32dat == gaddr)
                    {
                        ...
                        if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE) == vid)
                        {
    ...
    AIR_PORT_BITMAP_T *p_portmap)
{
    ...
    aml_writeReg(unit, PEARL_ATA1, &gaddr, sizeof(UI32_T));
    ...
    while (TRUE == search_next)
    {
        ...
        {
            /* Get address */
            addr = BITS_OFF_R(u32dat, PEARL_ATC_ADDR_OFFSET, PEARL_ATC_ADDR_LENGTH);
            /* Get banks */
            banks = BITS_OFF_R(u32dat, PEARL_ATC_ENTRY_HIT_OFFSET, PEARL_ATC_ENTRY_HIT_LENGTH);
            DIAG_PRINT(HAL_DBG_INFO, "u32dat=(0x%x), addr=(0x%x), banks=(0x%x)\n", u32dat, addr, banks);
            if (banks)
            ...
            if (((HAL_PEARL_L2_MAX_ADDR_NUM - 1) == addr) && (TRUE == search_next))
            {
    ...
    AIR_PORT_BITMAP_T  *p_portmap)
{
    ...
    {
        HAL_PEARL_IPMC_WRITE_IPV6_GROUP(unit, gaddr, sizeof(UI32_T));
    ...
    while (TRUE == search_next)
    {
        ...
        if (AIR_E_OK == rc)
        {
            ...
            if (banks == 0xF) /*IPv6 occupy all banks*/
            {
                ...
                if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE) == vid)
                {
                    for (bank = 0; bank < HAL_PEARL_L2_MAC_SET_NUM; bank++)
                    {
                        aml_writeReg(unit, PEARL_ATRDS, &bank, sizeof(bank));
                        aml_readReg(unit, PEARL_ATRD1, &u32dat, sizeof(u32dat));
                        HAL_PEARL_IPMC_U32_ENDIAN_XCHG(u32dat);
                        if (u32dat == group_addr6[HAL_PEARL_L2_MAC_SET_NUM - (bank + 1)])
                        {
                            hit++;
                        }
                        else
                    ...
                    if (hit == HAL_PEARL_L2_MAC_SET_NUM)
                    {
                        aml_readReg(unit, PEARL_ATRD3, &u32dat, sizeof(u32dat));
                        (*p_portmap)[0] |= u32dat;
                        HAL_AIR_PBMP_TO_MAC_PBMP(unit, HAL_PORT_BMP_ETH(unit), filter_mac_port_bitmap);
                ...
                if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE) == vid)
                {
                    if (!(CMLIB_UTIL_IP_ADDR_IS_ZERO(saddr)))
                    {
                        for (bank = 0; bank < HAL_PEARL_L2_MAC_SET_NUM; bank++)
                        {
                            ...
                            aml_readReg(unit, PEARL_ATRD2, &u32dat, sizeof(u32dat));
                            HAL_PEARL_IPMC_U32_ENDIAN_XCHG(u32dat);
                            if (u32dat == source_addr6[HAL_PEARL_L2_MAC_SET_NUM - (bank + 1)])
                            {
                                hit++;
                            }
                            else
    ...
    const AIR_IPMC_TYPE_T type)
{
    ...
    {
        /* set igmpv3 */
        u32dat |= PEARL_ISC_CSR_IGMPV3_EN;
    }
    ...
    if (AIR_E_OK == rc)
    {
        aml_writeReg(unit, PEARL_ISC, &u32dat, sizeof(UI32_T));
    }
...
{
    UI32_T u32dat = 0;
    aml_readReg(unit, PEARL_ISC, &u32dat, sizeof(UI32_T));

    if (u32dat & PEARL_ISC_CSR_IGMPV3_EN)
    {
    ...
    if (ptr_entry->group_addr.ipv4 == TRUE)
    {
                          ...
                          ptr_entry->vid, &curr_portmap) == AIR_E_ENTRY_NOT_FOUND)
        {
            ...
            if ((ret == AIR_E_ENTRY_NOT_FOUND) || (ptr_entry->type == AIR_IPMC_TYPE_GRP_SRC))
            {
                ...
                /* Set entry type DIP */
                u32dat = 0;
                u32dat |= (TRUE << PEARL_ATWD_IPM_VLD_OFFSET);
                /* Set attributes */
                if (ptr_entry->flags & AIR_IPMC_ENTRY_FLAGS_DISABLE_EGRESS_VLAN_FILTER)
                {
                    u32dat |= (1UL << PEARL_ATWD_IPM_LEAKY_OFFSET);
                }
                u32dat |= (ptr_entry->vid << PEARL_ATWD_IPM_VID_OFFSET);
                aml_writeReg(unit, PEARL_ATWD, &u32dat, sizeof(u32dat));
                ...
                /* Set DIP address */
                aml_writeReg(unit, PEARL_ATA1, &ptr_entry->group_addr.ip_addr.ipv4_addr, sizeof(u32dat));
                ...
                {
                    /* Set source address */
                    aml_writeReg(unit, PEARL_ATA2, &ptr_entry->source_addr.ip_addr.ipv4_addr, sizeof(u32dat));
                }
    ...
    else
    {
            ...
            AIR_E_ENTRY_NOT_FOUND)
        {
            ...
            if ((ret == AIR_E_ENTRY_NOT_FOUND) || (ptr_entry->type == AIR_IPMC_TYPE_GRP_SRC))
            {
                ...
                u32dat = 0;
                u32dat |= (TRUE << PEARL_ATWD_IPM_VLD_OFFSET);
                u32dat |= (TRUE << PEARL_ATWD_IPM_IPV6_OFFSET);
                /* Set attributes */
                if (ptr_entry->flags & AIR_IPMC_ENTRY_FLAGS_DISABLE_EGRESS_VLAN_FILTER)
                {
                    u32dat |= (1UL << PEARL_ATWD_IPM_LEAKY_OFFSET);
                ...
                /* Set DIP address */
                HAL_PEARL_IPMC_WRITE_IPV6_GROUP(unit, ptr_entry->group_addr, sizeof(u32dat));

                /* For IGMPv3 write source ip table */
                if ((ptr_entry->type == AIR_IPMC_TYPE_GRP_SRC) && !(CMLIB_UTIL_IP_ADDR_IS_ZERO(ptr_entry->source_addr)))
                {
                    /* Set SIP address */
                    HAL_PEARL_IPMC_WRITE_IPV6_SOURCE(unit, ptr_entry->source_addr, sizeof(u32dat));
                }
    ...
    AIR_IPMC_ENTRY_T *ptr_entry)
{
    ...
    if (ptr_entry->group_addr.ipv4 == TRUE)
    {
        ...
        atwd |= (((UI32_T)ptr_entry->vid) << PEARL_ATWD_IPM_VID_OFFSET);
        aml_writeReg(unit, PEARL_ATWD, &atwd, sizeof(atwd));
        aml_writeReg(unit, PEARL_ATA1, &ptr_entry->group_addr.ip_addr.ipv4_addr, sizeof(u32dat));
        ...
        aml_writeReg(unit, PEARL_ATC, &u32dat, sizeof(u32dat));
        // osal_memset(ptr_gIf, 0, sizeof(GROUPINFO_T)*HAL_PEARL_L2_MAC_SET_NUM);
        ...
        if (banks)
        {
            for (i = 0; i < HAL_PEARL_L2_MAC_SET_NUM; i++)
            {
                if (TRUE == BITS_OFF_R(banks, i, 1))
                {
                    ...
                    /* Get attributes */
                    aml_readReg(unit, PEARL_ATRD0, &u32dat, sizeof(u32dat));
                    vid = (UI16_T)BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE);
                    if (ptr_entry->vid != vid)
                    {
                        continue;
                    }
                    if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_LEAKY_OFFSET, PEARL_ATRD0_IPM_LEAKY_RANGE))
                    {
        ...
        {
            if ((HAL_PEARL_L2_MAX_ADDR_NUM - 1) == (addr))
            {
                DIAG_PRINT(HAL_DBG_INFO, "addr=(%d), return ENTRY_NOT_FOUND\n", addr);
            }
            HAL_PEARL_L2_FDB_UNLOCK(unit);
    ...
    {
        /* Search DIP_SIP table */
        atwd |= (ptr_entry->vid << PEARL_ATWD_IPM_VID_OFFSET);
        atwd |= (TRUE << PEARL_ATWD_IPM_IPV6_OFFSET);
        aml_writeReg(unit, PEARL_ATWD, &atwd, sizeof(atwd));
        HAL_PEARL_IPMC_WRITE_IPV6_GROUP(unit, ptr_entry->group_addr, sizeof(u32dat));
        ...
        {
            aml_readReg(unit, PEARL_ATRD0, &u32dat, sizeof(u32dat));
            vid = BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE);
            if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_LEAKY_OFFSET, PEARL_ATRD0_IPM_LEAKY_RANGE))
            {
                ptr_entry->flags |= AIR_IPMC_ENTRY_FLAGS_DISABLE_EGRESS_VLAN_FILTER;
            }
            ...
            for (i = 0; i < HAL_PEARL_L2_MAC_SET_NUM; i++)
            {
                ...
                osal_memcpy(&group_addr.ip_addr.ipv6_addr[(HAL_PEARL_L2_MAC_SET_NUM - (i + 1)) * 4], &u32dat,
                            sizeof(u32dat));
                if (ptr_entry->type == AIR_IPMC_TYPE_GRP_SRC)
                {
                    aml_readReg(unit, PEARL_ATRD2, &u32dat, sizeof(u32dat));
                    ...
                    osal_memcpy(&source_addr.ip_addr.ipv6_addr[(HAL_PEARL_L2_MAC_SET_NUM - (i + 1)) * 4], &u32dat,
                                sizeof(u32dat));
                }
            }
            /* group address match */
    ...
    const AIR_IPMC_ENTRY_T *ptr_entry)
{
    ...
    if (ptr_entry->group_addr.ipv4 == TRUE)
    {
        ...
        /* Set DIP STATUS = 0 */
        u32dat = 0;
        u32dat |= (FALSE << PEARL_ATWD_IPM_VLD_OFFSET);
        u32dat |= (ptr_entry->vid << PEARL_ATWD_IPM_VID_OFFSET);
        aml_writeReg(unit, PEARL_ATWD, &u32dat, sizeof(u32dat));
    ...
    if (!BITS_OFF_R(u32dat, PEARL_ATC_SINGLE_HIT_OFFSET, PEARL_ATC_SINGLE_HIT_LENGTH))
    {
        HAL_PEARL_L2_FDB_UNLOCK(unit);
        return AIR_E_ENTRY_NOT_FOUND;
    }
    ...
    AIR_IPMC_ENTRY_T *ptr_entry)
{
    ...
    if (ptr_entry->group_addr.ipv4 == TRUE)
    {
        ...
        for (i = 0; i < (HAL_PEARL_MAX_NUM_OF_PORTS - 1); i++)
        {
        ...
        atwd |= (TRUE << PEARL_ATWD_IPM_VLD_OFFSET);
        atwd |= (ptr_entry->vid << PEARL_ATWD_IPM_VID_OFFSET);
        aml_writeReg(unit, PEARL_ATWD, &atwd, sizeof(atwd));
        u32dat = 0;
        ...
        if (banks)
        {
            for (i = 0; i < HAL_PEARL_L2_MAC_SET_NUM; i++)
            {
                ...
                if (TRUE == BITS_OFF_R(banks, i, 1))
                {
                    ...
                    /* Get attributes */
                    aml_readReg(unit, PEARL_ATRD0, &u32dat, sizeof(u32dat));
                    ptr_entry->vid = (UI16_T)BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_VID_OFFSET, PEARL_ATRD0_IPM_VID_RANGE);
                    if (BITS_OFF_R(u32dat, PEARL_ATRD0_IPM_LEAKY_OFFSET, PEARL_ATRD0_IPM_LEAKY_RANGE))
                    {
                        ptr_entry->flags |= AIR_IPMC_ENTRY_FLAGS_DISABLE_EGRESS_VLAN_FILTER;
                    }
            ...
            if ((HAL_PEARL_L2_MAX_ADDR_NUM - 1) == (addr) && (*ptr_entry_cnt) == 0)
            {
                DIAG_PRINT(HAL_DBG_INFO, "u32dat=(0x%x), addr=(0x%x), banks=(0x%x)\n", u32dat, addr, banks);
                HAL_PEARL_L2_FDB_UNLOCK(unit);
                return AIR_E_ENTRY_NOT_FOUND;
    ...
    else if (AIR_IPMC_MATCH_TYPE_IPV6_GRP == match_type || AIR_IPMC_MATCH_TYPE_IPV6_GRP_SRC == match_type)
    {
        ...
        if (banks == 0xF)
        {
            ...
            for (i = 0; i < HAL_PEARL_L2_MAC_SET_NUM; i++)
            {
                ...
                osal_memcpy(&ptr_entry->group_addr.ip_addr.ipv6_addr[(HAL_PEARL_L2_MAC_SET_NUM - (i + 1)) * 4], &u32dat,
                            sizeof(u32dat));
                if (AIR_IPMC_MATCH_TYPE_IPV6_GRP_SRC == match_type)
                {
                    ...
                    osal_memcpy(&ptr_entry->source_addr.ip_addr.ipv6_addr[(HAL_PEARL_L2_MAC_SET_NUM - (i + 1)) * 4],
    ...
    const BOOL_T enable)
{
    ...
    if (TRUE == enable)
    {
        ...
        u32dat |= (PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_01 | PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_33 |
                   PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_224);
    }
    else
    {
    ...
    aml_writeReg(unit, PEARL_PIC(mac_port), &u32dat, sizeof(UI32_T));
    ...
    BOOL_T      *ptr_enable)
{
    ...
    aml_readReg(unit, PEARL_PIC(mac_port), &u32dat, sizeof(UI32_T));
    if (u32dat == (PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_01 | PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_33 |
                   PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_224))
    {
        *ptr_enable = TRUE;
    }
    ...
    UI32_T              *ptr_cnt)
{
    ...
    UI32_T         mat[2] = {PEARL_ATC_MAT_DIP, PEARL_ATC_MAT_SIP};
    ...
    if (AIR_E_OK == rc)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            reg_atc = (HAL_CMD_SEARCH | mat[i] | HAL_CMD_START);
            aml_writeReg(unit, PEARL_ATC, &reg_atc, sizeof(reg_atc));
            /* Check write state */
            rc = _checkAtcBusy(unit, &reg_atc);
            /* Get address */
            addr = BITS_OFF_R(reg_atc, PEARL_ATC_ADDR_OFFSET, PEARL_ATC_ADDR_LENGTH);
            /* Get banks */
            banks = BITS_OFF_R(reg_atc, PEARL_ATC_ENTRY_HIT_OFFSET, PEARL_ATC_ENTRY_HIT_LENGTH);
            ...
            while ((AIR_E_OK == rc) && (0 != banks))
            {
                ...
                reg_atc = (HAL_CMD_SEARCH_NEXT | mat[i] | HAL_CMD_START);
                aml_writeReg(unit, PEARL_ATC, &reg_atc, sizeof(reg_atc));
                /* Check write state */
                rc = _checkAtcBusy(unit, &reg_atc);
                /* Get address */
    ...
    const BOOL_T        enable)
{
    ...
    {
        cfg_val = (PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_01 | PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_224);
    }
    else
    {
        cfg_val = PEARL_PIC_PORT_IGMP_CTRL_CSR_IPM_33;
    }
