 ...
 */
DIAG_SET_MODULE_INFO(AIR_MODULE_STP, "hal_pearl_stp.c");
    ...
    const AIR_STP_STATE_T state)
{
    ...
    aml_readReg(unit, PEARL_SSC(mac_port), &u32dat, sizeof(u32dat));

    /* Write data to register */
    u32dat &= ~BITS(fid * 2, (fid * 2) + 1);
    u32dat |= BITS_OFF_L(state, (fid * 2), 2);
    aml_writeReg(unit, PEARL_SSC(mac_port), &u32dat, sizeof(u32dat));
