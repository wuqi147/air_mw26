 ...
 */
#define HAL_CORAL_IFMON_SEM_CONF(unit) _ptr_hal_coral_ifmon_cb[unit]->sem_conf

#define HAL_CORAL_IFMON_SEM_PORT(unit) _ptr_hal_coral_ifmon_cb[unit]->sem_portstatus

#define HAL_CORAL_IFMON_MONITOR_STATE(unit) _ptr_hal_coral_ifmon_cb[unit]->monitor_state

#define HAL_CORAL_IFMON_THREAD_ID(unit) _ptr_hal_coral_ifmon_cb[unit]->thread_id

#define HAL_CORAL_IFMON_MODE(unit) _ptr_hal_coral_ifmon_cb[unit]->mode

#define HAL_CORAL_IFMON_INTERVAL(unit) _ptr_hal_coral_ifmon_cb[unit]->interval

#define HAL_CORAL_IFMON_SCAN_PORT_BITMAP(unit) _ptr_hal_coral_ifmon_cb[unit]->scan_port_bitmap

#define HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit) _ptr_hal_coral_ifmon_cb[unit]->cur_state.link_bitmap

#define HAL_CORAL_IFMON_NEW_LINK_BITMAP(unit) _ptr_hal_coral_ifmon_cb[unit]->new_state.link_bitmap

#define HAL_CORAL_IFMON_DEV_LINK_BITMAP(unit) _ptr_hal_coral_ifmon_cb[unit]->dev_state.link_bitmap

#define PTR_CORAL_IFMON_PORT_LINK_STATUS(__unit__, __port__)     \
    (&(_ptr_hal_coral_ifmon_cb[__unit__]->dev_status[__port__]))

#define PTR_HAL_CORAL_IFMON_NOTIFY_HANDLER(__unit__, __idx__)       \
    (&(_ptr_hal_coral_ifmon_cb[__unit__]->notify_handler[__idx__]))
 ...
 */
DIAG_SET_MODULE_INFO(AIR_MODULE_IFMON, "hal_coral_ifmon.c");

static HAL_CORAL_IFMON_CB_T *_ptr_hal_coral_ifmon_cb[AIR_CFG_MAXIMUM_CHIPS_PER_SYSTEM];
/* LOCAL SUBPROGRAM BODIES
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_processCallbackFunc(
    const UI32_T unit,
    const UI32_T port,
    const BOOL_T link)
{
    HAL_CORAL_IFMON_NOTIFY_HANDLER_T *ptr_handler = NULL;
    UI32_T                            idx = 0;

    for (idx = 0; idx < HAL_CORAL_IFMON_NOTIFY_HANDLER_CNT; idx++)
    {
        ptr_handler = PTR_HAL_CORAL_IFMON_NOTIFY_HANDLER(unit, idx);
...
static AIR_ERROR_NO_T
_hal_coral_ifmon_updatePortLink(
    const UI32_T unit,
    const UI32_T port,
    ...
    const BOOL_T new_link)
{
    ...
    if (TRUE == notify)
    {
        rc = _hal_coral_ifmon_processCallbackFunc(unit, port, tx_en);
    }
...
/* FUNCTION NAME:   _hal_coral_ifmon_updateHwLinkStatus
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_updateHwLinkStatus(
    const UI32_T      unit,
    AIR_PORT_BITMAP_T link_bitmap)
{
    ...
    AIR_PORT_FOREACH(HAL_CORAL_IFMON_SCAN_PORT_BITMAP(unit), port)
    {
        rc = hal_coral_port_getPortStatus(unit, port, &port_status);
        if ((AIR_E_OK != rc) && (AIR_E_NOT_SUPPORT != rc))
        {
            DIAG_PRINT(HAL_DBG_WARN, " _hal_coral_ifmon_updateHwLinkStatus: unit %u port %u rc %d\n", unit, port, rc);
            continue;
        }
        ...
        ptr_dev_status = PTR_CORAL_IFMON_PORT_LINK_STATUS(unit, port);

        HAL_CORAL_IFMON_PORT_LOCK(unit); /* lock */
        ptr_dev_status->speed = port_status.speed;
        ptr_dev_status->duplex = port_status.duplex;
        ptr_dev_status->flags = port_status.flags;
        HAL_CORAL_IFMON_PORT_UNLOCK(unit); /* unlock */
    }

    return rc;
}

/* FUNCTION NAME:   _hal_coral_ifmon_processDevState
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_processDevState(
    const UI32_T unit)
{
    AIR_ERROR_NO_T rc = AIR_E_OK;

    rc = _hal_coral_ifmon_updateHwLinkStatus(unit, HAL_CORAL_IFMON_DEV_LINK_BITMAP(unit));
    CMLIB_PORT_BITMAP_SET(HAL_CORAL_IFMON_NEW_LINK_BITMAP(unit), HAL_CORAL_IFMON_DEV_LINK_BITMAP(unit));

    return rc;
}

/* FUNCTION NAME:   _hal_coral_ifmon_processLink
 * PURPOSE:
 *      To process link.
 * INPUT:
 *      unit                --  Device unit number
    ...
    const UI32_T unit)
{
    ...
    link_equal = AIR_PORT_BITMAP_EQUAL(HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit), HAL_CORAL_IFMON_NEW_LINK_BITMAP(unit));
    ...
    CMLIB_PORT_BITMAP_SET(changed_link_bitmap, HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit));
    CMLIB_PORT_BITMAP_XOR(changed_link_bitmap, HAL_CORAL_IFMON_NEW_LINK_BITMAP(unit));
    ...
    {
        cur_link = AIR_PORT_CHK(HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit), port) ? TRUE : FALSE;

        DIAG_PRINT(HAL_DBG_INFO, " Update unit %u port %u cur_link %d\n", unit, port, cur_link);

        /* Get new status */
        new_link = AIR_PORT_CHK(HAL_CORAL_IFMON_NEW_LINK_BITMAP(unit), port) ? TRUE : FALSE;
        ...
        if (new_link != cur_link)
        {
            if (TRUE == new_link)
            {
                AIR_PORT_ADD(HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit), port);
            }
            else
            {
                AIR_PORT_DEL(HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit), port);
            }

            rc = _hal_coral_ifmon_updatePortLink(unit, port, cur_link, new_link);
        }
    }
...
/* FUNCTION NAME:   _hal_coral_ifmon_thread
 * PURPOSE:
 *      IfMon thread.
 * INPUT:
 ...
 */
static void
_hal_coral_ifmon_thread(
    void *ptr_argv)
{
    UI32_T         unit = (UI32_T)(AIR_HUGE_T)ptr_argv;
    UI32_T         interval = HAL_CORAL_IFMON_INTERVAL_DFLT;
    AIR_ERROR_NO_T rc = AIR_E_OK;
    ...
    while (AIR_E_OK == osal_isRunThread())
    {
        ...
        HAL_CORAL_IFMON_CFG_LOCK(unit);

        interval = HAL_CORAL_IFMON_INTERVAL(unit);

        if (TRUE != HAL_CORAL_IFMON_MONITOR_STATE(unit))
        {
            HAL_CORAL_IFMON_CFG_UNLOCK(unit);
            continue;
        }

        rc = _hal_coral_ifmon_processDevState(unit);
        if (AIR_E_OK == rc)
        {
            rc = _hal_coral_ifmon_processLink(unit);
        }
...
/* FUNCTION NAME:   _hal_coral_ifmon_initThread
 * PURPOSE:
    ...
    const UI32_T unit)
{
    ...
    if (AIR_E_OK == rc)
    {
        ...
        {
            HAL_CORAL_IFMON_MONITOR_STATE(unit) = FALSE;
            rc = AIR_E_OK;
        }
        else /* TRUE == ifmon_en.value */
        {
            osal_memset(&cfg_pri, 0, sizeof(AIR_CFG_VALUE_T));
            cfg_pri.value = HAL_CORAL_IFMON_THREAD_PRI;
            rc = hal_cfg_getValue(unit, AIR_CFG_TYPE_IFMON_THREAD_PRI, &cfg_pri);
            if (AIR_E_OK == rc)
            {
                osal_memset(&cfg_stack_size, 0, sizeof(AIR_CFG_VALUE_T));
                cfg_stack_size.value = HAL_CORAL_IFMON_STACK_SIZE;
                rc = hal_cfg_getValue(unit, AIR_CFG_TYPE_IFMON_THREAD_STACK, &cfg_stack_size);
                if (AIR_E_OK == rc)
                {
                    rc = osal_createThread("IFMON", (UI32_T)cfg_stack_size.value, (UI32_T)cfg_pri.value,
                                           _hal_coral_ifmon_thread, (void *)((AIR_HUGE_T)unit),
                                           &(HAL_CORAL_IFMON_THREAD_ID(unit)));
                }
            }
        }
    }
...
/* FUNCTION NAME:   _hal_coral_ifmon_deinitThread
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_deinitThread(
    const UI32_T unit)
{
    AIR_ERROR_NO_T rc = AIR_E_OK;

    rc = osal_stopThread(&(HAL_CORAL_IFMON_THREAD_ID(unit)));
    if (AIR_E_OK == rc)
    {
        rc = osal_destroyThread(&(HAL_CORAL_IFMON_THREAD_ID(unit)));
    }

    return rc;
}

/* FUNCTION NAME:   _hal_coral_ifmon_initRsrc
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_initRsrc(
    const UI32_T unit)
{
    HAL_CORAL_IFMON_CB_T *ptr_cb = NULL;
    AIR_ERROR_NO_T        rc = AIR_E_OK;
    UI32_T                size = 0;

    size = sizeof(HAL_CORAL_IFMON_CB_T);
    ptr_cb = (HAL_CORAL_IFMON_CB_T *)osal_alloc(size, air_module_getModuleName(AIR_MODULE_IFMON));
    HAL_CHECK_PTR(ptr_cb);
    osal_memset(ptr_cb, 0, sizeof(HAL_CORAL_IFMON_CB_T));
    _ptr_hal_coral_ifmon_cb[unit] = ptr_cb;
...
/* FUNCTION NAME:   _hal_coral_ifmon_deinitRsrc
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_deinitRsrc(
    const UI32_T unit)
{
    HAL_CORAL_IFMON_CB_T *ptr_cb = NULL;
    AIR_ERROR_NO_T        rc = AIR_E_OK;

    ptr_cb = _ptr_hal_coral_ifmon_cb[unit];
    ...
    if (AIR_E_OK == rc)
    {
        ...
        {
            osal_free(ptr_cb);
            _ptr_hal_coral_ifmon_cb[unit] = NULL;
        }
    }
...
/* FUNCTION NAME:   _hal_coral_ifmon_initCfg
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_initCfg(
    const UI32_T unit)
{
    HAL_CORAL_IFMON_MODE(unit) = HAL_CORAL_IFMON_MODE_DFLT;
    HAL_CORAL_IFMON_INTERVAL(unit) = HAL_CORAL_IFMON_INTERVAL_DFLT;
    HAL_CORAL_IFMON_MONITOR_STATE(unit) = TRUE;
    CMLIB_BITMAP_SET(HAL_CORAL_IFMON_SCAN_PORT_BITMAP(unit), HAL_PORT_BMP_ETH(unit), AIR_PORT_BITMAP_SIZE);

    return AIR_E_OK;
}

/* FUNCTION NAME:   _hal_coral_ifmon_allocNotifyHandler
 * PURPOSE:
 ...
 *      None
 */
static HAL_CORAL_IFMON_NOTIFY_HANDLER_T *
_hal_coral_ifmon_allocNotifyHandler(
    const UI32_T unit)
{
    HAL_CORAL_IFMON_NOTIFY_HANDLER_T *ptr_handler = NULL;
    UI32_T                            idx = 0;
    ...
    if (HAL_CORAL_IFMON_NOTIFY_HANDLER_CNT != idx)
    {
...
/* FUNCTION NAME:   _hal_coral_ifmon_freeNotifyHandler
 * PURPOSE:
 ...
 */
static void
_hal_coral_ifmon_freeNotifyHandler(
    const UI32_T                      unit,
    HAL_CORAL_IFMON_NOTIFY_HANDLER_T *ptr_handler)
{
...
/* FUNCTION NAME:   _hal_coral_ifmon_getNotifyHandler
 * PURPOSE:
 ...
 */
static AIR_ERROR_NO_T
_hal_coral_ifmon_getNotifyHandler(
    const UI32_T                       unit,
    const AIR_IFMON_NOTIFY_FUNC_T      notify_func,
    void                              *ptr_cookie,
    HAL_CORAL_IFMON_NOTIFY_HANDLER_T **pptr_handler)
{
    AIR_ERROR_NO_T                    rc = AIR_E_OK;
    HAL_CORAL_IFMON_NOTIFY_HANDLER_T *ptr_handler = NULL;
    UI32_T                            idx = 0;
    ...
    const UI32_T unit)
{
    ...
    if (AIR_E_OK == rc)
    {
        ...
        {
            rc = _hal_coral_ifmon_initRsrc(unit);

            if (AIR_E_OK == rc)
            {
                rc = _hal_coral_ifmon_initCfg(unit);
            }
        }

        if (HAL_INIT_STAGE(unit) & HAL_INIT_STAGE_TASK)
        {
            rc = _hal_coral_ifmon_initThread(unit);
        }
    }
    ...
    const UI32_T unit)
{
    ...
    if (AIR_E_OK == rc)
    {
        if (HAL_INIT_STAGE(unit) & HAL_INIT_STAGE_TASK_RSRC)
        {
            rc = _hal_coral_ifmon_deinitRsrc(unit);
        }

        if (HAL_INIT_STAGE(unit) & HAL_INIT_STAGE_TASK)
        {
            HAL_CORAL_IFMON_MONITOR_STATE(unit) = FALSE;
            rc = _hal_coral_ifmon_deinitThread(unit);
        }
    }
    ...
    const UI32_T unit)
{
    ...
    return HAL_COMMON_LOCK_RESOURCE(&(HAL_CORAL_IFMON_SEM_CONF(unit)), AIR_SEMAPHORE_WAIT_FOREVER);
}
    ...
    const UI32_T unit)
{
    ...
    return HAL_COMMON_FREE_RESOURCE(&(HAL_CORAL_IFMON_SEM_CONF(unit)));
}
    ...
    const UI32_T unit)
{
    ...
    return HAL_COMMON_LOCK_RESOURCE(&(HAL_CORAL_IFMON_SEM_PORT(unit)), AIR_SEMAPHORE_WAIT_FOREVER);
}
    ...
    const UI32_T unit)
{
    ...
    return HAL_COMMON_FREE_RESOURCE(&(HAL_CORAL_IFMON_SEM_PORT(unit)));
}
    ...
    void                         *ptr_cookie)
{
    ...
    if (HAL_MODULE_INITED(unit, AIR_MODULE_IFMON) & HAL_INIT_STAGE_TASK_RSRC)
    {
        ...
        ptr_handler = _hal_coral_ifmon_allocNotifyHandler(unit);
        if (NULL != ptr_handler)
        {
    ...
    DIAG_PRINT(HAL_DBG_INFO, " hal_coral_ifmon_register: unit %u, rc %d\n", unit, rc);
    ...
    void                         *ptr_cookie)
{
    ...
    if (HAL_MODULE_INITED(unit, AIR_MODULE_IFMON) & HAL_INIT_STAGE_TASK_RSRC)
    {
        ...
        rc = _hal_coral_ifmon_getNotifyHandler(unit, notify_func, ptr_cookie, &ptr_handler);
        if (AIR_E_OK == rc)
        {
            _hal_coral_ifmon_freeNotifyHandler(unit, ptr_handler);
        }

        HAL_CORAL_IFMON_CFG_UNLOCK(unit);
    }

    DIAG_PRINT(HAL_DBG_INFO, " hal_coral_ifmon_deregister: unit %u, rc %d\n", unit, rc);
    ...
    const UI32_T            interval)
{
    ...
    HAL_CORAL_IFMON_MODE(unit) = mode;
    HAL_CORAL_IFMON_INTERVAL(unit) = interval;
    CMLIB_PORT_BITMAP_SET(HAL_CORAL_IFMON_SCAN_PORT_BITMAP(unit), port_bitmap);
    ...
    UI32_T            *ptr_interval)
{
    ...
    *ptr_mode = HAL_CORAL_IFMON_MODE(unit);
    *ptr_interval = HAL_CORAL_IFMON_INTERVAL(unit);
    CMLIB_PORT_BITMAP_SET(*ptr_port_bitmap, HAL_CORAL_IFMON_SCAN_PORT_BITMAP(unit));
    ...
    const BOOL_T enable)
{
    ...
    HAL_CORAL_IFMON_MONITOR_STATE(unit) = enable;

    if (TRUE == enable)
    {
        /* after enable, all link up port will be notified once */
        CMLIB_BITMAP_CLEAR(HAL_CORAL_IFMON_CUR_LINK_BITMAP(unit), AIR_PORT_BITMAP_SIZE);
    }
    ...
    BOOL_T      *ptr_enable)
{
    ...
    *ptr_enable = HAL_CORAL_IFMON_MONITOR_STATE(unit);
